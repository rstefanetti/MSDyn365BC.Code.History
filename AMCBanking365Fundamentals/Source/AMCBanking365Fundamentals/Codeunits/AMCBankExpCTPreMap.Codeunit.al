codeunit 20112 "AMC Bank Exp. CT Pre-Map"
{
    ...
    begin
        ...
        with PaymentExportData do begin
            ...
            repeat
                ...
                "End-to-End ID" := "Message ID" + '/' + Format("Line No.") + 'US'; //AMC-JN : Making uniq key we added US, because we want to use the same for Payment Information Id for later use.
                "Payment Information ID" := "Message ID" + '/' + FORMAT("Line No.") + 'TH'; //AMC-JN : Making uniq key we added TH, because we want to use the same for End-to-end Id for later use.
                "Applies-to Ext. Doc. No." := GenJnlLine."Applies-to Ext. Doc. No.";
                "Short Advice" := GenJnlLine."Document No.";
                "Recipient Creditor No." := GenJnlLine."Creditor No.";

                "Invoice Amount" := ABS(GenJnlLine.Amount);  //AMC-JN
                "Invoice Date" := GenJnlLine."Posting Date"; //AMC-JN
                ...
                if (GenJnlLine."Currency Code" <> '') then //AMC-JN This is to handle BC users that by mistake has set CurrencyCode to the same as GeneralLedgerSetup."LCY Code"
                ...
                "Sender Bank Name - Data Conv." := BankAccount."AMC Bank Name"; // AMC-JN Moved here from above function SetBankAsSenderBank
                ...
                Insert(true);
                MakeBankSpecTrans(PaymentExportData, GenJnlLine, SpecLineNo); // AMC-JN : Make banktransspec in Table 1206 for use in XMLPORT 51232
    ...
    begin
        //--> AMC-JN : Make banktransspec in Table 1206 for use in XMLPORT 51232
