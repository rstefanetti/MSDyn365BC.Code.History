        ...
        with CusLedgerEntry do begin
            ...
            if AdjAmount <> 0 then begin
                ...
                CalcFields(
                  Amount, "Amount (LCY)", "Remaining Amount", "Remaining Amt. (LCY)", "Original Amt. (LCY)",
                  "Debit Amount", "Credit Amount", "Debit Amount (LCY)", "Credit Amount (LCY)");
                if ("Document Situation" <> 0) and CarteraSetup.ReadPermission then
                    case "Document Situation" of
                        "Document Situation"::Cartera:
                            if CarteraDoc.Get(CarteraDoc.Type::Receivable, "Entry No.") then begin
                                CarteraDoc."Remaining Amt. (LCY)" := "Remaining Amt. (LCY)" + AdjAmount;
                                CarteraDoc."Original Amount (LCY)" := "Original Amt. (LCY)" + AdjAmount;
                                CarteraDoc."Adjusted Amount" := AdjAmount;
                                CarteraDoc.Adjusted := true;
                                CarteraDoc.Modify();
                            end;
                        "Document Situation"::"Posted BG/PO":
                            if PostedCarteraDoc.Get(PostedCarteraDoc.Type::Receivable, "Entry No.") then begin
                        ...
                        "Document Situation"::"Closed BG/PO", "Document Situation"::"Closed Documents":
                            if ClosedCarteraDoc.Get(ClosedCarteraDoc.Type::Receivable, "Entry No.") then begin
        ...
        with VendLedgerEntry do begin
            ...
            if AdjAmount <> 0 then begin
                ...
                if ("Document Situation" <> 0) and CarteraSetup.ReadPermission then
                    case "Document Situation" of
                        "Document Situation"::Cartera:
                            if CarteraDoc.Get(CarteraDoc.Type::Payable, "Entry No.") then begin
                                CarteraDoc."Remaining Amt. (LCY)" := Abs("Remaining Amt. (LCY)" + AdjAmount);
                                CarteraDoc."Original Amount (LCY)" := Abs("Original Amt. (LCY)" + AdjAmount);
                                CarteraDoc."Adjusted Amount" := -AdjAmount;
                                CarteraDoc.Adjusted := true;
                                CarteraDoc.Modify();
                            end;
                        "Document Situation"::"Posted BG/PO":
                            if PostedCarteraDoc.Get(PostedCarteraDoc.Type::Payable, "Entry No.") then begin
                                PostedCarteraDoc."Remaining Amt. (LCY)" := "Remaining Amt. (LCY)";
                                PostedCarteraDoc."Adjusted Amount" := AdjAmount + OldAdjAmount;
                                PostedCarteraDoc.ReAdjusted := true;
                                PostedCarteraDoc.Modify();
                        ...
                        "Document Situation"::"Closed BG/PO", "Document Situation"::"Closed Documents":
                            if ClosedCarteraDoc.Get(ClosedCarteraDoc.Type::Payable, "Entry No.") then begin
                                ClosedCarteraDoc."Remaining Amt. (LCY)" := "Remaining Amt. (LCY)";
                                ClosedCarteraDoc.Modify();
                            end;
                    end;
