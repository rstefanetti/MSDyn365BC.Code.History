page 232 "Apply Customer Entries"
{
    ...
    var
        ...
        Text013: Label 'The %1 entered must not be before %2.';
    ...
    begin
        ...
        if CalcType = CalcType::Direct then begin
            if ApplyingCustLedgEntry."Entry No." <> 0 then begin
                ...
                NewApplicationDate := CustEntryApplyPostedEntries.GetApplicationDate(Rec);
    ...
    [Scope('OnPrem')]
    procedure AskForDocNoAndApplnDate(var DocumentNo: Code[20]; var ApplicationDate: Date)
    var
        PostApplication: Page "Post Application";
        NewApplicationDate: Date;
    begin
        PostApplication.SetValues(DocumentNo, ApplicationDate);
        if ACTION::OK = PostApplication.RunModal then begin
            PostApplication.GetValues(DocumentNo, NewApplicationDate);
            if NewApplicationDate < ApplicationDate then
                Error(Text013, FieldCaption("Posting Date"), ApplicationDate);
        end else
            Error(Text019);
