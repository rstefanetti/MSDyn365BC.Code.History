codeunit 1535 "Approvals Mgmt."
{
    ...
    begin
        with ApprovalEntry do begin
            ...
            OnBeforeApprovalEntryInsert(ApprovalEntry, ApprovalEntryArgument, WorkflowStepArgument);
            Insert(true);
        end;
    end;
    ...
    begin
        // NAVCZ
        PmtOrdHdr.CalcFields(Amount, "Amount (LCY)");
        ApprovalAmount := PmtOrdHdr.Amount;
    ...
    begin
        ...
        ApprovalAmount := SalesAdvanceLetterHeader."Amount Including VAT";
        ApprovalAmountLCY := CurrencyExchangeRate.ExchangeAmtFCYToLCY(
          SalesAdvanceLetterHeader."Posting Date",
          SalesAdvanceLetterHeader."Currency Code",
          SalesAdvanceLetterHeader."Amount Including VAT",
    ...
    begin
        ...
        ApprovalAmount := PurchAdvanceLetterHeader."Amount Including VAT";
        ApprovalAmountLCY := CurrencyExchangeRate.ExchangeAmtFCYToLCY(
          PurchAdvanceLetterHeader."Posting Date",
          PurchAdvanceLetterHeader."Currency Code",
          PurchAdvanceLetterHeader."Amount Including VAT",
        ...
        case RecRef.Number of
            ...
            DATABASE::"Payment Order Header":
                begin
                    ...
                    CalcPaymentOrderAmount(PaymentOrderHeader, ApprovalAmount, ApprovalAmountLCY);
                    ApprovalEntryArgument."Document Type" := 0;
                    ApprovalEntryArgument."Document No." := PaymentOrderHeader."No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
            ...
            DATABASE::"Sales Advance Letter Header":
                begin
                    ...
                    CalcSalesAdvanceLetterAmount(SalesAdvanceLetterHeader, ApprovalAmount, ApprovalAmountLCY);
                    ApprovalEntryArgument."Document Type" := 0;
                    ApprovalEntryArgument."Document No." := SalesAdvanceLetterHeader."No.";
                    ApprovalEntryArgument."Salespers./Purch. Code" := SalesAdvanceLetterHeader."Salesperson Code";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := SalesAdvanceLetterHeader."Currency Code";
                end;
            DATABASE::"Purch. Advance Letter Header":
                begin
                    RecRef.SetTable(PurchAdvanceLetterHeader);
                    CalcPurchaseAdvanceLetterAmount(PurchAdvanceLetterHeader, ApprovalAmount, ApprovalAmountLCY);
                    ApprovalEntryArgument."Document Type" := 0;
                    ApprovalEntryArgument."Document No." := PurchAdvanceLetterHeader."No.";
                    ApprovalEntryArgument."Salespers./Purch. Code" := PurchAdvanceLetterHeader."Purchaser Code";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
    ...
    begin
        ...
        if UserSetup."Unlimited Bank Approval" or
           ((ApprovalAmountLCY <= UserSetup."Bank Amount Approval Limit") and (UserSetup."Bank Amount Approval Limit" <> 0))
        then
            exit(true);
    ...
    begin
        // NAVCZ
        if (PaymentOrderHeader.Status = PaymentOrderHeader.Status::Open) and
           IsPaymentOrderApprovalsWorkflowEnabled(PaymentOrderHeader)
        then
            Error(PreIssueCheckCashDocErr, PaymentOrderHeader."No.");
    ...
    begin
        // NAVCZ
        if (SalesAdvanceLetterHeader.Status = SalesAdvanceLetterHeader.Status::Open) and
           IsSalesAdvanceLetterApprovalsWorkflowEnabled(SalesAdvanceLetterHeader)
        then
            Error(PrePostCheckSalesAdvanceLetterErr, SalesAdvanceLetterHeader."No.");
    ...
    begin
        // NAVCZ
        if (PurchAdvanceLetterHeader.Status = PurchAdvanceLetterHeader.Status::Open) and
           IsPurchaseAdvanceLetterApprovalsWorkflowEnabled(PurchAdvanceLetterHeader)
        then
            Error(PrePostCheckPurchaseAdvanceLetterErr, PurchAdvanceLetterHeader."No.");
    ...
    begin
        // NAVCZ
        exit(WorkflowManagement.CanExecuteWorkflow(PmtOrdHdr,
    ...
    begin
        // NAVCZ
        exit(WorkflowManagement.CanExecuteWorkflow(SalesAdvanceLetterHeader,
    ...
    begin
        // NAVCZ
        exit(WorkflowManagement.CanExecuteWorkflow(PurchAdvanceLetterHeader,
    ...
    begin
        // NAVCZ
        if not IsPaymentOrderApprovalsWorkflowEnabled(PmtOrdHdr) then
            Error(NoWorkflowEnabledErr);
    ...
    begin
        // NAVCZ
        if not IsSalesAdvanceLetterApprovalsWorkflowEnabled(SalesAdvanceLetterHeader) then
            Error(NoWorkflowEnabledErr);
    ...
    begin
        // NAVCZ
        if not IsPurchaseAdvanceLetterApprovalsWorkflowEnabled(PurchAdvanceLetterHeader) then
            Error(NoWorkflowEnabledErr);
    ...
    begin
        ...
        case RecRef.Number of
            ...
            DATABASE::"Payment Order Header":
                begin
                    RecRef.SetTable(PaymentOrderHeader);
                    PaymentOrderHeader.Validate(Status, PaymentOrderHeader.Status::"Pending Approval");
            ...
            DATABASE::"Sales Advance Letter Header":
                begin
                    RecRef.SetTable(SalesAdvanceLetterHeader);
                    SalesAdvanceLetterHeader.SetStatus(SalesAdvanceLetterHeader.Status::"Pending Approval");
                    Variant := SalesAdvanceLetterHeader;
                end;
            DATABASE::"Purch. Advance Letter Header":
                begin
                    RecRef.SetTable(PurchAdvanceLetterHeader);
                    PurchAdvanceLetterHeader.SetStatus(PurchAdvanceLetterHeader.Status::"Pending Approval");
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeApprovalEntryInsert(var ApprovalEntry: Record "Approval Entry"; ApprovalEntryArgument: Record "Approval Entry"; WorkflowStepArgument: Record "Workflow Step Argument")
    begin
    end;
