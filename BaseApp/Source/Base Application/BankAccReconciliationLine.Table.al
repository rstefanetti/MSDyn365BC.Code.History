table 274 "Bank Acc. Reconciliation Line"
{
    ...
    fields
    {
        ...
        field(11710; "Currency Code"; Code[10])
        {
            ...
            begin
                ...
                if "Currency Code" <> '' then begin
                    ...
                    then
                        "Currency Factor" :=
                          CurrExchRate.ExchangeRate("Transaction Date", "Currency Code");
                end else
                    "Currency Factor" := 0;
        ...
        field(11715; "Statement Amount (LCY)"; Decimal)
        {
            ...
            begin
                // NAVCZ
                if "Currency Code" = '' then begin
                    "Statement Amount" := "Statement Amount (LCY)";
                    Validate("Statement Amount");
                end else begin
                    if CheckFixedCurrency then begin
                        ...
                        "Statement Amount" := Round(
                            CurrExchRate.ExchangeAmtLCYToFCY(
                              "Transaction Date", "Currency Code",
                              "Statement Amount (LCY)", "Currency Factor"),
                            Currency."Amount Rounding Precision")
                    end else begin
                        TestField("Statement Amount (LCY)");
    ...
    begin
        ...
        GLEntry.SetRange("Applies-to ID", '');
    ...
    begin
        // NAVCZ
        if "Statement Amount" > 0 then begin
            "Debit Amount" := "Statement Amount";
            "Credit Amount" := 0;
        end else begin
            "Debit Amount" := 0;
            "Credit Amount" := -"Statement Amount";
        end;

        if GetCurrencyCode = '' then begin
            "Applied Amount (BCY)" := "Applied Amount (LCY)";
            "Statement Amount (BCY)" := "Statement Amount (LCY)";
        end else begin
            "Applied Amount (BCY)" :=
              CurrExchRate.ExchangeAmount("Applied Amount", "Currency Code", GetCurrencyCode, "Transaction Date");
            "Statement Amount (BCY)" :=
              CurrExchRate.ExchangeAmount("Statement Amount", "Currency Code", GetCurrencyCode, "Transaction Date");
        end;

        Difference := "Statement Amount" - "Applied Amount";
        "Difference (LCY)" := "Statement Amount (LCY)" - "Applied Amount (LCY)";
        "Difference (BCY)" := "Statement Amount (BCY)" - "Applied Amount (BCY)";
    end;

    local procedure UpdateDocumentType()
    begin
        // NAVCZ
        case true of
            ("Account Type" = "Account Type"::Customer) and ("Statement Amount" > 0) or
          ("Account Type" = "Account Type"::Vendor) and ("Statement Amount" < 0):
    ...
    begin
        ...
        TempGenJnlLine.Init;
        TempGenJnlLine.CopyFromBankAccReconLine(Rec);
        TempGenJnlLine.Prepayment := true;
        TempGenJnlLine."Prepayment Type" := TempGenJnlLine."Prepayment Type"::Advance;
        TempGenJnlLine.Amount := -TempGenJnlLine.Amount;
        TempGenJnlLine."Amount (LCY)" := -TempGenJnlLine."Amount (LCY)";

        CODEUNIT.Run(CODEUNIT::"Gen. Jnl.-Link Letters", TempGenJnlLine);
        "Advance Letter Link Code" := TempGenJnlLine."Advance Letter Link Code";

        if not AreLettersLinked then begin
    ...
    begin
        ...
        PrepmtLinksMgt.LinkGenJnlLine(TempGenJnlLine);
        "Advance Letter Link Code" := TempGenJnlLine."Advance Letter Link Code";
        ...
        case "Account Type" of
            "Account Type"::Customer:
                begin
                    SalesAdvanceLetterLine.SetCurrentKey("Link Code");
                    SalesAdvanceLetterLine.SetRange("Link Code", "Advance Letter Link Code");
                    exit(not SalesAdvanceLetterLine.IsEmpty);
                end;
            "Account Type"::Vendor:
                begin
                    PurchAdvanceLetterLine.SetCurrentKey("Link Code");
                    PurchAdvanceLetterLine.SetRange("Link Code", "Advance Letter Link Code");
                    exit(not PurchAdvanceLetterLine.IsEmpty);
                end;
        end;
    end;

    local procedure CreateAppliedPaymentEntryForLetter()
    var
        AppliedPaymentEntry: Record "Applied Payment Entry";
    ...
    begin
        // NAVCZ
        AppliedPaymentEntry.FilterAppliedPmtEntry(Rec);
        AppliedPaymentEntry.SetRange("Applies-to Entry No.", 0);
        ...
        if not AppliedPaymentEntry.FindFirst then begin
            ...
            AppliedPaymentEntry."Match Confidence" := AppliedPaymentEntry."Match Confidence"::Manual;
            AppliedPaymentEntry.Quality := TempBankPmtApplRule.GetTextMapperScore;
            AppliedPaymentEntry.Description := AdvanceTxt;
            AppliedPaymentEntry.Validate("Applied Amount", "Applied Amount");
            AppliedPaymentEntry.Insert(true);
        end else begin
            AppliedPaymentEntry.Validate("Applied Amount", "Applied Amount");
            AppliedPaymentEntry.Modify(true);
        end;
    end;
    ...
    begin
        // NAVCZ
        if BankPmtApplRuleCode = '' then begin
            BankAcc.Get("Bank Account No.");
            BankPmtApplRuleCode := BankAcc."Bank Pmt. Appl. Rule Code";
        end;

        exit(BankPmtApplRuleCode);
    end;
    ...
    begin
        // NAVCZ
        if TextToAccMappingCode = '' then begin
            BankAcc.Get("Bank Account No.");
            TextToAccMappingCode := BankAcc."Text-to-Account Mapping Code";
        end;
    ...
    begin
        ...
        case "Account Type" of
            "Account Type"::Customer:
                exit(GetAppliedAmtFromSalesAdvLetters(InLCY));
            "Account Type"::Vendor:
                exit(GetAppliedAmtFromPurchAdvLetters(InLCY));
        end;
    end;

    local procedure GetAppliedAmtFromSalesAdvLetters(InLCY: Boolean): Decimal
    var
    ...
    begin
        ...
        if SalesAdvanceLetterLine.FindSet then
            repeat
                ...
                if (SalesAdvanceLetterHeader."Currency Factor" = 0) or not InLCY then
                    TotalAppliedAmount += -SalesAdvanceLetterLine."Amount Linked To Journal Line"
                else
                    TotalAppliedAmount +=
    ...
    local procedure GetAppliedAmtFromPurchAdvLetters(InLCY: Boolean): Decimal
    var
    ...
    begin
        ...
        if PurchAdvanceLetterLine.FindSet then
            repeat
                ...
                if (PurchAdvanceLetterHeader."Currency Factor" = 0) or not InLCY then
                    TotalAppliedAmount += -PurchAdvanceLetterLine."Amount Linked To Journal Line"
                else
                    TotalAppliedAmount +=
