page 17382 "Budgeted Position Card"
{
    Caption = 'Budgeted Position Card';
    PageType = Card;
    ...
    actions
    {
        area(navigation)
        {
            group(Position)
            {
                ...
                action("Where Used")
                {
                    ...
                    RunPageLink = "Budgeted Position No." = FIELD("No.");
                    RunPageView = SORTING("No.");
                    ToolTip = 'View budgets where the position is used.';
                }
            }
        }
        area(processing)
        {
            group("F&unctions")
            {
                ...
                action("Create from Budget")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Create from Budget';
                    Ellipsis = true;
                    Image = Splitlines;
                    Promoted = true;
                    PromotedCategory = Process;
                    ...
                    trigger OnAction()
                    var
                        CreatefromBudget: Report "Create Positions from Budget";
                    begin
                        CreatefromBudget.Set(Rec);
