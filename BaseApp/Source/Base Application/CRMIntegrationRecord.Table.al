table 5331 "CRM Integration Record"
{
    ...
    begin
        ...
        SendTraceTag('0000DQ9', CrmTelemetryCategoryTok, Verbosity::Warning, StrSubstNo(ZeroTableIdTxt, "Integration ID", "CRM ID"), DataClassification::SystemMetadata);
        exit(0);
    end;
    ...
    begin
        ...
        if FindMappingByLocalRecordId(IntegrationTableMapping) then begin
            ...
            Modify();
            SendTraceTag('0000DQ7', CrmTelemetryCategoryTok, Verbosity::Warning, StrSubstNo(FixedTableIdTxt, "Table ID", "Integration ID", "CRM ID"), DataClassification::SystemMetadata);
            exit(true);
        end;
    ...
    begin
        ...
        if FindMappingByCRMRecordId(IntegrationTableMapping) then
            if IntegrationTableMapping."Table ID" <> 0 then begin
                "Table ID" := IntegrationTableMapping."Table ID";
                Modify();
                SendTraceTag('0000DQ8', CrmTelemetryCategoryTok, Verbosity::Warning, StrSubstNo(FixedTableIdTxt, "Table ID", "Integration ID", "CRM ID"), DataClassification::SystemMetadata);
                exit(true);
            end;
