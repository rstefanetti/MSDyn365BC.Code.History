report 20 "Calc. and Post VAT Settlement"
{
    ...
    dataset
    {
        dataitem("VAT Posting Setup"; "VAT Posting Setup")
        {
            ...
            column(VATAmount; VATAmount - VATAmountOfPropDedSettled)
            {
                                ...
                                case VATType of
                                    VATEntry.Type::Purchase:
                                        begin
                                            ...
                                            if PostSettlement then
                                                PostGenJnlLine(GenJnlLine);
                                            if "VAT Posting Setup"."Calc. Prop. Deduction VAT" and
                                               (VATAmountOfPropDed <> VATEntry.Amount) and (VATAmountOfPropDed <> 0)
                                            then begin
            ...
            var
                VATPeriod: Record "Settled VAT Period";
                PeriodStart: Date;
                CurrentPeriod: Integer;
            begin
                // Post to settlement account
                if VATAmount + VATAmountOfPropDedSettled <> 0 then begin
                    GenJnlLine.Init();
                    GenJnlLine."System-Created Entry" := true;
                    GenJnlLine."Account Type" := GenJnlLine."Account Type"::"G/L Account";
                ...
                if PostSettlement then begin
                    PeriodStart := VATTools.FirstDateInVATPeriod(EntrdStartDate);
                    while PeriodStart <= EndDateReq do begin
                        CurrentPeriod := VATTools.VATPeriodNo(PeriodStart);
                        VATPeriod."Period No." := CurrentPeriod;
                        VATPeriod.Year := Date2DMY(PeriodStart, 3);
                        if not VATPeriod.Find then begin
                            VATPeriod."Settlement Date" := PostingDate;
                            VATPeriod.Insert
                        end else begin
                            VATPeriod."Settlement Date" := PostingDate;
                            VATPeriod.Modify();
                        end;
                        PeriodStart := CalcDate('<+2M>', PeriodStart);
                    end;
                end;
            end;
    ...
    var
        ...
        VATAmountOfPropDed: Decimal;
        VATAmountOfPropDedSettled: Decimal;
    ...
    begin
        ...
        if VATPostingSetup."Proportional Deduction VAT %" = 100 then
            exit(VATEntry.Amount);
