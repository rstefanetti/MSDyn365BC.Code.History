codeunit 841 "Cash Flow Management"
{
    ...
    begin
        case SourceType of
          ...
          CFWorksheetLine."Source Type"::"Fixed Assets Disposal":
            ...
            // NAVCZ
            CFWorksheetLine."Source Type"::"Sales Advance Letters":
                ShowSAdvLetters(SourceNo);
            CFWorksheetLine."Source Type"::"Purchase Advance Letters":
                ShowPAdvLetters(SourceNo);
            // NAVCZ
    ...
    local procedure ShowSAdvLetters(SourceNo: Code[20])
    var
        SalesAdvanceLetterHeader: Record "Sales Advance Letter Header";
        SalesAdvanceLetter: Page "Sales Advance Letter";
    begin
        // NAVCZ
        SalesAdvanceLetterHeader.SetRange("No.", SourceNo);
        if not SalesAdvanceLetterHeader.FindFirst then
            Error(SourceDataDoesNotExistErr, SalesAdvanceLetter.Caption, SourceNo);
        SalesAdvanceLetter.SetTableView(SalesAdvanceLetterHeader);
        SalesAdvanceLetter.Run;
    end;

    local procedure ShowPAdvLetters(SourceNo: Code[20])
    var
        PurchAdvanceLetterHeader: Record "Purch. Advance Letter Header";
        PurchaseAdvanceLetter: Page "Purchase Advance Letter";
    begin
        // NAVCZ
        PurchAdvanceLetterHeader.SetRange("No.", SourceNo);
        if not PurchAdvanceLetterHeader.FindFirst then
            Error(SourceDataDoesNotExistErr, PurchaseAdvanceLetter.Caption, SourceNo);
        PurchaseAdvanceLetter.SetTableView(PurchAdvanceLetterHeader);
        PurchaseAdvanceLetter.Run;
    end;
    ...
    begin
        ...
        CreateCashFlowAccount(CashFlowAccount."Source Type"::"Sales Advance Letters", '');
        CreateCashFlowAccount(CashFlowAccount."Source Type"::"Purchase Advance Letters", '');
    ...
    begin
        ...
        CashFlowSetup.Validate(
            "S. Adv. Letter CF Account No.", GetNoFromSourceType(CashFlowAccount."Source Type"::"Sales Advance Letters"));
        CashFlowSetup.Validate(
            "P. Adv. Letter CF Account No.", GetNoFromSourceType(CashFlowAccount."Source Type"::"Purchase Advance Letters"));
