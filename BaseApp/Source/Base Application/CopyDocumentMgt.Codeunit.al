codeunit 6620 "Copy Document Mgt."
{
    ...
    begin
        with ToSalesHeader do begin
            ...
            if (not ("Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"])) and
                (FromDocType in [SalesDocType::"Return Order", SalesDocType::"Credit Memo", SalesDocType::"Posted Credit Memo"])
            then begin
                "Bank Account Code" := '';
                "Bank Account No." := '';
                "Specific Symbol" := '';
            ...
            if ("Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"]) and
                (not (FromDocType in [SalesDocType::"Return Order",
                                      SalesDocType::"Credit Memo", SalesDocType::"Posted Credit Memo"]))
            then begin
                "Bank Account Code" := '';
                "Bank Account No." := '';
                "Specific Symbol" := '';
    ...
    begin
        with ToPurchHeader do begin
            ...
            if ("Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"]) and
                (not (FromDocType in [SalesDocType::"Return Order", SalesDocType::"Credit Memo", SalesDocType::"Posted Credit Memo"
                                        ]))
            then begin
                "Bank Account Code" := '';
                "Bank Account No." := '';
    ...
    begin
        ...
        with SalesHeader do begin
            Status := Status::Open;
            if FromDocType = SalesDocType::"Arch. Quote" then
                if OriginalSalesHeader."Posting Date" = 0D then
                    "Posting Date" := WorkDate
                else
                    "Posting Date" := OriginalSalesHeader."Posting Date";
    ...
    begin
        ...
        with PurchaseHeader do begin
            Status := Status::Open;
            if FromDocType = PurchDocType::"Arch. Quote" then
                if OriginalPurchaseHeader."Posting Date" = 0D then
                    "Posting Date" := WorkDate
                else
                    "Posting Date" := OriginalPurchaseHeader."Posting Date";
        ...
        if ValueEntry.FindSet() then begin
            ...
            repeat
                ...
                OnCopyFromPurchLineItemChargeAssignOnAfterValueEntryLoop(
                    FromPurchHeader, ToPurchLine, ValueEntry, TempToItemChargeAssignmentPurch, ToItemChargeAssignmentPurch, ItemChargeAssgntNextLineNo);
        ...
        if ValueEntry.FindSet then begin
            ...
            repeat
                ...
                OnCopyFromSalesLineItemChargeAssignOnAfterValueEntryLoop(
                    FromSalesHeader, ToSalesLine, ValueEntry, TempToItemChargeAssignmentSales, ToItemChargeAssignmentSales, ItemChargeAssgntNextLineNo);
            until ValueEntry.Next = 0;
    ...
    begin
        OnBeforeCheckItemAvailability(ToSalesHeader, ToSalesLine, HideDialog);
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckItemAvailability(var ToSalesHeader: Record "Sales Header"; var ToSalesLine: Record "Sales Line"; var HideDialog: Boolean);
    begin
    end;
