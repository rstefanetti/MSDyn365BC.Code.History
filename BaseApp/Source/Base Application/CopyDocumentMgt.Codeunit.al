codeunit 6620 "Copy Document Mgt."
{
    ...
    begin
        with ToSalesHeader do begin
            ...
            case FromDocType of
                ...
                "Sales Document Type From"::"Arch. Blanket Order",
                "Sales Document Type From"::"Arch. Return Order":
                    CopySalesHeaderFromSalesHeaderArchive(FromSalesHeaderArchive, ToSalesHeader, OldSalesHeader, FromDocType);
            end;
            ...
            // NAVCZ
            "Tax Corrective Document" := OldSalesHeader."Tax Corrective Document";
            OldVATCountry := "VAT Country/Region Code";
            ...
            if OldVATCountry <> '' then
                Validate("VAT Country/Region Code", OldVATCountry);
            // NAVCZ
            ...
            if ("Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"]) and
                (not (FromDocType in ["Sales Document Type From"::"Return Order",
                                      "Sales Document Type From"::"Credit Memo",
                                      "Sales Document Type From"::"Posted Credit Memo"]))
            then begin
                "Bank Account Code" := '';
    ...
    begin
        ...
        UpdateSalesHeaderWhenCopyFromSalesHeaderArchive(ToSalesHeader, OriginalSalesHeader, FromDocType);
        CopyFromArchSalesDocDimToHdr(ToSalesHeader, FromSalesHeaderArchive);
        OnAfterCopySalesHeaderArchive(ToSalesHeader, OriginalSalesHeader, FromSalesHeaderArchive)
    end;
    ...
    begin
        with ToPurchHeader do begin
            ...
            case FromDocType of
                ...
                "Purchase Document Type From"::"Arch. Blanket Order",
                "Purchase Document Type From"::"Arch. Return Order":
                    CopyPurchHeaderFromPurchHeaderArchive(FromPurchHeaderArchive, ToPurchHeader, OldPurchHeader, FromDocType);
            end;
            ...
            // NAVCZ
            OldVATCountryCode := "VAT Country/Region Code";
            OldVATBusPostingGroup := "VAT Bus. Posting Group";
            // NAVCZ
            ...
            // NAVCZ
            if OldVATCountryCode <> '' then
                Validate("VAT Country/Region Code", OldVATCountryCode);
            // NAVCZ
    ...
    local procedure CopyPurchHeaderFromPurchHeaderArchive(FromPurchHeaderArchive: Record "Purchase Header Archive"; var ToPurchHeader: Record "Purchase Header"; OriginalPurchaseHeader: Record "Purchase Header"; FromDocType: Option)
    begin
        ...
        UpdatePurchHeaderWhenCopyFromPurchHeaderArchive(ToPurchHeader, OriginalPurchaseHeader, FromDocType);
        CopyFromArchPurchDocDimToHdr(ToPurchHeader, FromPurchHeaderArchive);
        OnAfterCopyPurchHeaderArchive(ToPurchHeader, OriginalPurchaseHeader, FromPurchHeaderArchive)
    end;
    ...
    local procedure UpdateSalesHeaderWhenCopyFromSalesHeaderArchive(var SalesHeader: Record "Sales Header"; OriginalSalesHeader: Record "Sales Header"; FromDocType: Option)
    begin
        ClearSalesLastNoSFields(SalesHeader);
        with SalesHeader do begin
            Status := Status::Open;
            if FromDocType = "Sales Document Type From"::"Arch. Quote" then
                if OriginalSalesHeader."Posting Date" = 0D then
                    "Posting Date" := WorkDate
                else
                    "Posting Date" := OriginalSalesHeader."Posting Date";
    ...
    begin
        ...
        OnAfterUpdateSalesLine(
          ToSalesHeader, ToSalesLine, FromSalesHeader, FromSalesLine,
          CopyThisLine, RecalculateAmount, FromSalesDocType.AsInteger(), CopyPostedDeferral,
          ExactCostRevMandatory, MoveNegLines);
    end;
    ...
    begin
        ...
        OnCopyPurchDocLineOnBeforeCopyThisLine(ToPurchLine, FromPurchLine, MoveNegLines);
        if CopyThisLine then begin
    ...
    local procedure UpdatePurchHeaderWhenCopyFromPurchHeaderArchive(var PurchaseHeader: Record "Purchase Header"; OriginalPurchaseHeader: Record "Purchase Header"; FromDocType: Option)
    begin
        ClearPurchLastNoSFields(PurchaseHeader);
        with PurchaseHeader do begin
            Status := Status::Open;
            if FromDocType = "Purchase Document Type From"::"Arch. Quote" then
                if OriginalPurchaseHeader."Posting Date" = 0D then
                    "Posting Date" := WorkDate
                else
                    "Posting Date" := OriginalPurchaseHeader."Posting Date";
    ...
    local procedure WarnSalesInvoicePmtDisc(var ToSalesHeader: Record "Sales Header"; var FromSalesHeader: Record "Sales Header"; FromDocType: Enum "Sales Document Type From"; FromDocNo: Code[20])
    var
    ...
    local procedure WarnPurchInvoicePmtDisc(var ToPurchHeader: Record "Purchase Header"; var FromPurchHeader: Record "Purchase Header"; FromDocType: Enum "Purchase Document Type From"; FromDocNo: Code[20])
    var
            ...
            if FromSalesInvLine.FindSet then
                repeat
                    if not IsItemBlocked(FromSalesInvLine."No.") then begin
                        ...
                        CheckCopyFromSalesInvoiceAvailOnBeforeCheckItemAvailability(ToSalesLine, FromSalesInvLine);
                        CheckItemAvailability(ToSalesHeader, ToSalesLine);
        ...
        if FromSalesLineArchive.FindSet() then
            repeat
                if FromSalesLineArchive.Quantity > 0 then begin
                    ...
                    ToSalesLine."Drop Shipment" := FromSalesLineArchive."Drop Shipment";
                    CheckItemAvailability(ToSalesHeader, ToSalesLine);
            ...
            if FindSet then begin
                ...
                repeat
                    ...
                    if "Shipment No." <> OldInvDocNo then begin
                        ...
                        FirstLineShipped := true;
                        // NAVCZ
                        if ToSalesHeader."Tax Corrective Document" then begin
                            FromSalesInvLine.CalcSums(Amount, "Amount Including VAT");
                            TaxCorrDocumentMgt.AddTaxCorrDocDescToSalesLine(
                              ToSalesHeader,
                              NextLineNo,
                              "Shipment No.",
                              FromSalesInvLine.Amount,
                              FromSalesInvLine."Amount Including VAT");
                        end else begin
                            // NAVCZ
                    ...
                    then begin
                        ...
                        OnCopySalesInvLinesToDocOnBeforeCopyItemTracking(TempSalesLineBuf, ToSalesHeader, FromSalesInvLine, ItemLedgEntryBuf, TempItemTrkgEntry);
                        // copy item tracking
            ...
            if FindSet then begin
                ...
                repeat
                    ...
                    if "Shipment No." <> OldCrMemoDocNo then begin
                        ...
                        // NAVCZ
                        if ToSalesHeader."Tax Corrective Document" then begin
                            FromSalesCrMemoLine.CalcSums(Amount, "Amount Including VAT");
                            TaxCorrDocumentMgt.AddTaxCorrDocDescToSalesLine(
                              ToSalesHeader,
                              NextLineNo,
                              FromSalesCrMemoLine."Document No.",
                              FromSalesCrMemoLine.Amount,
                              FromSalesCrMemoLine."Amount Including VAT");
                        end else
                            // NAVCZ
        ...
        with FromPurchInvLine do
            if FindSet then
                repeat
                    ...
                    OnCopyPurchInvLinesToDocOnAfterTransferFields(FromPurchLine, FromPurchHeader, ToPurchHeader, FromPurchInvHeader);
    ...
    var
        ToCashDocLine: Record "Cash Document Line";
    begin
        // NAVCZ
        ToCashDocLine.LockTable();
        ToCashDocLine.SetRange("Cash Desk No.", ToCashDocHeader."Cash Desk No.");
        ToCashDocLine.SetRange("Cash Document No.", ToCashDocHeader."No.");
        if ToCashDocLine.FindLast then
            exit(ToCashDocLine."Line No.");
        exit(0);
    end;
    ...
    var
        ToCashDocLine2: Record "Cash Document Line";
    begin
        // NAVCZ
        if SkipCopyFromDescription then
            exit;
        ...
        NextLineNo := NextLineNo + 10000;
        ToCashDocLine2.Init();
        ToCashDocLine2."Line No." := NextLineNo;
        ToCashDocLine2."Cash Desk No." := ToCashDocHeader."Cash Desk No.";
        ToCashDocLine2."Cash Document No." := ToCashDocHeader."No.";
        ToCashDocLine2.Description := StrSubstNo(Text015, OldCashDeskNo, OldDocNo);
    ...
    begin
        ...
        if CopyThisLine then begin
            OnCopyArchSalesLineOnBeforeToSalesLineInsert(ToSalesLine, FromSalesLineArchive, RecalculateLines, NextLineNo, TransferOldExtLines);
            ToSalesLine.Insert();
    ...
    begin
        with ToSalesHeader do begin
            ...
            "Prepmt. Posting Description" := OldSalesHeader."Prepmt. Posting Description";
            "Postponed VAT" := OldSalesHeader."Postponed VAT"; // NAVCZ
    ...
    begin
        // NAVCZ
        with ToCashDocHeader do begin
            "No. Series" := OldCashDocHeader."No. Series";
            "Created ID" := OldCashDocHeader."Created ID";
            "Released ID" := OldCashDocHeader."Released ID";
            "Created Date" := OldCashDocHeader."Created Date";
        end;
    end;
    ...
    begin
        // NAVCZ
        with ToCashDocHeader do begin
            FromCashDocHeader.TestField("Currency Code", "Currency Code");
            FromCashDocHeader.TestField("Amounts Including VAT", "Amounts Including VAT");
        end;
    end;

    [Obsolete('Moved to Cash Desk Localization for Czech.', '17.0')]
    local procedure CheckFromPostedCashDocumentHeader(FromPostedCashDocHeader: Record "Posted Cash Document Header"; ToCashDocHeader: Record "Cash Document Header")
    begin
        // NAVCZ
        with ToCashDocHeader do begin
            FromPostedCashDocHeader.TestField("Currency Code", "Currency Code");
            FromPostedCashDocHeader.TestField("Amounts Including VAT", "Amounts Including VAT");
        end;
    end;
    ...
    local procedure CopyPostedDeferrals(DeferralDocType: Enum "Deferral Document Type"; FromDocType: Integer; FromDocNo: Code[20]; FromLineNo: Integer; ToDocType: Integer; ToDocNo: Code[20]; ToLineNo: Integer) StartDate: Date
    var
    ...
    local procedure IsDeferralToBeCopied(DeferralDocType: Enum "Deferral Document Type"; ToDocType: Option; FromDocType: Option): Boolean
    var
        SalesLine: Record "Sales Line";
        SalesCommentLine: Record "Sales Comment Line";
    ...
    local procedure IsDeferralToBeDefaulted(DeferralDocType: Enum "Deferral Document Type"; ToDocType: Option; FromDocType: Option): Boolean
    var
        SalesLine: Record "Sales Line";
        SalesCommentLine: Record "Sales Comment Line";
        PurchLine: Record "Purchase Line";
    ...
    local procedure RemoveDefaultDeferralCode(DeferralDocType: Enum "Deferral Document Type"; DocType: Integer; DocNo: Code[20]; LineNo: Integer)
    var
        DeferralHeader: Record "Deferral Header";
        DeferralLine: Record "Deferral Line";
    begin
    ...
    local procedure CopySalesPostedDeferrals(ToSalesLine: Record "Sales Line"; DeferralDocType: Enum "Deferral Document Type"; FromDocType: Integer; FromDocNo: Code[20]; FromLineNo: Integer; ToDocType: Integer; ToDocNo: Code[20]; ToLineNo: Integer)
    begin
        ToSalesLine."Returns Deferral Start Date" :=
            CopyPostedDeferrals(
    ...
    local procedure CopyPurchPostedDeferrals(ToPurchaseLine: Record "Purchase Line"; DeferralDocType: Enum "Deferral Document Type"; FromDocType: Integer; FromDocNo: Code[20]; FromLineNo: Integer; ToDocType: Integer; ToDocNo: Code[20]; ToLineNo: Integer)
    begin
    ...
    begin
        // NAVCZ
        if (FromCashDocHeader."Cash Desk No." = ToCashDocHeader."Cash Desk No.") and
           (FromCashDocHeader."No." = ToCashDocHeader."No.")
        then
    ...
    begin
        IsHandled := false;
        OnBeforeUpdateCustLedgEntry(ToSalesHeader, CustLedgEntry, IsHandled);
        if IsHandled then
            exit;
    ...
    var
        ToCashDocLine: Record "Cash Document Line";
        OldCashDocHeader: Record "Cash Document Header";
        FromCashDocHeader: Record "Cash Document Header";
        FromCashDocLine: Record "Cash Document Line";
        FromPostedCashDocHeader: Record "Posted Cash Document Header";
        FromPostedCashDocLine: Record "Posted Cash Document Line";
        ...
        with ToCashDocHeader do begin
            ...
            case FromDocType of
                CashDocType::"Cash Document":
                    begin
                        FromCashDocHeader.Get(FromCashDeskNo, FromDocNo);
                        CheckCashdDocItselfCopy(ToCashDocHeader, FromCashDocHeader);

                        if not IncludeHeader and not RecalculateLines then
                            CheckFromCashDocumentHeader(FromCashDocHeader, ToCashDocHeader);
                    end;
                CashDocType::"Posted Cash Document":
                    begin
                        FromPostedCashDocHeader.Get(FromCashDeskNo, FromDocNo);

                        if not IncludeHeader and not RecalculateLines then
                            CheckFromPostedCashDocumentHeader(FromPostedCashDocHeader, ToCashDocHeader);
                    end;
            end;

            ToCashDocLine.LockTable();
            ToCashDocLine.SetRange("Cash Desk No.", "Cash Desk No.");
            ToCashDocLine.SetRange("Cash Document No.", "No.");
            if IncludeHeader then
                if not ToCashDocLine.IsEmpty then begin
                    Commit();
                    ConfirmText := DeleteLinesQst;
                    if not
                       Confirm(
                         ConfirmText, true,
                         "Cash Desk No.", "No.")
                    then
                        exit;
                    ToCashDocLine.DeleteAll(true);
                end;

            if ToCashDocLine.FindLast then
                NextLineNo := ToCashDocLine."Line No."
            else
                NextLineNo := 0;

            if IncludeHeader then begin
                OldCashDocHeader := ToCashDocHeader;
                case FromDocType of
                    CashDocType::"Cash Document":
                        TransferFields(FromCashDocHeader, false);
                    CashDocType::"Posted Cash Document":
                        TransferFields(FromPostedCashDocHeader, false);
                end;
                ...
                CopyFieldsFromOldCashDocHeader(ToCashDocHeader, OldCashDocHeader);
                if RecalculateLines then
                    CreateDim(
                      DATABASE::"Responsibility Center", "Responsibility Center",
            ...
            LinesNotCopied := 0;
            ErrorMessageMgt.Activate(ErrorMessageHandler);
            ErrorMessageMgt.PushContext(ErrorContextElement, RecordId, 0, StrSubstNo(CashErrorContextMsg, FromDocNo));
            case FromDocType of
                CashDocType::"Cash Document":
                    begin
                        FromCashDocLine.Reset();
                        FromCashDocLine.SetRange("Cash Desk No.", FromCashDocHeader."Cash Desk No.");
                        FromCashDocLine.SetRange("Cash Document No.", FromCashDocHeader."No.");
                        if FromCashDocLine.FindSet then
                            repeat
                                CopyCashDocLine(
                                  ToCashDocHeader, ToCashDocLine, FromCashDocHeader, FromCashDocLine, NextLineNo, LinesNotCopied);
                            until FromCashDocLine.Next = 0;
                    end;
                CashDocType::"Posted Cash Document":
                    begin
                        FromPostedCashDocLine.Reset();
                        FromPostedCashDocLine.SetRange("Cash Desk No.", FromPostedCashDocHeader."Cash Desk No.");
                        FromPostedCashDocLine.SetRange("Cash Document No.", FromPostedCashDocHeader."No.");
                        CopyPostedCashDocLinesToDoc(ToCashDocHeader, FromPostedCashDocLine, LinesNotCopied);
                    end;
            end;
        end;

        if ReleaseDocument then begin
            ToCashDocHeader.Status := ToCashDocHeader.Status::Released;
            CashDocumentRelease.Reopen(ToCashDocHeader);
        end else
            if (FromDocType = CashDocType::"Cash Document") and
               not IncludeHeader and not RecalculateLines
            then
                if FromCashDocHeader.Status = FromCashDocHeader.Status::Released then begin
                    CashDocumentRelease.Run(ToCashDocHeader);
                    CashDocumentRelease.Reopen(ToCashDocHeader);
                end;
    ...
    [Obsolete('Moved to Cash Desk Localization for Czech.', '17.0')]
    local procedure CopyCashDocLine(var ToCashDocHeader: Record "Cash Document Header"; var ToCashDocLine: Record "Cash Document Line"; var FromCashDocHeader: Record "Cash Document Header"; var FromCashDocLine: Record "Cash Document Line"; var NextLineNo: Integer; var LinesNotCopied: Integer): Boolean
    var
        CopyThisLine: Boolean;
    begin
        // NAVCZ
        CopyThisLine := true;
        if IsEntityBlocked(0, false, FromCashDocLine."Account Type", FromCashDocLine."Account No.") then begin
            LinesNotCopied := LinesNotCopied + 1;
            exit(false);
        end;

        if RecalculateLines and not FromCashDocLine."System-Created Entry" then
            ToCashDocLine.Init
        else
            ToCashDocLine := FromCashDocLine;

        NextLineNo := NextLineNo + 10000;
        ToCashDocLine."Cash Desk No." := ToCashDocHeader."Cash Desk No.";
        ToCashDocLine."Cash Document No." := ToCashDocHeader."No.";
        ToCashDocLine."Line No." := NextLineNo;
        ToCashDocLine.Validate("Currency Code", FromCashDocHeader."Currency Code");

        UpdateCashDocLine(ToCashDocHeader, ToCashDocLine, FromCashDocHeader, FromCashDocLine, CopyThisLine);

        if not RecalculateLines then begin
            ToCashDocLine."Dimension Set ID" := FromCashDocLine."Dimension Set ID";
            ToCashDocLine."Shortcut Dimension 1 Code" := FromCashDocLine."Shortcut Dimension 1 Code";
            ToCashDocLine."Shortcut Dimension 2 Code" := FromCashDocLine."Shortcut Dimension 2 Code";
        end;

        if CopyThisLine then
            ToCashDocLine.Insert
        else
            LinesNotCopied := LinesNotCopied + 1;
        exit(true);
    end;
    ...
    [Obsolete('Moved to Cash Desk Localization for Czech.', '17.0')]
    local procedure UpdateCashDocLine(var ToCashDocHeader: Record "Cash Document Header"; var ToCashDocLine: Record "Cash Document Line"; var FromCashDocHeader: Record "Cash Document Header"; var FromCashDocLine: Record "Cash Document Line"; var CopyThisLine: Boolean)
    var
        GLAcc: Record "G/L Account";
        VATPostingSetup: Record "VAT Posting Setup";
    begin
        // NAVCZ
        if RecalculateLines and not FromCashDocLine."System-Created Entry" then begin
            if FromCashDocLine."Cash Desk Event" <> '' then
                ToCashDocLine.Validate("Cash Desk Event", FromCashDocLine."Cash Desk Event")
            else begin
                ToCashDocLine.Validate("Account Type", FromCashDocLine."Account Type");
                ToCashDocLine.Description := FromCashDocLine.Description;
                ToCashDocLine.Validate("Description 2", FromCashDocLine."Description 2");
                if (FromCashDocLine."Account Type" <> 0) and (FromCashDocLine."Account No." <> '') then
                    if ToCashDocLine."Account Type" = ToCashDocLine."Account Type"::"G/L Account" then begin
                        ToCashDocLine."Account No." := FromCashDocLine."Account No.";
                        if GLAcc."No." <> FromCashDocLine."Account No." then
                            GLAcc.Get(FromCashDocLine."Account No.");
                        CopyThisLine := GLAcc."Direct Posting";
                        if CopyThisLine then
                            ToCashDocLine.Validate("Account No.", FromCashDocLine."Account No.");
                    end else
                        ToCashDocLine.Validate("Account No.", FromCashDocLine."Account No.");
            end;

            if (FromCashDocHeader."Currency Code" <> ToCashDocHeader."Currency Code") or
               (FromCashDocHeader."Amounts Including VAT" <> ToCashDocHeader."Amounts Including VAT")
            then
                ToCashDocLine.Amount := 0
            else
                ToCashDocLine.Validate(Amount, FromCashDocLine.Amount);
            if (FromCashDocLine."Account Type" = FromCashDocLine."Account Type"::" ") and (FromCashDocLine."Account No." <> '') then
                ToCashDocLine.Validate("Account No.", FromCashDocLine."Account No.");
        end else
            if VATPostingSetup.Get(ToCashDocLine."VAT Bus. Posting Group", ToCashDocLine."VAT Prod. Posting Group") then
                ToCashDocLine."VAT Identifier" := VATPostingSetup."VAT Identifier";
    end;

    [Obsolete('Moved to Cash Desk Localization for Czech.', '17.0')]
    [Scope('OnPrem')]
    procedure CopyPostedCashDocLinesToDoc(ToCashDocHeader: Record "Cash Document Header"; var FromPostedCashDocLine: Record "Posted Cash Document Line"; var LinesNotCopied: Integer)
    var
        FromPostedCashDocHeader: Record "Posted Cash Document Header";
        FromCashDocHeader: Record "Cash Document Header";
        FromCashDocLine: Record "Cash Document Line";
    ...
    begin
        // NAVCZ
        InitCurrency(ToCashDocHeader."Currency Code");
        OpenWindow;

        with FromPostedCashDocLine do
            if FindSet then
                repeat
                    FromLineCounter := FromLineCounter + 1;
                    if IsTimeForUpdate then
                        Window.Update(1, FromLineCounter);
                    if (FromPostedCashDocHeader."Cash Desk No." <> "Cash Desk No.") or
                       (FromPostedCashDocHeader."No." <> "Cash Document No.")
                    then
                        FromPostedCashDocHeader.Get("Cash Desk No.", "Cash Document No.");

                    FromCashDocHeader.TransferFields(FromPostedCashDocHeader);
                    FromCashDocLine.TransferFields(FromPostedCashDocLine);

                    if ("Cash Document No." <> OldDocNo) or ("Cash Desk No." <> OldCashDeskNo) then begin
                        OldDocNo := "Cash Document No.";
                        OldCashDeskNo := "Cash Desk No.";
                        InsertDocNoLine := true;
                    end;

                    Window.Update(1, FromLineCounter);
                    NextLineNo := GetLastToCashDocLineNo(ToCashDocHeader);
                    if InsertDocNoLine then begin
                        InsertOldCashDocNoLine(ToCashDocHeader, "Cash Document No.", "Cash Desk No.", NextLineNo);
                        InsertDocNoLine := false;
                    end;
                    ToLineCounter := ToLineCounter + 1;
                    if IsTimeForUpdate then
                        Window.Update(2, ToLineCounter);
                    CopyCashDocLine(ToCashDocHeader, ToCashDocLine, FromCashDocHeader, FromCashDocLine, NextLineNo, LinesNotCopied);
                until Next = 0;
            ...
            case FromDocType2 of
                ...
                "Sales Document Type From"::"Posted Invoice":
                    begin
                        ...
                        WarnSalesInvoicePmtDisc(ToSalesHeader, FromSalesHeader, FromDocType2, FromDocNo);
                        if not CheckDateOrder(
                             "Posting No.", "Posting No. Series",
            ...
            case FromDocType2 of
                ...
                "Purchase Document Type From"::"Posted Invoice":
                    begin
                        ...
                        WarnPurchInvoicePmtDisc(ToPurchaseHeader, FromPurchaseHeader, FromDocType2, FromDocNo);
                        if not IncludeHeader and not RecalculateLines then
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateCustLedgEntry(var ToSalesHeader: Record "Sales Header"; var CustLedgerEntry: Record "Cust. Ledger Entry"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInitAndCheckSalesDocuments(FromDocType: enum "Sales Document Type From"; FromDocNo: Code[20]; FromDocOccurrenceNo: Integer; FromDocVersionNo: Integer; var FromSalesHeader: Record "Sales Header"; var ToSalesHeader: Record "Sales Header"; var ToSalesLine: Record "Sales Line"; MoveNegLines: boolean; IncludeHeader: Boolean; RecalculateLines: Boolean; var Result: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCopyArchSalesLineOnBeforeToSalesLineInsert(var ToSalesLine: Record "Sales Line"; FromSalesLineArchive: Record "Sales Line Archive"; RecalculateLines: Boolean; var NextLineNo: Integer; var TransferOldExtLines: Codeunit "Transfer Old Ext. Text Lines")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCopyPurchInvLinesToDocOnAfterTransferFields(var FromPurchaseLine: Record "Purchase Line"; var FromPurchaseHeader: Record "Purchase Header"; var ToPurchaseHeader: Record "Purchase Header"; FromPurchInvHeader: Record "Purch. Inv. Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCopySalesReturnRcptLinesToDocOnBeforeCopySalesDocLine(ToSalesHeader: Record "Sales Header"; var FromSalesLineBuf: Record "Sales Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCopySalesShptLinesToDocOnBeforeCopySalesLine(ToSalesHeader: Record "Sales Header"; var FromSalesLine: Record "Sales Line"; FromSalesShptLine: record "Sales Shipment Line"; CopyItemTrkg: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCopySalesInvLinesToDocOnBeforeCopyItemTracking(var TempSalesLineBuf: Record "Sales Line" temporary; ToSalesHeader: Record "Sales Header"; var FromSalesInvLine: Record "Sales Invoice Line"; var ItemLedgEntryBuf: Record "Item Ledger Entry" temporary; var TempItemTrkgEntry: Record "Reservation Entry" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCopySalesDocLineOnBeforeCheckLocationOnWMS(var ToSalesHeader: Record "Sales Header"; var ToSalesLine: Record "Sales Line"; var FromSalesLine: Record "Sales Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterCopySalesDocSalesLine(var ToSalesLine: Record "Sales Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCopySalesDocLineOnBeforeCopyThisLine(ToSalesHeader: Record "Sales Header"; var ToSalesLine: Record "Sales Line"; FromSalesLine: Record "Sales Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure CheckCopyFromSalesInvoiceAvailOnBeforeCheckItemAvailability(var ToSalesLine: Record "Sales Line"; var FromSalesInvLine: Record "Sales Invoice Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCopyPurchDocLineOnBeforeCopyThisLine(var ToPurchLine: Record "Purchase Line"; var FromPurchLine: Record "Purchase Line"; MoveNegLines: Boolean)
    begin
    end;
