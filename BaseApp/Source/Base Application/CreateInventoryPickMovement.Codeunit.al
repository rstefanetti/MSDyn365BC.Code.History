codeunit 7322 "Create Inventory Pick/Movement"
{
    ...
    begin
        ...
        if RemQtyToPickBase > 0 then begin
            ItemTrackingMgt.CheckWhseItemTrkgSetup(NewWhseActivLine."Item No.", NewWhseActivLine."Location Code", SNRequired, LNRequired, CDRequired, false);
            if SNRequired or LNRequired or CDRequired then begin
    ...
    begin
        ...
        with FromBinContent do begin
            ...
            if NewWhseActivLine."CD No." <> '' then
                SetRange("CD No. Filter", NewWhseActivLine."CD No.");
    ...
    local procedure CalcInvtAvailability(WhseActivLine: Record "Warehouse Activity Line"; LotNo: Code[50]; SerialNo: Code[50]; CDNo: Code[20]): Decimal
    var
        Item2: Record Item;
    ...
    begin
        with WhseActivLine do begin
            ...
            if CDNo <> '' then
                Item2.SetRange("CD No. Filter", LotNo);
            Item2.CalcFields(Inventory);
            if not IsBlankInvtMovement then
                Item2.CalcFields("Reserved Qty. on Inventory");

            QtyAssgndtoPick := WhseAvailMgt.CalcQtyAssgndtoPick(Location, "Item No.", "Variant Code", CDNo);
            if "Activity Type" <> "Activity Type"::"Invt. Movement" then // Invt. Movement from Dedicated Bin is allowed
                QtyOnDedicatedBins := WhseAvailMgt.CalcQtyOnDedicatedBins("Location Code", "Item No.", "Variant Code", LotNo, SerialNo, CDNo);
            QtyBlocked :=
              WhseAvailMgt.CalcQtyOnBlockedITOrOnBlockedOutbndBins("Location Code", "Item No.", "Variant Code", LotNo, SerialNo, CDNo, false, false, false);
            LineReservedQty :=
              WhseAvailMgt.CalcLineReservedQtyOnInvt(
                "Source Type", "Source Subtype", "Source No.", "Source Line No.", "Source Subline No.", true, LotNo, SerialNo, '',
                TempWhseActivLine2);
    ...
    local procedure ItemTrackedQuantity(LotNo: Code[50]; SerialNo: Code[50]; CDNo: Code[30]): Decimal
    begin
                    ...
                    if not ((EntrySummary."Serial No." <> '') and (QtyTracked > 0)) then begin
                        if Location."Bin Mandatory" then
                            TotalAvailQtyToPickBase :=
                              CalcQtyAvailToPickOnBins(
                                WhseActivLine, EntrySummary."Lot No.", EntrySummary."Serial No.", EntrySummary."CD No.", RemQtyToPickBase)
                        else
                            TotalAvailQtyToPickBase := CalcInvtAvailability(WhseActivLine, EntrySummary."Lot No.", EntrySummary."Serial No.", EntrySummary."CD No.");
    ...
    begin
        ...
        then begin
            ...
            if NewWhseActivLine.TrackingExists then begin
                ...
                ReservationEntry.SetTrackingFilter(NewWhseActivLine."Serial No.", NewWhseActivLine."Lot No.", NewWhseActivLine."CD No.");
            ...
            QtyToPickBase := QtyToAsmBase -
              WMSMgt.CalcQtyBaseOnATOInvtPick(ATOSalesLine, NewWhseActivLine."Serial No.", NewWhseActivLine."Lot No.", NewWhseActivLine."CD No.");
    ...
    local procedure CalcQtyAvailToPickOnBins(WhseActivLine: Record "Warehouse Activity Line"; LotNo: Code[50]; SerialNo: Code[50]; CDNo: Code[20]; RemQtyToPickBase: Decimal): Decimal
    var
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertPickOrMoveBinWhseActLine(NewWhseActivLine: Record "Warehouse Activity Line"; BinCode: Code[20]; DefaultBin: Boolean; var RemQtyToPickBase: Decimal; var IsHandled: Boolean)
    begin
    end;
