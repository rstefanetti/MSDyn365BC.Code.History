table 287 "Customer Bank Account"
{
    ...
    fields
    {
        ...
        field(13; "Bank Branch No."; Code[5])
        {
    ...
    [Scope('OnPrem')]
    procedure GetPaymentType(var SwissPaymentType: Option " ","1","2.1","2.2","3","4","5","6"; CurrencyCode: Code[10]): Boolean
    var
    ...
    begin
        ...
        if ("Bank Account No." = '') and
           CHMgt.IsDomesticCurrency(CurrencyCode) and
           CHMgt.IsDomesticIBAN(IBAN)
        then begin
            SwissPaymentType := SwissPaymentType::"2.2";
            exit(true);
        end;

        if "Bank Account No." <> '' then begin
            SwissPaymentType := SwissPaymentType::"6";
            exit(true);
        end;
