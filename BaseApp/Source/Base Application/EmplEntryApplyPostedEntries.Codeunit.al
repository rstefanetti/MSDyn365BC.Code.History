codeunit 224 "EmplEntry-Apply Posted Entries"
{
        ...
        if PreviewMode then
            case RunOptionPreviewContext of
                RunOptionPreview::Apply:
                    Apply(Rec, DocumentNoPreviewContext, ApplicationDatePreviewContext, JournalTemplateNameContext, JournalBatchNameContext);
            ...
            Apply(Rec, "Document No.", ApplicationDate, GLSetup."Jnl. Templ. Name for Applying", GLSetup."Jnl. Batch Name for Applying");
        end;
    end;
    ...
    var
        ...
        PreviewMode: Boolean;
        JournalTemplateNameContext: Code[10];
        JournalBatchNameContext: Code[10];
        EnterJnlTemplateNameErr: Label 'Please enter a Journal Template Name.';
        EnterJnlBatchNameErr: Label 'Please enter a Journal Batch Name.';
    ...
    procedure Apply(EmplLedgEntry: Record "Employee Ledger Entry"; DocumentNo: Code[20]; ApplicationDate: Date; JnlTemplateName: Code[10]; JnlBatchName: Code[10])
    begin
        ...
        with EmplLedgEntry do begin
            ...
            if JnlTemplateName = '' then
                Error(EnterJnlTemplateNameErr);
            if JnlBatchName = '' then
                Error(EnterJnlBatchNameErr);

            if DocumentNo = '' then
                DocumentNo := "Document No.";

            EmplPostApplyEmplLedgEntry(EmplLedgEntry, DocumentNo, ApplicationDate, JnlTemplateName, JnlBatchName);
        end;
    end;
    ...
    local procedure EmplPostApplyEmplLedgEntry(EmplLedgEntry: Record "Employee Ledger Entry"; DocumentNo: Code[20]; ApplicationDate: Date; JnlTemplateName: Code[10]; JnlBatchName: Code[10])
    var
    ...
    procedure PreviewApply(EmployeeLedgerEntry: Record "Employee Ledger Entry"; DocumentNo: Code[20]; ApplicationDate: Date; JournalTemplateName: Code[10]; JournalBatchName: Code[10])
    var
        GenJnlPostPreview: Codeunit "Gen. Jnl.-Post Preview";
        EmplEntryApplyPostedEntries: Codeunit "EmplEntry-Apply Posted Entries";
    begin
        BindSubscription(EmplEntryApplyPostedEntries);
        EmplEntryApplyPostedEntries.SetApplyContext(ApplicationDate, DocumentNo, JournalTemplateName, JournalBatchName);
    ...
    procedure SetApplyContext(ApplicationDate: Date; DocumentNo: Code[20]; JournalTemplateName: Code[10]; JournalBatchName: Code[10])
    begin
        ...
        JournalTemplateNameContext := JournalTemplateName;
        JournalBatchNameContext := JournalBatchName;
    end;
