codeunit 424 "Export Analysis View"
{
    ...
    local procedure FindGLAccountParent(var Account: Code[20]; Indent: Integer)
    begin
        if Account <> '' then begin
            ...
            if TempGLAcc3.Indentation = Indent - 1 then
                ...
                LastGLAccount := TempGLAcc3."No.";
                Account := '';
            end;
        end else begin
            TempGLAcc3.Get(LastGLAccount);
            if TempGLAcc3.Indentation = Indent - 1 then
                Account := TempGLAcc3."No."
            else
                Account := '';
