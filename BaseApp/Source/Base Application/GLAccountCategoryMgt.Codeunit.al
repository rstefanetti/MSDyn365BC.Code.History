codeunit 570 "G/L Account Category Mgt."
{
    ...
    var
        ...
        AdvancesTxt: Label 'Advances';
        TaxesTxt: Label 'Taxes';
        OtherIncomeTxt: Label 'Other Income';
        FinancialExtraIncomeTxt: Label 'Financial and Extra Income';
        ServicesTxt: Label 'Services';
        DeprecationsTxt: Label 'Deprecations';
        FinancialExpenseTxt: Label 'Financial Expense';
    ...
    begin
        ...
        AddLocalAccountCategories; // NAVCZ
    end;

    local procedure AddLocalAccountCategories()
    var
        GLAccountCategory: Record "G/L Account Category";
    begin
        // NAVCZ
        with GLAccountCategory do begin
            if GetAccountSubcategory(GLAccountCategory, "Account Category"::Assets, CurrentAssetsTxt) then
                AddCategory(0, "Entry No.", "Account Category", AdvancesTxt, false, "Additional Report Definition"::"Operating Activities");

            Reset;
            if GetAccountCategory(GLAccountCategory, "Account Category"::Liabilities) then begin
                AddCategory(0, "Entry No.", "Account Category", AdvancesTxt, false, "Additional Report Definition"::"Operating Activities");
                AddCategory(0, "Entry No.", "Account Category", TaxesTxt, false, "Additional Report Definition"::"Financing Activities");
            end;

            if GetAccountCategory(GLAccountCategory, "Account Category"::Income) then begin
                AddCategory(0, "Entry No.", "Account Category", OtherIncomeTxt, false, 0);
                AddCategory(0, "Entry No.", "Account Category", FinancialExtraIncomeTxt, false, 0);
            end;

            if GetAccountCategory(GLAccountCategory, "Account Category"::Expense) then begin
                AddCategory(0, "Entry No.", "Account Category", ServicesTxt, false, 0);
                AddCategory(0, "Entry No.", "Account Category", DeprecationsTxt, false, 0);
                AddCategory(0, "Entry No.", "Account Category", FinancialExpenseTxt, false, 0);
            end;
        end;
    end;
    ...
    procedure GetAdvances(): Text
    begin
        // NAVCZ
        exit(AdvancesTxt);
    end;

    procedure GetTaxes(): Text
    begin
        // NAVCZ
        exit(TaxesTxt);
    end;

    procedure GetOtherIncome(): Text
    begin
        // NAVCZ
        exit(OtherIncomeTxt);
    end;

    procedure GetFinancialAndExtraIncome(): Text
    begin
        // NAVCZ
        exit(FinancialExtraIncomeTxt);
    end;

    procedure GetServices(): Text
    begin
        // NAVCZ
        exit(ServicesTxt);
    end;

    procedure GetDeprecations(): Text
    begin
        // NAVCZ
        exit(DeprecationsTxt);
    end;

    procedure GetFinancialExpense(): Text
    begin
        // NAVCZ
        exit(FinancialExpenseTxt);
    end;
