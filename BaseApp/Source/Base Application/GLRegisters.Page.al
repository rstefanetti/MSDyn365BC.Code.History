    ...
    actions
    {
        area(navigation)
        {
            group("&Register")
            {
                Caption = '&Register';
                Image = Register;
                action("G/L Book Ledger")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'G/L Book Ledger';
                    Image = LedgerBook;
                    ToolTip = 'View the general ledger transaction details that result from posted sales and purchase transactions. ';

                    trigger OnAction()
                    var
                        GLBookEntry: Record "GL Book Entry";
                    begin
                        GLBookEntry.SetRange("Entry No.", "From Entry No.", "To Entry No.");
                        PAGE.Run(PAGE::"GL Book Entries", GLBookEntry);
                    end;
                }
                ...
                action("VAT Book Entries")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'VAT Book Entries';
                    Image = VATEntries;
                    ToolTip = 'View the VAT entries that were placed in a general ledger register in connection with posting.';

                    trigger OnAction()
                    var
                        VATBookEntry: Record "VAT Book Entry";
                    begin
                        VATBookEntry.SetRange("Entry No.", "From VAT Entry No.", "To VAT Entry No.");
                        PAGE.Run(PAGE::"VAT Book Entries", VATBookEntry);
                    end;
                }
                action("VAT Entries")
                {
