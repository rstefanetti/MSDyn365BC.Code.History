codeunit 11 "Gen. Jnl.-Check Line"
{
    ...
    var
        ...
        InconsistenAppErr: Label 'You cannot apply these entries because unapplication could create inconsistent entries.';
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if not OverrideDimErr then begin
                DimMgt.SetCheckDim("Posting Date");  // NAVCZ
        ...
        case AccountType of
            AccountType::Customer:
                if Customer.Get(AccountNo) then begin
                    if not Compensation or (Customer.Blocked <> Customer.Blocked::Invoice) then // NAVCZ
        ...
        if (VATAllowPostingFrom = 0D) and (VATAllowPostingTo = 0D) then begin
            if UserId <> '' then
                if UserSetup.Get(UserId) then begin
                    VATAllowPostingFrom := UserSetup."Allow VAT Posting From";
                    VATAllowPostingTo := UserSetup."Allow VAT Posting To";
    ...
    begin
        // NAVCZ
        exit(IsVATDateNotAllowed(VATDate, SetupRecordID));
    end;
    ...
    begin
        // NAVCZ
        VATPeriod.SetRange("Starting Date", 0D, VATDate);
    ...
    begin
        // NAVCZ
        if ((GenJnlLine."Bal. VAT Amount" <> 0) or (GenJnlLine."VAT Amount" <> 0)) and
