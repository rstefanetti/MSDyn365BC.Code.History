table 81 "Gen. Journal Line"
{
    ...
    fields
    {
        ...
        field(6; "Document Type"; Enum "Gen. Journal Document Type")
        {
            ...
            begin
                ...
                if "Document Type" = "Document Type"::"Credit Memo" then begin
                    if "Refers to Period" <> "Refers to Period"::" " then
                        "Refers to Period" := "Refers to Period"::Current
                end else
        ...
        field(12; "Currency Code"; Code[10])
        {
            ...
            begin
                ...
                if "Currency Code" <> '' then begin
                    ...
                    then
                        "Currency Factor" :=
                          CurrExchRate.ExchangeRate(ExchangeDate, "Currency Code");
                end else
                    "Currency Factor" := 0;
        ...
        field(47; "Payment Terms Code"; Code[10])
        {
            ...
            begin
                ...
                "Payment Discount %" := 0;
                if "Payment Terms Code" <> xRec."Payment Terms Code" then
                    DeleteSalesPaymentLines();
                if ("Account Type" <> "Account Type"::"G/L Account") or
                   ("Bal. Account Type" <> "Bal. Account Type"::"G/L Account")
                then
                    if ("Document Type" in ["Document Type"::Invoice, "Document Type"::"Credit Memo"]) and
                       ("Payment Terms Code" <> '') and ("Document Date" <> 0D) and
        ...
        field(63; "Bal. Account Type"; Enum "Gen. Journal Account Type")
        {
            ...
            begin
                ...
                if xRec."Bal. Account Type" in [xRec."Bal. Account Type"::Customer, xRec."Bal. Account Type"::Vendor] then
                    DeleteSalesPaymentLines();
                if ("Account Type" in [
        ...
        field(64; "Bal. Gen. Posting Type"; Enum "General Posting Type")
        {
            ...
            begin
                ...
                Validate("Include in VAT Transac. Rep.");
        ...
        field(172; "Payment Method Code"; Code[10])
        {
            ...
            begin
                ...
                if PaymentMethod.Get("Payment Method Code") then
                    if (PaymentMethod."Bill Code" <> '') and ("Document Type" = "Document Type"::Invoice) then begin
        ...
        field(12103; "Bal. Deductible %"; Decimal)
        {
            Caption = 'Bal. Deductible %';
            DecimalPlaces = 0 : 5;
            MaxValue = 100;
            MinValue = 0;
        }
        field(12110; "Operation Occurred Date"; Date)
        {
        ...
        field(12112; "Reverse Sales VAT No."; Code[20])
        {
            Caption = 'Reverse Sales VAT No.';
        }
        field(12113; "Closing Balance Sheet"; Boolean)
        {
        ...
        field(12128; "Include in VAT Transac. Rep."; Boolean)
        {
        ...
        field(12174; "Cumulative Bank Receipts"; Boolean)
        {
            Caption = 'Cumulative Bank Receipts';
        }
        field(12175; "Document Type to Close"; Enum "Gen. Journal Document Type")
        {
            Caption = 'Document Type to Close';
        }
        field(12176; "Document No. to Close"; Code[20])
        {
            Caption = 'Document No. to Close';
        }
        field(12177; "Document Occurrence to Close"; Integer)
        {
            Caption = 'Document Occurrence to Close';
        }
        field(12179; "Allow Issue"; Boolean)
        {
            Caption = 'Allow Issue';
        }
    ...
    begin
        ...
        if GenJnlLine.FindFirst then begin
            ...
            "Operation Occurred Date" := LastGenJnlLine."Posting Date";
            "Bill-to/Pay-to No." := LastGenJnlLine."Bill-to/Pay-to No.";
        ...
        "Posting No. Series" := GenJnlBatch."Posting No. Series";
        "Refers to Period" := LastGenJnlLine."Refers to Period";
    ...
    procedure InsertPaymentTermsLines()
    begin
        GenJnlTemplate.Get("Journal Template Name");
        ...
        if PaymentTermsLine.Find('-') then begin
            ...
            repeat
                if "Account Type" = "Account Type"::Vendor then
                    CreatePurchasePaymentLine()
                else
                    CreateSalesPaymentLine();
            until PaymentTermsLine.Next() = 0;
        end;
    end;

    [Obsolete('Replaced by DeleteSalesPaymentLines()', '17.0')]
    [Scope('OnPrem')]
    ...
    procedure DeleteSalesPaymentLines()
    begin
        PaymentSales.Reset();
        PaymentSales.SetRange("Sales/Purchase", PaymentSales."Sales/Purchase"::" ");
        PaymentSales.SetRange(Type, PaymentSales.Type::"General Journal");
        PaymentSales.SetRange(Code, "Journal Batch Name");
        PaymentSales.SetRange("Journal Template Name", "Journal Template Name");
        PaymentSales.SetRange("Journal Line No.", "Line No.");
        if PaymentSales.FindFirst then
            PaymentSales.DeleteAll();
    end;

    [Scope('OnPrem')]
    procedure PurchasePayment()
    begin
        CreatePurchasePaymentLine();
    end;

    PROCEDURE CreatePurchasePaymentLine();
    var
        Vend: Record Vendor;
    begin
        PaymentPurchase.Init();
        PaymentPurchase."Sales/Purchase" := PaymentPurchase."Sales/Purchase"::" ";
        PaymentPurchase.Type := PaymentPurchase.Type::"General Journal";
        PaymentPurchase."Journal Template Name" := "Journal Template Name";

        if "Line No." = 0 then
            PaymentPurchase."Journal Line No." := 10000
        else
            PaymentPurchase."Journal Line No." := "Line No.";

        PaymentPurchase.Code := "Journal Batch Name";
        PaymentCounter := PaymentCounter + 10000;
        PaymentPurchase."Line No." := PaymentCounter;
        PaymentPurchase."Payment %" := PaymentTermsLine."Payment %";
        PaymentPurchase."Due Date Calculation" := PaymentTermsLine."Due Date Calculation";
        PaymentPurchase."Discount Date Calculation" := PaymentTermsLine."Discount Date Calculation";
        PaymentPurchase."Discount %" := PaymentTermsLine."Discount %";
        PaymentPurchase."Due Date" := CalcDate(PaymentTermsLine."Due Date Calculation", "Document Date");

        if PaymentPurchase."Due Date" < "Document Date" then
            PaymentPurchase."Due Date" := "Document Date";

        FixedDueDate.Reset();
        Vend.Get("Account No.");
        if Vend."Apply Company Payment days" = true then begin
            FixedDueDate.SetRange(Type, FixedDueDate.Type::Company);
            FixedDueDate.SetRange(Code, '');
        end else begin
            FixedDueDate.SetRange(Type, FixedDueDate.Type::Vendor);
            FixedDueDate.SetRange(Code, "Account No.");
        end;

        if FixedDueDate.Count > 0 then begin
            FixedDueDate.SetRange("Payment Days", Date2DMY(PaymentPurchase."Due Date", 1), 99);
            if FixedDueDate.FindFirst then begin
                Day := FixedDueDate."Payment Days";
                MaximumDay := Date2DMY(CalcDate('CM', PaymentPurchase."Due Date"), 1);
                if Day > MaximumDay then
                    Day := MaximumDay;
                Month := Date2DMY(PaymentPurchase."Due Date", 2);
                Year := Date2DMY(PaymentPurchase."Due Date", 3);
                PaymentPurchase."Due Date" := DMY2Date(Day, Month, Year);
            end else begin
                FixedDueDate.SetRange("Payment Days");
                FixedDueDate.FindFirst;
                Day := FixedDueDate."Payment Days";
                MaximumDay := Date2DMY(CalcDate('CM+1M', PaymentPurchase."Due Date"), 1);
                if Day > MaximumDay then
                    Day := MaximumDay;
                Month := Date2DMY(PaymentPurchase."Due Date", 2) + 1;
                Year := Date2DMY(PaymentPurchase."Due Date", 3);
        ...
        if not PaymentTerms."Calc. Pmt. Disc. on Cr. Memos" and ("Document Type" = "Document Type"::"Credit Memo") then begin
            PaymentPurchase."Discount %" := 0;
            Evaluate(PaymentPurchase."Discount Date Calculation", '<0D>');
            PaymentPurchase."Due Date" := "Posting Date";
        end else begin
            PaymentPurchase."Pmt. Discount Date" := CalcDate(PaymentTermsLine."Discount Date Calculation", "Document Date");
            if PaymentPurchase."Pmt. Discount Date" < "Document Date" then
                PaymentPurchase."Pmt. Discount Date" := "Document Date";
        end;

        PaymentPurchase.Insert();
    end;

    [Obsolete('Replaced by CreateSalesPaymentLine().', '17.0')]
    [Scope('OnPrem')]
    procedure SalePayment()
    begin
        CreateSalesPaymentLine();
    end;

    procedure CreateSalesPaymentLine()
    begin
        PaymentSales.Init();
        PaymentSales."Sales/Purchase" := PaymentSales."Sales/Purchase"::" ";
        PaymentSales.Type := PaymentSales.Type::"General Journal";
        PaymentSales."Journal Template Name" := "Journal Template Name";

        if "Line No." = 0 then
            PaymentSales."Journal Line No." := 10000
        else
            PaymentSales."Journal Line No." := "Line No.";

        PaymentSales.Code := "Journal Batch Name";
        PaymentCounter := PaymentCounter + 10000;
        ...
        DefDueDates.SetRange("No.", "Account No.");
        ...
        if not PaymentTerms."Calc. Pmt. Disc. on Cr. Memos" and ("Document Type" = "Document Type"::"Credit Memo") then begin
            ...
            PaymentSales."Due Date" := "Posting Date"
        end else begin
    ...
    [Scope('OnPrem')]
    procedure CheckincludeInVATSetup(): Boolean
    begin
        if ("Account Type" = "Account Type"::"G/L Account") or ("Account Type" = "Account Type"::"Fixed Asset") then begin
            if ("Gen. Posting Type" = "Gen. Posting Type"::Sale) or ("Gen. Posting Type" = "Gen. Posting Type"::Purchase) then
                if VATPostingSetup.IncludeInVATTransReport("VAT Bus. Posting Group", "VAT Prod. Posting Group") then
                    exit(true);
        end else
            if "Bal. Account Type" = "Bal. Account Type"::"G/L Account" then
                if ("Bal. Gen. Posting Type" = "Bal. Gen. Posting Type"::Sale) or
    ...
    local procedure GetReverseSalesVATNoSeries()
    var
        VATBusPostingGroup: Record "VAT Business Posting Group";
        NoSeries: Record "No. Series";
    begin
        "Reverse Sales VAT No. Series" := '';
        if ("Document Type" = "Document Type"::Invoice) and ((("Bal. Gen. Posting Type" = "Bal. Gen. Posting Type"::Purchase) and
          ("Bal. VAT Calculation Type" = "Bal. VAT Calculation Type"::"Reverse Charge VAT") and
          ("Bal. VAT Bus. Posting Group" <> '')) or (("Gen. Posting Type" = "Gen. Posting Type"::Purchase) and
          ("VAT Calculation Type" = "VAT Calculation Type"::"Reverse Charge VAT") and
    ...
    begin
        ...
        "VAT Identifier" := PrepmtInvLineBuffer."VAT Identifier";
        "Deductible %" := PrepmtInvLineBuffer."Deductible %";
        "Service Tariff No." := PrepmtInvLineBuffer."Service Tariff No.";
        "Transport Method" := PrepmtInvLineBuffer."Transport Method";
        "Refers to Period" := PrepmtInvLineBuffer."Prepmt. CM Refers to Period";
