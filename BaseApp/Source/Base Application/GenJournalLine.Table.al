table 81 "Gen. Journal Line"
{
    ...
    fields
    {
                    ...
                    "VAT Calculation Type"::"Full VAT":
                        ...
                        begin
                            if ("Prepayment Type" = "Prepayment Type"::Advance) and not "System-Created Entry" and
                               (("Amount (LCY)" <> xRec."Amount (LCY)") or ("Advance VAT Base Amount" = 0))
                            then begin
                                VATPostingSetup.Get("VAT Bus. Posting Group", "VAT Prod. Posting Group");
                                "Advance VAT Base Amount" :=
                                  Round("Amount (LCY)" / VATPostingSetup."VAT %" * 100, Currency."Amount Rounding Precision");
                            end;
                            // NAVCZ
                            "VAT Amount" := Amount;
                        end; // NAVCZ
        ...
        field(11700; "Bank Account Code"; Code[20])
        {
            ...
            begin
                ...
                case true of
                    ("Account Type" = "Account Type"::Customer) and ("Document Type" in ["Document Type"::Payment, "Document Type"::"Credit Memo"]),
                    ("Bal. Account Type" = "Bal. Account Type"::Customer) and
                    ("Document Type" in ["Document Type"::Payment, "Document Type"::"Credit Memo"]):
                        begin
                            CustBankAcc.Get("Bill-to/Pay-to No.", "Bank Account Code");
                    ...
                    ("Account Type" = "Account Type"::Customer) and ("Document Type" in ["Document Type"::Refund, "Document Type"::Invoice]),
                    ("Bal. Account Type" = "Bal. Account Type"::Customer) and
                    ("Document Type" in ["Document Type"::Refund, "Document Type"::Invoice]),
                    ("Account Type" = "Account Type"::Vendor) and ("Document Type" in ["Document Type"::Payment, "Document Type"::"Credit Memo"]),
                    ...
                    ("Account Type" = "Account Type"::Vendor) and ("Document Type" in ["Document Type"::Refund, "Document Type"::Invoice]),
                    ("Bal. Account Type" = "Bal. Account Type"::Vendor) and
                    ("Document Type" in ["Document Type"::Refund, "Document Type"::Invoice]):
                        begin
                            VendBankAcc.Get("Bill-to/Pay-to No.", "Bank Account Code");
                        ...
                        Error(BankAccountCodeErr, "Account Type", FieldCaption("Bal. Account Type"), FieldCaption("Document Type"),
        ...
        field(31102; "Original Document Partner No."; Code[20])
        {
            ...
            begin
                // NAVCZ
                TestField("Original Document Partner Type");
                if "Original Document Partner No." <> '' then
                    case "Original Document Partner Type" of
                        "Original Document Partner Type"::Customer:
                            begin
                                Cust.Get("Original Document Partner No.");
                                Validate("Country/Region Code", Cust."Country/Region Code");
                                Validate("VAT Registration No.", Cust."VAT Registration No.");
                            end;
                        "Original Document Partner Type"::Vendor:
                            begin
                                Vend.Get("Original Document Partner No.");
            ...
            AccType::"G/L Account":
                begin
                    ...
                    if xRec."Applies-to ID" <> '' then begin
                        ...
                        GLEntry.SetRange("Applies-to ID", xRec."Applies-to ID");
                        ...
                        if xRec."Applies-to Doc. No." <> '' then begin
                            ...
                            GLEntry.SetRange("Document No.", xRec."Applies-to Doc. No.");
                            GLEntry.SetRange("Document Type", xRec."Applies-to Doc. Type");
                            GLEntry.SetRange("G/L Account No.", AccNo);
                            GLEntry.SetRange(Closed, false);
                            if GLEntry.FindSet then
                                repeat
    ...
    procedure GetAppliedCustLedgerEntry(var AppliedCustLedgEntry: Record "Cust. Ledger Entry")
    begin
        // NAVCZ
        AppliedCustLedgEntry.Copy(CustLedgEntry);
    end;

    procedure GetAppliedVendLedgerEntry(var AppliedVendLedgEntry: Record "Vendor Ledger Entry")
    begin
        // NAVCZ
        AppliedVendLedgEntry.Copy(VendLedgEntry);
    end;

    procedure GetAppliedEmplLedgerEntry(var AppliedEmplLedgEntry: Record "Employee Ledger Entry")
    begin
        // NAVCZ
    ...
    begin
        // NAVCZ
        PrepmtLinksMgt.LinkGenJnlLine(Rec);
    end;
    ...
    begin
        // NAVCZ
        PrepmtLinksMgt.UnLinkGenJnlLine(Rec);
    end;
