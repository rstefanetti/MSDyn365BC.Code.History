table 81 "Gen. Journal Line"
{
    ...
    var
        ...
        AccNo: Code[20];
    begin
        if "Bal. Account Type" in ["Bal. Account Type"::Customer, "Bal. Account Type"::Vendor, "Bal. Account Type"::Employee] then begin
        ...
        case AccType of
                    ...
                    if ApplyCustEntries.RunModal = ACTION::LookupOK then begin
                        ...
                        if "Currency Code" <> CustLedgEntry."Currency Code" then
                            if Amount = 0 then
                                ...
                                GenJnlApply.CheckAgainstApplnCurrency("Currency Code", CustLedgEntry."Currency Code",
            ...
            AccType::Employee:
                LookupAppliesToDocNoForEmployee(AccNo);
        end;
    ...
    local procedure LookupAppliesToDocNoForEmployee(AccNo: Code[20])
    var
    ...
    begin
        ...
        ApplyEmployeeEntries.SetGenJnlLine(Rec, GenJnlLine.FieldNo("Applies-to Doc. No."));
        ApplyEmployeeEntries.SetTableView(EmplLedgEntry);
        ApplyEmployeeEntries.SetRecord(EmplLedgEntry);
        ApplyEmployeeEntries.LookupMode(true);
        if ApplyEmployeeEntries.RunModal = ACTION::LookupOK then begin
            ApplyEmployeeEntries.GetRecord(EmplLedgEntry);
            EmplLedgEntry.CalcFields("Remaining Amount");
            Clear(ApplyEmployeeEntries);
            if "Currency Code" <> EmplLedgEntry."Currency Code" then
                if Amount = 0 then
                    Validate("Currency Code", EmplLedgEntry."Currency Code")
                else
                    GenJnlApply.CheckAgainstApplnCurrency(
                      "Currency Code", EmplLedgEntry."Currency Code", GenJnlLine."Account Type"::Employee, true);
            if Amount = 0 then begin
