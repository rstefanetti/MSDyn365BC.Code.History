table 81 "Gen. Journal Line"
{
    ...
    fields
    {
        ...
        field(5; "Posting Date"; Date)
        {
            ...
            begin
                ...
                if ("Posting Date" <> xRec."Posting Date") and (Amount <> 0) then begin
                    ChangePostingDateWarning();
            ...
            begin
                ...
                if DTASetup.ReadPermission then
                    if CurrFieldNo = FieldNo("Currency Code") then
                        DTAMgt.CheckEsrCurrency("Currency Code", "ESR/ISR Coding Line");
            end;
        }
        ...
        field(288; "Recipient Bank Account"; Code[20])
        {
            ...
            begin
                ...
                if "Account Type" = "Account Type"::Vendor then
                    if VendBank.Get("Account No.", "Recipient Bank Account") then
        ...
        field(3010544; "ESR/ISR Coding Line"; Code[70])
        {
            ...
            trigger OnValidate()
            begin
                DTAMgt.ProcessGlLine(Rec, "ESR/ISR Coding Line");
            end;
        }
        field(3010545; "Debit Bank"; Code[20])
        {
            Caption = 'Debit Bank';
    ...
    keys
    {
        ...
        key(Key7; "Journal Template Name", "Journal Batch Name", "Posting Date", Clearing, "Debit Bank", "Account No.", "Currency Code")
        {
        }
        key(Key8; "Document No.", "Posting Date", "Source Code")
        {
            MaintainSQLIndex = false;
            SumIndexFields = "VAT Amount (LCY)", "Bal. VAT Amount (LCY)";
        }
        key(Key9; "Data Exch. Entry No.")
        {
        }
        key(Key10; "Journal Batch Name", "Journal Template Name")
        {
    ...
    begin
        ...
        if (Amount <> xRec.Amount) or ("Amount (LCY)" <> xRec."Amount (LCY)") then begin
            if CurrFieldNo = 0 then                     // batch job
            ...
            then
                if FAPostingGr.GetPostingGroup("Posting Group", "Depreciation Book Code") then begin
                    ...
                    if not FABalAcc then begin
                        ...
                        Validate("Bal. VAT Bus. Posting Group", LocalGLAcc."VAT Bus. Posting Group");
                        Validate("Bal. VAT Prod. Posting Group", LocalGLAcc."VAT Prod. Posting Group");
    ...
    var
        AppVendorEntry: Record "Vendor Ledger Entry";
        OldPmtDiscPossible: Boolean;
        NewPmtDiscPossible: Boolean;
    begin
        if ("Applies-to Doc. No." = '') or
           ("Document Type" <> "Document Type"::Payment) or
           (CurrFieldNo <> FieldNo("Posting Date")) or
           ("Account Type" <> "Account Type"::Vendor) or
           (Amount = 0)
        then
            exit;

        AppVendorEntry.SetCurrentKey("Document No.");
        AppVendorEntry.SetRange("Document Type", "Applies-to Doc. Type");
        AppVendorEntry.SetRange("Document No.", "Applies-to Doc. No.");
        if AppVendorEntry.FindFirst then
            if AppVendorEntry."Remaining Pmt. Disc. Possible" <> 0 then begin
                OldPmtDiscPossible := (xRec."Posting Date" <= AppVendorEntry."Pmt. Discount Date") or
                  (xRec."Posting Date" <= AppVendorEntry."Pmt. Disc. Tolerance Date");
                NewPmtDiscPossible := ("Posting Date" <= AppVendorEntry."Pmt. Discount Date") or
                  ("Posting Date" <= AppVendorEntry."Pmt. Disc. Tolerance Date");
                if OldPmtDiscPossible <> NewPmtDiscPossible then
                    Message(Text11500);
            end;
    end;
    ...
    begin
        ...
        Validate("Payment Terms Code");
        CheckPaymentTolerance;
        if VendBank.Get("Account No.", Vend."Preferred Bank Account Code") then
            "Payment Fee Code" := VendBank."Payment Fee Code";
        if DTASetup.ReadPermission then
            DTAMgt.TransferVendorGlLine(Rec);
