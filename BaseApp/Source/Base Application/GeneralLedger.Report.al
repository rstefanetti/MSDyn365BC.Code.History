report 11771 "General Ledger"
{
    ...
    dataset
    {
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            column(STRSUBSTNO___1__2__gtcPeriod_gtePeriodText_; StrSubstNo('%1 %2', Period, PeriodText))
            {
            }
            ...
            column(STRSUBSTNO_gtcStartBalance__FORMAT_gdaStartDate__; StrSubstNo(StartBalance, Format(StartDate)))
            {
            }
                ...
                dataitem(GlAccount2; "G/L Account")
                {
                    ...
                    begin
                        if Level = 0 then
                            ...
                            SetFilter("No.", StrSubstNo('%1%2', AccountFilter, '*'));
                    end;
                }
                dataitem(TotalPer; "Integer")
                {
                    DataItemTableView = SORTING(Number) WHERE(Number = CONST(1));
                    column(STRSUBSTNO_gtcNetChange_gdaStartPeriod_gdaEndPeriod_; StrSubstNo(NetChange, StartPeriod, EndPeriod))
                    {
                    }
                    ...
                    column(STRSUBSTNO_gtcEndBalance_gdaEndPeriod_; StrSubstNo(EndBalance, EndPeriod))
                    {
                    }
            ...
            dataitem(TotalAcc; "Integer")
            {
                ...
                column(STRSUBSTNO_gtcEndBalance_gdaEndDate_; StrSubstNo(EndBalance, EndDate))
                {
                }
                ...
                column(STRSUBSTNO_gtcNetChange_gdaStartDate_gdaEndDate_; StrSubstNo(NetChange, StartDate, EndDate))
                {
                }
            ...
            begin
                ...
                if Level = 0 then begin
                    ...
                    if CopyStr("No.", 1, Level) = AccountFilter then
                        ...
                        AccountFilter := CopyStr("No.", 1, Level);
                        GLAccount.SetFilter("No.", StrSubstNo('%1%2', AccountFilter, '*'));
                        GLAccount.SetFilter("Account Type", '<>%1', GLAccount."Account Type"::Posting);
                        if GLAccount.Find('-') then
                            GLAccDescr := GLAccount.Name
                        else
                            GLAccDescr := StrSubstNo('%1 %2', AccountFilter, '*');
    ...
    var
        ...
        IncomeInFY: Boolean;
        Period: Label 'Period:';
        StartBalance: Label 'Starting balance to %1';
        EndBalance: Label 'Final balance to %1';
        NetChange: Label 'Net Change period %1..%2';
