    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Local Currency"; "Local Currency")
                {
    ...
    actions
    {
        area(processing)
        {
            group("F&unctions")
            {
                ...
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Get Allowed Posting Range';
                    Image = Ranges;
                    RunObject = Page "Allowed Posting Range";
                    ShortCutKey = 'F7';
                    ToolTip = 'View the date range in which posting is allowed. The range uses the start and end dates of the first and last fiscal periods that are not fiscally closed.';
                }
            }
        }
        area(navigation)
