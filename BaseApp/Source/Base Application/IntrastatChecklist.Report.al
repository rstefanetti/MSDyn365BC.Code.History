report 502 "Intrastat - Checklist"
{
    ...
    dataset
    {
        dataitem("Intrastat Jnl. Batch"; "Intrastat Jnl. Batch")
        {
            ...
            dataitem("Intrastat Jnl. Line"; "Intrastat Jnl. Line")
            {
                ...
                column(CountryRegionOfOriginName; CountryRegionOfOrigin.Name)
                {
                }
                column(CountryRegionOfOriginCodeCaption; CountryRegionOfOriginCodeCaptionLbl)
                {
                }
    ...
    local procedure SetCountryRegionOfOriginIntrastatCode(IntrastatJnlLine: Record "Intrastat Jnl. Line")
    begin
        if IntrastatJnlLine."Country/Region of Origin Code" = '' then
            exit;
        CountryRegionOfOrigin.Get(IntrastatJnlLine."Country/Region of Origin Code");
        if CountryRegionOfOrigin."Intrastat Code" <> '' then
            CountryRegionOfOriginIntrastatCode := CountryRegionOfOrigin."Intrastat Code"
        else
            CountryRegionOfOriginIntrastatCode := CountryRegionOfOrigin.Code;
    end;
}
