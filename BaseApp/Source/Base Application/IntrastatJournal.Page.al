page 311 "Intrastat Journal"
{
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            action("Export DEB DTI+")
            {
                ApplicationArea = BasicEU;
                Caption = 'Export DEB DTI+';
                Ellipsis = true;
                ToolTip = 'Export Intrastat journal lines to an XML file that is based on the DTI+ format.';
                ...
                begin
                    ...
                    REPORT.Run(REPORT::"Export DEB DTI", true, false, IntrastatJnlBatch);
                end;
            }
            action(Form)
            {
