codeunit 311 "Item-Check Avail."
{
    ...
    [Scope('OnPrem')]
    procedure ItemDocCheckLine(ItemDocLine: Record "Item Document Line") Rollback: Boolean
    begin
        NotificationLifecycleMgt.RecallNotificationsForRecordWithAdditionalContext(
          ItemDocLine.RecordId, GetItemAvailabilityNotificationId, true);
        if ItemDocLineShowWarning(ItemDocLine) then
            Rollback := ShowAndHandleAvailabilityPage(ItemDocLine.RecordId);
    end;

    [Scope('OnPrem')]
    procedure ItemDocLineShowWarning(ItemDocLine: Record "Item Document Line"): Boolean
    var
        SalesSetup: Record "Sales & Receivables Setup";
    begin
        SalesSetup.Get();
        ...
        case ItemDocLine."Document Type" of
            ItemDocLine."Document Type"::Receipt:
                ItemNetChange := ItemDocLine.Quantity;
        ...
        exit(
          ShowWarning(
            ...
            ItemDocLine."Unit of Measure Code",
            ItemDocLine."Qty. per Unit of Measure",
            ItemNetChange,
            0,
            0D,
            0D));
