codeunit 21 "Item Jnl.-Check Line"
{
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            if "Red Storno" and InvtSetup."Enable Red Storno" and not ItemJnlLineReserve.ReservEntryExist(ItemJnlLine) then
    ...
    procedure FindValueEntry(ItemLedgerEntryNo: Integer) ValueEntryNo: Integer
    var
    ...
    procedure RedStornoPostingCheck(ItemJnlLine: Record "Item Journal Line")
    var
        Item: Record Item;
    begin
        with ItemJnlLine do begin
            if Item.Get("Item No.") then
                if Item."Costing Method" = Item."Costing Method"::Standard then
                    Item.FieldError("Costing Method");
            ...
            if "Entry Type" = "Entry Type"::"Negative Adjmt." then begin
                ...
                CheckGenBusPostingGroup(ItemJnlLine, "Applies-from Entry");
            end;

            if "Entry Type" = "Entry Type"::"Positive Adjmt." then begin
                TestField("Applies-to Entry");
                TestField("Applies-from Entry", 0);
                CheckGenBusPostingGroup(ItemJnlLine, "Applies-to Entry");
            end;

            if "Document Type" = "Document Type"::"Sales Return Receipt" then
                TestField("Applies-from Entry");

            if ("Document Type" = "Document Type"::"Purchase Return Shipment") or
              ("Entry Type" = "Entry Type"::Transfer) then
                TestField("Applies-to Entry");

            if ("Value Entry Type" = "Value Entry Type"::Revaluation) and
              ("Unit Cost" > 0) then
                TestField("Red Storno", false);

        end;
    end;

    procedure CheckGenBusPostingGroup(ItemJnlLine: Record "Item Journal Line"; ItemLedgerEntryNo: Integer)
    var
        ...
        with ItemJnlLine do begin
            ...
            then begin
                ...
                if ("FA No." <> '') or (LocItemLedgerEntry."FA No." <> '') then begin
                    if "FA No." <> LocItemLedgerEntry."FA No." then
                        Error(Text12401, FieldCaption("FA No."), Format(LocItemLedgerEntry."FA No."));
                    if "Depreciation Book Code" <> LocItemLedgerEntry."Depreciation Book Code" then
                    ...
                    if LocLatestItemLedgerEntry.FindFirst then
                        Error(Text12402, FieldCaption("FA No."), Format("FA No."),
                          FieldCaption("Depreciation Book Code"), Format("Depreciation Book Code"));

                    if FADeprBook.Get("FA No.", "Depreciation Book Code") then begin
                        FADeprBook.CalcFields("Book Value");
                        if FADeprBook."Book Value" < -Amount then
                            Error(Text12403, FADeprBook.FieldCaption("Book Value"), Format(FADeprBook."Book Value"),
                              FieldCaption("FA No."), Format("FA No."),
                              FieldCaption("Depreciation Book Code"), Format("Depreciation Book Code"),
                              FieldCaption(Amount), Format(Amount),
                              FieldCaption("Line No."), Format("Line No."));
                    end;
                end;
                //end FA check
            ...
            //Gen Bus/Prod check
            if ValueEntry.Get(FindValueEntry(ItemLedgerEntryNo)) then begin
                if ValueEntry."Gen. Bus. Posting Group" <> "Gen. Bus. Posting Group" then
                    Error(Text12400, FieldCaption("Gen. Bus. Posting Group"), Format(LineNo),
                ...
                if ValueEntry."Gen. Prod. Posting Group" <> "Gen. Prod. Posting Group" then
                    Error(Text12400, FieldCaption("Gen. Prod. Posting Group"), Format(LineNo),
