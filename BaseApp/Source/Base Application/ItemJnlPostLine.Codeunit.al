codeunit 22 "Item Jnl.-Post Line"
{
    ...
    var
        ...
        IlegalApptErr: Label 'Ilegal application.\\ %1 %2 in %3 %7 %4 must not be less then %1 %5 in %3 %7 %6.';
    ...
    begin
        with ItemJnlLine do begin
            ...
            if InvtSetup."Posting Desc. Code" <> '' then begin
                ...
                InventoryPostingToGL.SetPostingDesc(PostingDesc.ParsePostDescString(PostingDesc, RecRef));
            end;
            // NAVCZ
            ...
            then begin
                ...
                */
                ...
                if CapLedgEntry.Subcontracting then begin
                    if lreWorkCenter.Get(CapLedgEntry."Work Center No.") then
                        lreWorkCenter.Init();
                    if lreWorkCenter."Unit Cost Calculation" = lreWorkCenter."Unit Cost Calculation"::Units then
                        CapLedgEntry."Completely Invoiced" := CapLedgEntry."Invoiced Quantity" = CapLedgEntry."Output Quantity"
                    else
                        CapLedgEntry."Completely Invoiced" := CapLedgEntry."Invoiced Quantity" = CapLedgEntry.Quantity;
                end else
    ...
    begin
        ...
        with ValueEntry do
            if ("Valued Quantity" > 0) and not ("Expected Cost" or ItemJnlLine.Adjustment) then begin
                ...
                if "Drop Shipment" and (LastDirectCost <> 0) then begin
                ...
                if not "Drop Shipment" then begin
                    ...
                    ItemCostMgt.UpdateUnitCost(
                      Item, "Location Code", "Variant Code", LastDirectCost, 0, not InvtSetup."Skip Update SKU on Posting", true, false, 0);
                end;
                // NAVCZ
            end;
    ...
    begin
        ...
        ItemTrackingMgt.GetItemTrackingSetup(
            ItemTrackingCode, ItemJnlLine."Entry Type".AsInteger(), ItemJnlLine.Signed(ItemJnlLine."Quantity (Base)") > 0, ItemTrackingSetup);
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            ItemLedgEntry."Net Weight" := "Net Weight";
            ItemLedgEntry."Country/Region of Origin Code" := "Country/Region of Origin Code";
            ItemLedgEntry."Intrastat Transaction" := "Intrastat Transaction";
            ItemLedgEntry."Physical Transfer" := "Physical Transfer";
            ItemLedgEntry."Perform. Country/Region Code" := "Perform. Country/Region Code";
            ItemLedgEntry."Currency Code" := "Currency Code";
            ItemLedgEntry."Currency Factor" := "Currency Factor";
            ...
            ItemLedgEntry."Maintenance Code" := "Maintenance Code";
    ...
    begin
        ...
        if Quantity < 0 then begin
            ...
            if InboundItemEntry <> 0 then begin
                GetInvtSetup;
                if InvtSetup."Date Order Inventory Change" then
                    if ItemLedgEntry2.Get(InboundItemEntry) then begin
                        if ItemLedgEntry2."Posting Date" > PostingDate then
                            Error(IlegalApptErr, ItemLedgEntry2.FieldCaption("Posting Date"), PostingDate, ItemLedgEntry2.TableCaption,
                              OutboundItemEntry, ItemLedgEntry2."Posting Date", InboundItemEntry, ItemLedgEntry2.FieldCaption("Entry No."));
                    end else begin
                        // for non chech issue (bellow zero quantity);
                        ItemLedgEntry2.Get(OutboundItemEntry);
                        if ItemLedgEntry2."Posting Date" < PostingDate then
                            Error(IlegalApptErr, ItemLedgEntry2.FieldCaption("Posting Date"), ItemLedgEntry2."Posting Date", ItemLedgEntry2.TableCaption,
                              OutboundItemEntry, PostingDate, InboundItemEntry, ItemLedgEntry2.FieldCaption("Entry No."));
                    end;
            end;
            // NAVCZ
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            // NAVCZ
            // Maintenance adjustment;
            if InvtSetup."Automatic Maintenance Posting" and
               (not ValueEntry."Expected Cost") and (ValueEntry."Entry Type" = ValueEntry."Entry Type"::"Direct Cost") and
               (ValueEntry."Item Ledger Entry Type" = ValueEntry."Item Ledger Entry Type"::"Negative Adjmt.") and
               ("FA No." <> '') and (ValueEntry."Cost Amount (Actual)" <> 0)
            then
                PostToFA("Document No.", "Posting Date", "FA No.", "Maintenance Code", -ValueEntry."Cost Amount (Actual)",
                  ValueEntry."Item Ledger Entry No.", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code");
            // NAVCZ
    ...
    begin
        if not InvtSetupRead then begin
            ...
            // NAVCZ
            if InvtSetup."Posting Desc. Code" <> '' then begin
                PostingDesc.Get(InvtSetup."Posting Desc. Code");
                PostingDesc.TestField(Type, PostingDesc.Type::"Post Inventory Cost");
                RecRef.Close;
    ...
    var
        ...
        DeprecBook: Record "Depreciation Book";
        FADeprBook: Record "FA Depreciation Book";
        FAMaintSetup: Record "FA Extended Posting Group";
    ...
    begin
        ...
        // Maintenance adjustment;
        GetInvtSetup;
        SourceCodeSetup.TestField("Maintenance Adjustment");
        FASetup.Get();

        FixedAssed.Get(FACode);
        FixedAssed.TestField("Deprec. Book Code (Mainten.)");
        DeprecBook.Get(FixedAssed."Deprec. Book Code (Mainten.)");

        if DeprecBook."G/L Integration - Maintenance" then begin
            with GenJnlLine do begin
                Init;
                "Posting Date" := PostingDate;
                "VAT Date" := PostingDate;
                Description := FixedAssed.Description;
                "Source Code" := SourceCodeSetup."Maintenance Adjustment";
                "Document Type" := "Document Type"::" ";
                "Document No." := DocNo;
                "Account Type" := "Account Type"::"Fixed Asset";
                "FA Posting Type" := "FA Posting Type"::Maintenance;
                "Account No." := FixedAssed."No.";
                "Depreciation Book Code" := DeprecBook.Code;
                "Maintenance Code" := MaintenanceCode;

                FADeprBook.Get(FixedAssed."No.", "Depreciation Book Code");
                "Posting Group" := FADeprBook."FA Posting Group";

                FASetup.Get();
                if FASetup."FA Maintenance By Maint. Code" then begin
                    FAMaintSetup.Get(FADeprBook."FA Posting Group", 2, MaintenanceCode);
                    FAMaintSetup.TestField("Maintenance Expense Account");
                    LocalGlAcc.Get(FAMaintSetup."Maintenance Expense Account");
                    FAMaintSetup.TestField("Maintenance Bal. Acc.");
                    BalAccountNo := FAMaintSetup."Maintenance Bal. Acc.";
                end else begin
                    FAPostingGr.Get(FADeprBook."FA Posting Group");
                    FAPostingGr.TestField("Maintenance Expense Account");
                    LocalGlAcc.Get(FAPostingGr."Maintenance Expense Account");
                    FAPostingGr.TestField("Maintenance Bal. Acc.");
                    BalAccountNo := FAPostingGr."Maintenance Bal. Acc.";
                end;
                ...
                InventoryPostingToGL.PostGLSpec(GenJnlLine);
            end;
        end else
    ...
    [Scope('OnPrem')]
    procedure xSetExtLotSN(ExternalLotSNNew: Boolean)
    begin
        // NAVCZ
    ...
    begin
        ...
        if ValueEntry."Cost Amount (Actual)" = 0 then
            GetCostAmt := ValueEntry."Cost Amount (Expected)"
        else
            GetCostAmt := ValueEntry."Cost Amount (Actual)";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertTempSplitItemJnlLine(ItemJournalLine: Record "Item Journal Line"; IsServUndoConsumption: Boolean; PostponeReservationHandling: Boolean; var TempSplitItemJnlLine: Record "Item Journal Line"; var PostItemJnlLine: Boolean; var IsHandled: Boolean)
    begin
    end;
