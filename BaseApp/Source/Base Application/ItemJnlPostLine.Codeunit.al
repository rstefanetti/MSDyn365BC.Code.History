codeunit 22 "Item Jnl.-Post Line"
{
    ...
    var
        ...
        CDNoRequired: Boolean;
        Text12400: Label '%1 must not be greater than %2 in %3 %4=%5';
        Text12405: Label 'Item in CD No. %1 CD Line No. %2 for Item No. %3 Variant %4 cannot be fully applied.';
        Text12410: Label '%1 with %2 %3 must be posted to %4';
        Text12411: Label '%1 with %2 %3 must be posted to G/L.';
        CDNoRequiredErr: Label 'You must assign a CD number for item %1.', Comment = '%1 - Item No.';
        Text12418: Label 'Item Tracking Serial No. %1 Lot No. %2 CD No. %3 for Item No. %4 Variant %5 cannot be fully applied.';
        PreviewMode: Boolean;
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            IsHandled := false;
            OnCodeOnBeforeCheckItemTracking(ItemJnlLine, DisableItemTracking, IsHandled);
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            OnPostItemOnAfterGetSKU(ItemJnlLine, SKUExists);
    ...
    begin
        with ItemJnlLine do begin
            ...
            ValueEntry."Posting Date" := GetValueEntryPostingDate(ValueEntry, "Posting Date");
            ValueEntry.Positive := ValueEntry."Valued Quantity" > 0;
    ...
    begin
        with ItemJnlLine do begin
            ...
            if InsertItemLedgEntryNeeded then begin
                ...
                CreateBatchLink(GlobalItemLedgEntry); // create link for Tax Register
            end;
        end;
    end;
        ...
        repeat
            ...
            if not CausedByTransfer and not PostponeReservationHandling then begin
                ...
                if UseReservationApplication then begin
                    ...
                    CheckApplicationDate(OldItemLedgEntry, ItemLedgEntry);
    ...
    begin
        IsHandled := false;
        OnBeforeCheckPostingDateWithExpirationDate(ItemLedgEntry, ItemTrackingCode, OldItemLedgEntry, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        with ToItemLedgEntry do begin
            ...
            if ItemTrackingCode."CD Specific Tracking" then
                SetRange("CD No.", FromItemLedgEntry."CD No.");
    ...
    begin
        ...
        if ItemTrackingCode."CD Specific Tracking" then
            OldItemLedgEntry.TestField("CD No.", ItemLedgEntry."CD No.");
        if ItemLedgEntry."Drop Shipment" and (OldItemLedgEntry."CD No." <> '') then
            OldItemLedgEntry.TestField("CD No.", ItemLedgEntry."CD No.");
    ...
    begin
        with ItemJnlLine do begin
            ...
            OnInsertTransferEntryOnTransferValues(NewItemLedgEntry, OldItemLedgEntry, ItemLedgEntry, ItemJnlLine, TempItemEntryRelation);
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            ItemLedgEntry."FA No." := "FA No.";
            ItemLedgEntry."Depreciation Book Code" := "Depreciation Book Code";
            if "FA No." <> '' then
                ItemLedgEntry."FA Entry No." := CreateFAEntryFromItemLedgEntry(ItemLedgEntry);
            if "Entry Type" in
               ["Entry Type"::Sale,
            ...
            if ItemLedgEntry.Open then begin
                ...
                then
                    if (ItemLedgEntry.Quantity < 0) and
                       (ItemTrackingCode."SN Specific Tracking" or ItemTrackingCode."Lot Specific Tracking" or
                        ItemTrackingCode."CD Specific Tracking")
                    then
                        Error(Text12418, "Serial No.", "Lot No.", "CD No.", "Item No.", "Variant Code");
    ...
    begin
        IsHandled := false;
        OnBeforeInsertPhysInventoryEntry(ItemJnlLineOrigin, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            ValueEntry.Type := Type;
            ValueEntry."Posting Date" := "Posting Date";
            if ItemLedgEntry."FA No." <> '' then begin
                ValueEntry."FA No." := ItemLedgEntry."FA No.";
                ValueEntry."Depreciation Book Code" := ItemLedgEntry."Depreciation Book Code";
                ValueEntry."FA Entry No." := ModifyFAEntryFromItemLedgEntry(ValueEntry);
            end;
            ...
            if "Value Entry Type" <> "Value Entry Type"::Rounding then begin
                ...
                then begin
                    ...
                    then begin
                        ...
                        if not Adjustment and ("Item Charge No." = '') then
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            if ItemLedgEntry."FA No." <> '' then begin
                ValueEntry."FA No." := ItemLedgEntry."FA No.";
                ValueEntry."Depreciation Book Code" := ItemLedgEntry."Depreciation Book Code";
                ValueEntry."FA Entry No." := ItemLedgEntry."FA Entry No.";
                ModifyFAEntryFromItemLedgEntry(ValueEntry);
            end;
            ...
            OnInsertValueEntryOnBeforeCalcExpectedCost(ItemJnlLine, ItemLedgEntry, ValueEntry);
            ...
            GetInvtSetup;
            if InvtSetup."Enable Red Storno" then
                ValueEntry."Red Storno" := ItemJnlLineOrigin."Red Storno";
    ...
    begin
        with GlobalValueEntry do begin
            ...
            if AverageTransfer then begin
                ValuateAppliedAvgEntry(GlobalValueEntry, Item);
                if CDNoRequired then begin
                    ValueEntry."Cost Amount (Actual)" :=
                      "Cost Amount (Actual)" * ValueEntry."Valued Quantity" / "Valued Quantity";
                    ValueEntry."Cost Amount (Actual) (ACY)" :=
                      "Cost Amount (Actual) (ACY)" * ValueEntry."Valued Quantity" / "Valued Quantity";
                end else begin
    ...
    begin
        ...
        CreateBatchLink(NewItemLedgEntry); // create link for Tax Register
            ...
            if ItemTrackingSetup."CD No. Required" then begin
                ...
                if TrackingSpecification."New CD No." <> '' then begin
                    if CDNoInfo.Get(CDNoInfo.Type::Item, ItemJnlLine2."Item No.", ItemJnlLine2."Variant Code", TrackingSpecification."New CD No.") then begin
                        ...
                        if TrackingSpecification."New CD No." <> '' then
                            CreateCDNoInfo(ItemJnlLine2."Item No.", ItemJnlLine2."Variant Code", TrackingSpecification."New CD No.");
                end;
            end;
        ...
        with InvtAdjmtEntryOrder do
            if not Get(OrderType, OrderNo, OrderLineNo) then
                ...
                if "Allow Online Adjustment" or "Cost is Adjusted" then begin
                    LockTable();
                    IsHandled := false;
                    OnSetOrderAdjmtPropertiesOnBeforeSetCostIsAdjusted(InvtAdjmtEntryOrder, ModifyOrderAdjmt, IsHandled, OriginalPostingDate);
                    if not IsHandled then
                        if "Cost is Adjusted" then begin
                            "Cost is Adjusted" := false;
                    ...
                    IsHandled := false;
                    OnSetOrderAdjmtPropertiesOnBeforeSetAllowOnlineAdjustment(InvtAdjmtEntryOrder, ModifyOrderAdjmt, IsHandled, OriginalPostingDate);
                    if not IsHandled then
                        if "Allow Online Adjustment" then begin
                            "Allow Online Adjustment" := AllowAdjmtOnPosting(OriginalPostingDate);
    ...
    local procedure CreateFAEntryFromItemLedgEntry(var ItemLedgerEntry: Record "Item Ledger Entry"): Integer
    var
    ...
    begin
        ...
        FALedgerEntry.Amount := FALedgerEntry.Amount - ValueEntry2."Cost Posted to G/L";
        ...
        FALedgerEntry."Gen. Bus. Posting Group" := ValueEntry2."Gen. Bus. Posting Group";
        FALedgerEntry."Gen. Prod. Posting Group" := ValueEntry2."Gen. Prod. Posting Group";
        ...
        FALedgerEntry."Value Entry No." := ValueEntry2."Entry No.";
    ...
    local procedure ModifyFAEntryFromItemLedgEntry(var ValueLedgerEntry: Record "Value Entry"): Integer
    var
        FA: Record "Fixed Asset";
        FALedgerEntry: Record "FA Ledger Entry";
        LastLine: Integer;
        FADepreciationBook: Record "FA Depreciation Book";
        ItemLedgerEntry: Record "Item Ledger Entry";
        ValueLedgerEntry2: Record "Value Entry";
        AmountTotal: Decimal;
    begin
        AmountTotal := 0;

        if ItemLedgerEntry.Get(ValueLedgerEntry."Item Ledger Entry No.") then begin
            if ValueLedgerEntry."FA No." <> '' then begin
                if FALedgerEntry.Get(ValueLedgerEntry."FA Entry No.") then begin
                    if FALedgerEntry."Value Entry No." = 0 then
                        FALedgerEntry."Value Entry No." := ValueLedgerEntry."Entry No."
                    else
        ...
        exit(ItemLedgerEntry."FA Entry No.");
    end;

    [Scope('OnPrem')]
    procedure GetValueEntryPostingDate(ValueEntryLoc: Record "Value Entry"; PostingDate: Date) PostDate: Date
    var
        ItemLedgEntryLoc: Record "Item Ledger Entry";
        ItemApplnEntryLoc: Record "Item Application Entry";
        CapLedgEntryLoc: Record "Capacity Ledger Entry";
        GenJnlCheckLine: Codeunit "Gen. Jnl.-Check Line";
        Text12404: Label 'ENU=is not within your range of allowed posting dates';
    begin
        GetInvtSetup;
        PostDate := PostingDate;
        if InvtSetup."Automatic Posting Date Adjmt." > 0 then begin
            if (ValueEntryLoc."Item Charge No." <> '') and
              (InvtSetup."Automatic Posting Date Adjmt." = InvtSetup."Automatic Posting Date Adjmt."::"Item Cost Only")
            then
                exit(PostDate);
            if (ValueEntryLoc."Item Ledger Entry No." <> 0) then begin
                if ItemLedgEntryLoc.Get(ValueEntryLoc."Item Ledger Entry No.") then begin
                    ...
                    if ValueEntryLoc."Entry Type" = ValueEntryLoc."Entry Type"::Rounding then begin
                        ...
                        if ItemApplnEntryLoc.Find('+') then
                            PostDate := ItemApplnEntryLoc."Posting Date";
                    end;
                end;
            end else
                if (ValueEntryLoc."Capacity Ledger Entry No." <> 0) then begin
                    if CapLedgEntryLoc.Get(ValueEntryLoc."Capacity Ledger Entry No.") then begin
                        PostDate := CapLedgEntryLoc."Posting Date";
                    end;
                end;
            if GenJnlCheckLine.DateNotAllowed(PostDate) then
                ValueEntryLoc.FieldError("Posting Date", Text12404);
        end;
        exit(PostDate);
    end;

    [Scope('OnPrem')]
    procedure CheckApplicationDate(OldItemLedgEntry: Record "Item Ledger Entry"; ItemLedgEntry: Record "Item Ledger Entry")
    begin
        GetInvtSetup;
        if InvtSetup."Check Application Date" then begin
            if (OldItemLedgEntry.Positive) and
               (OldItemLedgEntry."Posting Date" > ItemLedgEntry."Posting Date")
            then
                Error(Text12400, OldItemLedgEntry.FieldCaption("Posting Date"), ItemLedgEntry."Posting Date",
                    OldItemLedgEntry.TableCaption, OldItemLedgEntry.FieldCaption("Entry No."),
                    OldItemLedgEntry."Entry No.");
            if (not OldItemLedgEntry.Positive) and
               (OldItemLedgEntry."Posting Date" < ItemLedgEntry."Posting Date")
            then
                Error(Text12400, ItemLedgEntry.FieldCaption("Posting Date"), OldItemLedgEntry."Posting Date",
                    ItemLedgEntry.TableCaption, ItemLedgEntry.FieldCaption("Entry No."),
                    ItemLedgEntry."Entry No.");
        end;
    end;

    local procedure CreateBatchLink(ItemLedgEntry0: Record "Item Ledger Entry")
    var
        ItemApplnEntry0: Record "Item Application Entry";
        EntryNo: Integer;
    begin
        if ItemLedgEntry0."Entry Type" = ItemLedgEntry0."Entry Type"::Transfer then
            exit;

        ItemApplnEntry0.SetCurrentKey("Item Ledger Entry No.");
        ItemApplnEntry0.SetRange("Item Ledger Entry No.", ItemLedgEntry0."Entry No.");
        if not ItemApplnEntry0.Find('-') then
            exit;

        repeat
            if ItemLedgEntry0.Quantity >= 0 then
                EntryNo := ItemLedgEntry0."Entry No."
            else
    ...
    local procedure SearchBatchLink(ItemApplnEntry0: Record "Item Application Entry"): Integer
    var
        ItemLedgEntry1: Record "Item Ledger Entry";
        ItemApplnEntry1: Record "Item Application Entry";
    begin
        if not ItemLedgEntry1.Get(ItemApplnEntry0."Inbound Item Entry No.") then
            exit(0);
        if ItemLedgEntry1.Quantity < 0 then
            exit(0);
        ItemApplnEntry1.SetCurrentKey("Item Ledger Entry No.");
        while ItemLedgEntry1."Entry Type" = ItemLedgEntry1."Entry Type"::Transfer do begin
            ItemApplnEntry1.SetRange("Item Ledger Entry No.", ItemLedgEntry1."Entry No.");
            ItemApplnEntry1.Find('-');
            if not ItemLedgEntry1.Get(ItemApplnEntry1."Transferred-from Entry No.") then
                exit(0);
            if ItemLedgEntry1.Quantity < 0 then
                exit(0);
        end;
        exit(ItemLedgEntry1."Entry No.");
    end;

    [Scope('OnPrem')]
    procedure EnableRedStorno()
    var
        GLEntry: Record "G/L Entry";
        GLItemLedgRelation: Record "G/L - Item Ledger Relation";
        ValueEntry: Record "Value Entry";
        Window: Dialog;
        ProgressCounter: Decimal;
        TempVar: Decimal;
        UnpostedValueEntryNo: Integer;
    begin
        UnpostedValueEntryNo := AreAllEntriesPostedToGL(false);
        if UnpostedValueEntryNo <> -1 then
        ...
        with GLEntry do
            if GLEntry.FindSet then begin
                ...
                repeat
                    ...
                    ProgressCounter := ProgressCounter + TempVar;
                    Window.Update(1, Format(GLEntry."Entry No."));
                    Window.Update(2, Round(ProgressCounter, 1));
    ...
    [Scope('OnPrem')]
    procedure DisableRedStorno()
    var
        ValueEntry: Record "Value Entry";
        LocGenJnlLine: Record "Gen. Journal Line";
        LocItemJnlLine: Record "Item Journal Line";
        UnpostedValueEntryNo: Integer;
    begin
        UnpostedValueEntryNo := AreAllEntriesPostedToGL(true);
        if UnpostedValueEntryNo <> -1 then
            Error(Text12411, ValueEntry.TableCaption, ValueEntry.FieldCaption("Entry No."),
              UnpostedValueEntryNo);
    ...
    [Scope('OnPrem')]
    procedure AreAllEntriesPostedToGL(RedStorno: Boolean) UnPostedValueEntry: Integer
    var
        ValueEntry: Record "Value Entry";
    begin
        ValueEntry.Reset();
    ...
    [Scope('OnPrem')]
    procedure CreateCDNoInfo(ItemNo: Code[20]; VariantCode: Code[20]; CDNo: Code[30])
    var
        CDNoInfo: Record "CD No. Information";
    begin
        CDNoInfo.Init();
    ...
    begin
        InventoryPostingToGL.SetPreviewMode(PreviewMode);
        if InventoryPostingToGL.BufferInvtPosting(ValueEntry) then
            InventoryPostingToGL.PostInvtPostBufPerEntry(ValueEntry);
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertConsumpEntry(var ProdOrderComponent: Record "Prod. Order Component"; QtyBase: Decimal; var ModifyProdOrderComp: Boolean; var ItemJnlLine: Record "Item Journal Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemOnAfterGetSKU(var ItemJnlLine: Record "Item Journal Line"; var SKUExists: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnInsertTransferEntryOnBeforeInitTransValueEntry(var TempItemEntryRelation: Record "Item Entry Relation"; NewItemLedgEntry: Record "Item Ledger Entry"; Item: Record Item)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCodeOnBeforeCheckItemTracking(var ItemJnlLine: Record "Item Journal Line"; DisableItemTracking: Boolean; var IsHandled: Boolean)
    begin
    end;
}
