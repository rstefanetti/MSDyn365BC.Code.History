page 1292 "Payment Application"
{
    ...
    layout
    {
        area(content)
        {
            group(PaymentInformation)
            {
                ...
                field(VariableSymbol; BankAccReconLine."Variable Symbol")
                {
                    ApplicationArea = Basic, Suite;
    ...
    begin
        ...
        if WithSetup then begin
            GetBankStmtLineCandidates.SetWithSetup(true);
    ...
    begin
        if ("Match Confidence" = "Match Confidence"::Accepted) or ("Match Confidence" = "Match Confidence"::Manual) then
            "Match Confidence" := BankPmtApplRule.GetMatchConfidence(Quality, false); // NAVCZ
    end;
