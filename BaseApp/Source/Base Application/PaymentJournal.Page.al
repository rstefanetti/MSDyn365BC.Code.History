page 256 "Payment Journal"
{
    ...
    DelayedInsert = true;
    PageType = Worksheet;
    PromotedActionCategories = 'New,Process,Report,Bank,Prepare,Approve,Page,Post/Print,Line,Account,Check,DTA';
    SaveValues = true;
        ...
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                field("Currency Code"; "Currency Code")
                {
                    ...
                    trigger OnAssistEdit()
                    begin
                        if "Source Code" = 'DTA' then
                            if not (("Applies-to Doc. No." <> '') or
                               (("Currency Code" <> '') and
                               ("Currency Code" = CopyStr(Description, (StrLen(Description) - 3), 3))))
                            then
                                Error(Text90600);
                        ...
                        if DtaSetup.ReadPermission and ("Currency Factor" <> xRec."Currency Factor") then begin
        ...
        area(navigation)
        {
            group("&DTA")
            {
                ...
                action("Generate &DTA File")
                {
                    ...
                    Promoted = true;
                    PromotedCategory = Category12;
                    ToolTip = 'Create a file to prepare to submit DatenTrÔÇ×gerAustausch (DTA) payments to your bank for payment.';
