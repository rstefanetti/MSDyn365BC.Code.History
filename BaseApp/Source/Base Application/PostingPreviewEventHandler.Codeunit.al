codeunit 20 "Posting Preview Event Handler"
{
    ...
    [EventSubscriber(ObjectType::Table, Database::"G/L Correspondence Entry", 'OnAfterInsertEvent', '', false, false)]
    local procedure OnInsertGLCorrespondenceEntry(var Rec: Record "G/L Correspondence Entry"; RunTrigger: Boolean)
    begin
        if Rec.IsTemporary() then
            exit;

        TempGLCorrespondenceEntry := Rec;
        TempGLCorrespondenceEntry."Document No." := '***';
