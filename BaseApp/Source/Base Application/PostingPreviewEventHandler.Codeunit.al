codeunit 20 "Posting Preview Event Handler"
{
    ...
    [EventSubscriber(ObjectType::Table, 12401, 'OnAfterInsertEvent', '', false, false)]
    [Scope('OnPrem')]
    procedure OnInsertGLCorrespondenceEntry(var Rec: Record "G/L Correspondence Entry"; RunTrigger: Boolean)
    begin
        if Rec.IsTemporary then
            exit;
