table 122 "Purch. Inv. Header"
{
    ...
    fields
    {
        ...
        field(12402; "Remaining/Overdraft Doc. No."; Code[20])
        {
            ...
            TableRelation = "Vendor Ledger Entry"."Document No." WHERE("Vendor No." = FIELD("Pay-to Vendor No."));
        }
    ...
    [Scope('OnPrem')]
    procedure FindReceipts(PurchInvHeader: Record "Purch. Inv. Header")
    var
    ...
    begin
        ...
        if PurchInvLine.Find('-') then
            repeat
                ...
                ValueEntry.SetRange("Item No.", PurchInvLine."No.");
    ...
    [Scope('OnPrem')]
    procedure ChangeVATInvoice(PurchInvHeader: Record "Purch. Inv. Header")
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        VendLedgEntry.Reset();
        ...
        if VendLedgEntry.Find('-') then
            repeat
                VendLedgEntry."Vendor VAT Invoice No." := PurchInvHeader."Vendor VAT Invoice No.";
                VendLedgEntry."Vendor VAT Invoice Date" := PurchInvHeader."Vendor VAT Invoice Date";
