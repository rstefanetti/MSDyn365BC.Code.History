codeunit 884 "ReadSoft OCR Master Data Sync"
{
    ...
    begin
        ...
        if not TryLoadXml(AdjustedBody, XmlDoc) then begin
            ClearLastError();
            SendTraceTag('0000DOD', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(CannotLoadXmlDocumentTxt, RequestType, ResponseBody), DataClassification::CustomerContent);
            exit(false);
        end;
        if not XmlDoc.GetRoot(RootXmlElement) then begin
            SendTraceTag('0000DOE', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(CannotGetRootNodeTxt, RequestType, ResponseBody), DataClassification::CustomerContent);
            exit(false);
        end;
        XmlNodeList := RootXmlElement.GetChildNodes();
        Count := XmlNodeList.Count();
        if Count = 0 then begin
            SendTraceTag('0000DOF', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(CannotFindChildNodesTxt, RequestType, ResponseBody), DataClassification::CustomerContent);
            exit(false);
        end;
        if Count > 1 then
            SendTraceTag('0000DOG', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(ChildNodeCountTxt, RequestType, Count, ResponseBody), DataClassification::CustomerContent);
    ...
    begin
        ...
        if ElementCount = 0 then
            SendTraceTag('0000DOI', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(UpdateResultCannotParseTxt, RequestType, Number, ResponseBody), DataClassification::CustomerContent);
    ...
    begin
        ...
        if (ErrorCode <> '') or (ErrorMessage <> '') then
            SendTraceTag('0000DOL', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(ServiceErrorDetailsTxt, RequestType, Number, ErrorCode, ErrorMessage), DataClassification::CustomerContent)
        else
            SendTraceTag('0000DOM', TelemetryCategoryTok, Verbosity::Normal, StrSubstNo(ServiceErrorCannotParseTxt, RequestType, Number, ResponseBody), DataClassification::CustomerContent);
