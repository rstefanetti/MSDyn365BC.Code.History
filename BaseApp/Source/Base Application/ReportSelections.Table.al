table 77 "Report Selections"
{
    ...
    //           PARO = P.Arch.Return
    //           // RU
    //           USI = Unposted Sales Invoice
    //           USCM = Unposted Sales Credit Memo
    //           UCI = Unposted Cash In Order
    //           UCO = Unposted Cash Out Order
    //           CB = Cash Book
    //           CI = Cash In Order
    //           CO = Cash Out Order
    //           UAS = Unposted Advance Statement
    //           AS = Advance Statement
    //           UIS = Unposted Item Shipment
    //           UIR = Unposted Item Receipt
    //           IS = Item Shipment
    //           IR = Item Receipt
    //           UFAW = Unposted FA Writeoff
    //           UFAR = Unposted FA Release
    //           UFAM = Unposted FA Movement
    //           FAW = FA Writeoff
    //           FAR = FA Release
    //           FAM = FA Movement
    //           FAJ = FA Journal
    //           FARJ = FA Reclassification Journal
    //           PIJ = Phys. Inventory Journal
    //           UPI = Unposted Purchase Invoice
    //           IRJ = Item Reclassification Journal
    //           UPCM = Unposted Purchase Credit Memo
    //           UCSD = Unposted Corrective Sales Document
    //           CSI = Corrective Sales Invoice
    //           CSCM = Corrective Sales Credit Memo
    ...
    fields
    {
        ...
        field(12410; Default; Boolean)
        {
            Caption = 'Default';
        }
        field(12411; "Excel Export"; Boolean)
        {
    ...
    var
        DummyDocumentPrintBuffer: Record "Document Print Buffer";
        Handled: Boolean;
    begin
        ...
        PrintDocumentsWithCheckGUIYesNoCommon(
    ...
    begin
        ...
        PrintDocumentsWithCheckGUIYesNoCommon(
          ReportUsage, RecordVariant, IsGUI, VendorNoFieldNo, true, DATABASE::Vendor, DummyDocumentPrintBuffer);
    end;
    ...
    [Scope('OnPrem')]
    procedure PrintWithDocPrintOption(ReportUsage: Integer; RecordVariant: Variant; CustomerFieldNo: Integer; ShowRequestPage: Boolean; TableNo: Integer; var DocPrintBuffer: Record "Document Print Buffer")
    begin
        PrintDocumentsWithCheckGUIYesNoCommon(ReportUsage, RecordVariant, ShowRequestPage, CustomerFieldNo, false, TableNo, DocPrintBuffer);
    end;
    ...
    begin
        ...
        PrintDocumentsWithCheckGUIYesNoCommon(
          ReportUsage, RecordVariant, IsGUI, CustomerNoFieldNo, false, DATABASE::Customer, DummyDocumentPrintBuffer);
    end;
    ...
    begin
        ...
        PrintDocumentsWithCheckGUIYesNoCommon(
          ReportUsage, RecordVariant, IsGUI, VendorNoFieldNo, false, DATABASE::Vendor, DummyDocumentPrintBuffer);
    end;

    local procedure PrintDocumentsWithCheckGUIYesNoCommon(ReportUsage: Integer; RecordVariant: Variant; IsGUI: Boolean; AccountNoFieldNo: Integer; WithCheck: Boolean; TableNo: Integer; var DocumentPrintBuffer: Record "Document Print Buffer")
    var
    ...
    begin
        ...
        if TempReportSelections.Count > 1 then begin
            ...
            if PAGE.RunModal(PAGE::"Report Selection - Print", TempReportSelections) <> ACTION::LookupOK then
                TempReportSelections.DeleteAll;

            TempReportSelections.SetRange(Default, true);
            TempReportSelections.FindSet;
        end;
    end;

    local procedure PrintTempReportSelection(var TempReportSelections: Record "Report Selections" temporary; IsGUI: Boolean; RecordVariant: Variant; var DocumentPrintBuffer: Record "Document Print Buffer")
    begin
        if not TempReportSelections."Excel Export" then
            REPORT.RunModal(TempReportSelections."Report ID", IsGUI, false, RecordVariant)
        else
            REPORT.RunModal(TempReportSelections."Report ID", IsGUI, false, DocumentPrintBuffer);
    end;
    ...
    begin
        ...
        if CustomReportSelection.FindSet then
            repeat
                ...
                ToReportSelections.Default := true;
