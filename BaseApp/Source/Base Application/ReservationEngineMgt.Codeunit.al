codeunit 99000831 "Reservation Engine Mgt."
{
    ...
    local procedure ItemTrackingMismatch(ReservEntry: Record "Reservation Entry"; NewSerialNo: Code[50]; NewLotNo: Code[50]; NewCDNo: Code[30]): Boolean
    var
        ReservEntry2: Record "Reservation Entry";
    begin
        if (NewLotNo = '') and (NewSerialNo = '') and (NewCDNo = '') then
            exit(false);
        ...
        if (ReservEntry2."CD No." <> '') and (NewCDNo <> '') then
            if (ReservEntry2."CD No." <> NewCDNo) then
                exit(true);
        ...
        if TempReservEntry2.Binding = TempReservEntry2.Binding::"Order-to-Order" then begin
            ...
            if TempReservEntry2.Positive and
               ((TempReservEntry2."Serial No." <> TrackingSpecification2."Serial No.") or
                (TempReservEntry2."Lot No." <> TrackingSpecification2."Lot No.") or
                (TempReservEntry2."CD No." <> TrackingSpecification2."CD No."))
            then
                Error(Text12408,
