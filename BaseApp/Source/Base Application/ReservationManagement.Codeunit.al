codeunit 99000845 "Reservation Management"
{
    ...
    var
        ...
        ForItemDocLine: Record "Item Document Line";
        ...
        Text12450: Label 'Receipt,Shipment,Outbound Transfer,Inbound Transfer';
        Text12409: Label 'Item Tracking cannot be fully matched.\Serial No.: %1, Lot No.: %2, CD No. %3, outstanding quantity: %4.';
    ...
    [Scope('OnPrem')]
    procedure ItemDocLineUpdateValues(var CurrentItemDocLine: Record "Item Document Line"; var QtyToReserve: Decimal; var QtyToReserveBase: Decimal; var QtyReservedThisLine: Decimal; var QtyReservedThisLineBase: Decimal)
    begin
        with CurrentItemDocLine do begin
            case "Document Type" of
                1: // Shipment
                    begin
                        ...
                        QtyReservedThisLine := "Reserved Quantity Outbnd.";
                        QtyReservedThisLineBase := "Reserved Qty. Outbnd. (Base)";
                        QtyToReserve := -Quantity + "Reserved Quantity Outbnd.";
                        QtyToReserveBase := -"Quantity (Base)" + "Reserved Qty. Outbnd. (Base)";
                    end;
                0: // Receipt
                    begin
                        ...
                        QtyReservedThisLine := "Reserved Quantity Inbnd.";
                        QtyReservedThisLineBase := "Reserved Qty. Inbnd. (Base)";
                        QtyToReserve := Quantity - "Reserved Quantity Inbnd.";
                        QtyToReserveBase := "Quantity (Base)" - "Reserved Qty. Inbnd. (Base)";
                    end;
            end;
        end;
    end;
        ...
        for i := 1 to SetValueArray(ValueArrayNo) do begin
            ...
            case ValueArray[i] of
                ...
                12450, 12451: // Item Document Line
                    UpdateItemDocLineStats(ReservSummEntry, AvailabilityDate, i, CalcSumValue);
                6500: // Item Tracking
    ...
    begin
        ...
        if ReservEntry.FindSet then
            repeat
                ...
                ReservEntrySummary."CD No." := ReservEntry."CD No.";
                if ReservEntry.FindSet then
                    repeat
    ...
    local procedure UpdateItemDocLineStats(var ReservEntrySummary: Record "Entry Summary"; AvailabilityDate: Date; i: Integer; var CalcSumValue: Decimal)
    begin
        if CalcItemDocLine.ReadPermission then begin
            InitFilter(ValueArray[i], AvailabilityDate);
            if CalcItemDocLine.FindSet then
                repeat
                    case ValueArray[i] of
                        12450:
                            begin
                                CalcItemDocLine.CalcFields("Reserved Qty. Outbnd. (Base)");
                                ReservEntrySummary."Total Reserved Quantity" -= CalcItemDocLine."Reserved Qty. Outbnd. (Base)";
                                CalcSumValue -= CalcItemDocLine."Quantity (Base)";
                            end;
                        12451:
                            begin
                                CalcItemDocLine.CalcFields("Reserved Qty. Inbnd. (Base)");
                                ReservEntrySummary."Total Reserved Quantity" += CalcItemDocLine."Reserved Qty. Inbnd. (Base)";
                                CalcSumValue += CalcItemDocLine."Quantity (Base)";
                            end;
                    end;
                until CalcItemDocLine.Next = 0;
            ...
            if CalcSumValue <> 0 then
                with ReservEntrySummary do
                    if (CalcSumValue > 0) = Positive then begin
                        ...
                        "Summary Type" :=
                          CopyStr(
                            StrSubstNo('%1, %2', CalcItemDocLine.TableCaption, SelectStr(ValueArray[i] - 100, Text006)),
                            1, MaxStrLen("Summary Type"));
                        "Total Quantity" := CalcSumValue;
                        "Total Available Quantity" := "Total Quantity" - "Total Reserved Quantity";
                        if not Insert then
          ...
          102: // Transfer
            ...
            152: // Assembly Line
                AutoReserveAssemblyLine(
                  ReservSummEntryNo, RemainingQtyToReserve, RemainingQtyToReserveBase, Description, AvailabilityDate, Search, NextStep);
            12450,
            12451: // Item Document Line
                AutoReserveItemDocLine(
                  ReservSummEntryNo, RemainingQtyToReserve, RemainingQtyToReserveBase, Description, AvailabilityDate, Search, NextStep);
            else
            ...
            repeat
                ...
                then begin
                    ...
                    CallTrackingSpecification.InitTrackingSpecification(
                      ...
                      CalcItemLedgEntry."Serial No.", CalcItemLedgEntry."Lot No.", CalcItemLedgEntry."CD No.",
        ...
        if CalcPurchLine.Find(Search) then
            repeat
                ...
                CallTrackingSpecification.InitTrackingSpecification(
                  ...
                  CalcReservEntry."Serial No.", CalcReservEntry."Lot No.", CalcReservEntry."CD No.",
    ...
    local procedure AutoReserveItemDocLine(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; Search: Text[1]; NextStep: Integer)
    var
        QtyThisLine: Decimal;
        QtyThisLineBase: Decimal;
        ReservQty: Decimal;
    ...
    begin
        InitFilter(ReservSummEntryNo, AvailabilityDate);
        if CalcItemDocLine.Find(Search) then
            repeat
                case ReservSummEntryNo of
                    DATABASE::"Item Receipt Header": // Outbound
                        begin
                            CalcItemDocLine.CalcFields("Reserved Qty. Outbnd. (Base)");
                            QtyThisLine := -CalcItemDocLine.Quantity;
                            QtyThisLineBase := -CalcItemDocLine."Quantity (Base)";
                            ReservQty := -CalcItemDocLine."Reserved Qty. Outbnd. (Base)";
                            EntryDate := CalcItemDocLine."Posting Date";
                            LocationCode := CalcItemDocLine."Location Code";
                            if Positive = (QtyThisLine < 0) then begin
                                QtyThisLine := 0;
                                QtyThisLineBase := 0;
                            end;
                        end;
                    DATABASE::"Item Document Header": // Inbound
                        begin
                            CalcItemDocLine.CalcFields("Reserved Qty. Inbnd. (Base)");
                            QtyThisLine := CalcItemDocLine.Quantity;
                            QtyThisLineBase := CalcItemDocLine."Quantity (Base)";
                            ReservQty := CalcItemDocLine."Reserved Qty. Inbnd. (Base)";
                            EntryDate := CalcItemDocLine."Posting Date";
                            LocationCode := CalcItemDocLine."Location Code";
                            if Positive = (QtyThisLine < 0) then begin
                                QtyThisLine := 0;
                ...
                if QtyThisLine <> 0 then begin
                    if Abs(QtyThisLine - ReservQty) > 0 then begin
                        ...
                        CallTrackingSpecification.InitTrackingSpecification(
                          DATABASE::"Item Document Line", ReservSummEntryNo - DATABASE::"Item Document Header", CalcItemDocLine."Document No.", '',
                          0, CalcItemDocLine."Line No.",
                          CalcItemDocLine."Variant Code", LocationCode,
                          CalcReservEntry."Serial No.", CalcReservEntry."Lot No.", CalcReservEntry."CD No.",
                          CalcItemDocLine."Qty. per Unit of Measure");
            ...
            until (CalcItemDocLine.Next(NextStep) = 0) or (RemainingQtyToReserve = 0);
    end;
    ...
    begin
        ...
        case CalcReservEntry."Source Type" of
            ...
            DATABASE::"Item Document Line":
                begin
                    ReserveItemDocLine.CreateReservationSetFrom(TrackingSpecification);
                    ReserveItemDocLine.CreateReservation(
                      ForItemDocLine, Description, ExpectedDate, Quantity, QuantityBase,
                      CalcReservEntry."Serial No.", CalcReservEntry."Lot No.", CalcReservEntry."CD No.");
                    ForItemDocLine.CalcFields("Reserved Qty. Outbnd. (Base)");
                    ForItemDocLine.CalcFields("Reserved Qty. Inbnd. (Base)");
                end;
        end;
    end;
    ...
    begin
        ...
        if ItemTrackingCode."SN Specific Tracking" or ItemTrackingCode."Lot Specific Tracking" or
           ItemTrackingCode."CD Specific Tracking" or
           (CurrentSerialNo <> '') or (CurrentLotNo <> '') or (CurrentCDNo <> '')
        then begin
            ...
            case ItemTrackingHandling of
                ...
                ItemTrackingHandling::Match:
                    begin
                        if not ((CurrentSerialNo = '') and (CurrentLotNo = '') and (CurrentCDNo = '')) then begin
                            ...
                            ReservEntry.SetTrackingFilter(CurrentSerialNo, CurrentLotNo, CurrentCDNo);
                        end else
            ...
            DATABASE::"Item Document Line":
                begin
                    case CalcReservEntry."Source Subtype" of
                        1: // Shipment
                            begin
                                ForItemDocLine.CalcFields("Reserved Quantity Outbnd.", "Reserved Qty. Outbnd. (Base)");
                                RemainingQty := ForItemDocLine.Quantity - Abs(ForItemDocLine."Reserved Quantity Outbnd.");
                                RemainingQtyBase := ForItemDocLine."Quantity (Base)" - Abs(ForItemDocLine."Reserved Qty. Outbnd. (Base)");
                            end;
                        0: // Receipt
                            begin
                                ForItemDocLine.CalcFields("Reserved Quantity Inbnd.", "Reserved Qty. Inbnd. (Base)");
                                RemainingQty := ForItemDocLine.Quantity - Abs(ForItemDocLine."Reserved Quantity Inbnd.");
                                RemainingQtyBase := ForItemDocLine."Quantity (Base)" - Abs(ForItemDocLine."Reserved Qty. Inbnd. (Base)");
                            end;
                    end;
                end;
            else
    ...
    local procedure FieldFilterNeeded(var ReservEntry: Record "Reservation Entry"; "Field": Option "Lot No.","Serial No.","CD No."): Boolean
    begin
        exit(FieldFilterNeeded(ReservEntry, Positive, Field));
    end;

    procedure FieldFilterNeeded(var ReservEntry: Record "Reservation Entry"; SearchForSupply: Boolean; "Field": Option "Lot No.","Serial No.","CD No."): Boolean
    var
        ...
        if FieldValue <> '' then begin
            if Field = 2 then begin
                if SearchForSupply then // Demand
                    ReservEntry2.SetRange("CD No.", FieldValue)
                else // Supply
                    ReservEntry2.SetFilter("CD No.", '%1|%2', FieldValue, '');
                FieldFilter := ReservEntry2.GetFilter("CD No.");
            end else begin
                if SearchForSupply then // Demand
        ...
        case EntryID of
            1:
                begin // Item Ledger Entry
                    ...
                    if FieldFilterNeeded(CalcReservEntry, Positive, 2) then
                        CalcItemLedgEntry.SetFilter("CD No.", GetFieldFilter);
                end;
            12, 16:
                begin // Purchase Line
                    CalcPurchLine.Reset;
            ...
            12450:
                begin // Item Receipt
                    CalcItemDocLine.Reset;
                    CalcItemDocLine.SetCurrentKey("Location Code");
                    CalcItemDocLine.SetRange("Item No.", CalcReservEntry."Item No.");
                    CalcItemDocLine.SetRange("Variant Code", CalcReservEntry."Variant Code");
                    CalcItemDocLine.SetRange("Location Code", CalcReservEntry."Location Code");
                    CalcItemDocLine.SetRange("Document Type", CalcItemDocLine."Document Type"::Receipt);
                    CalcItemDocLine.SetFilter("Document Date", GetAvailabilityFilter(AvailabilityDate));
                    if Positive then
                        CalcItemDocLine.SetFilter("Quantity (Base)", '>0')
                    else
                        CalcItemDocLine.SetFilter("Quantity (Base)", '<0');
                end;
            12451:
                begin // Item Shipment
                    ...
                    CalcItemDocLine.SetRange("Variant Code", CalcReservEntry."Variant Code");
                    CalcItemDocLine.SetRange("Location Code", CalcReservEntry."Location Code");
                    CalcItemDocLine.SetRange("Document Type", CalcItemDocLine."Document Type"::Shipment);
                    CalcItemDocLine.SetFilter("Document Date", GetAvailabilityFilter(AvailabilityDate));
                    if Positive then
                        CalcItemDocLine.SetFilter("Quantity (Base)", '<0')
                    else
    ...
    begin
        ...
        ReservEntry.CopyFilter("CD No.", ReservEntry2."CD No.");
    ...
    begin
        ...
        if FieldFilterNeeded(ReservEntry, SearchForSupply, 2) then
            SurplusEntry.SetFilter("CD No.", GetFieldFilter);
    ...
    begin
        ...
        CreateReservEntry.CreateReservEntryFor(
          ...
          0, sign * Quantity,
          FromReservEntry."Serial No.", FromReservEntry."Lot No.", FromReservEntry."CD No.");
        CreateReservEntry.CreateReservEntryFrom(
          ...
          ToReservEntry."Serial No.", ToReservEntry."Lot No.", ToReservEntry."CD No.");
    ...
    begin
        with ReservEntry do
            case "Source Type" of
                ...
                DATABASE::"Item Document Line":
                    exit(GetSourceItemDocLineValue(ReservEntry, SetAsCurrent, ReturnOption));
                else begin
    ...
    local procedure GetSourceItemDocLineValue(var ReservEntry: Record "Reservation Entry"; SetAsCurrent: Boolean; ReturnOption: Option "Net Qty. (Base)","Gross Qty. (Base)"): Decimal
    var
                ...
                SurplusEntry.Binding::" ":
                    begin
                        if UseGlobalSettings then begin
                            ...
                            if FieldFilterNeeded(SurplusEntry, 2) then
                                ReservEntry.SetFilter("CD No.", GetFieldFilter);
    ...
    begin
        ...
        if FieldFilterNeeded(ReservEntry, SearchForSupply, 2) then
            FilterReservEntry.SetFilter("CD No.", GetFieldFilter);
        ...
        with CalcReservEntry do begin
            ...
            if "CD No." <> '' then begin
                ...
                if WhseActivLine.FindSet then
                    repeat
                        CalcQty := CalcQty + WhseActivLine."Qty. Outstanding (Base)";
                    until WhseActivLine.Next = 0;
                WhseActivLine."Qty. Outstanding (Base)" := CalcQty;
            end else
            ...
            if Location."Require Pick" then begin
                ...
                QtyOnAssemblyBin :=
                  WhseAvailMgt.CalcQtyOnBin(
                    "Location Code", Location."To-Assembly Bin Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", "CD No.");

                QtyOnOpenShopFloorBin :=
                  WhseAvailMgt.CalcQtyOnBin(
                    "Location Code", Location."Open Shop Floor Bin Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", "CD No.");

                QtyOnToProductionBin :=
                  WhseAvailMgt.CalcQtyOnBin(
                    "Location Code", Location."To-Production Bin Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", "CD No.");
            end;
    ...
    begin
        ItemTrackingMgt.CheckWhseItemTrkgSetup(ItemNo, LocationCode, WhseSNRequired, WhseLNRequired, WhseCDRequired, false);
        if not (WhseSNRequired or WhseLNRequired or WhseCDRequired) or (BinCode = '') then
            exit(true);
            ...
            repeat
                if ReservEntryPositiveTypeIsItemLedgerEntry(ReservationEntry."Entry No.") then begin
                    ...
                    if ReservationEntry."CD No." <> '' then
                        WhseEntry.SetRange("CD No.", ReservationEntry."CD No.");
    ...
    begin
        ClearAll;
        ForItemDocLine := NewItemDocLine;

        CalcReservEntry.SetSource(
          DATABASE::"Item Document Line", NewItemDocLine."Document Type", NewItemDocLine."Document No.",
          NewItemDocLine."Line No.", '', 0);

        CalcReservEntry.SetItemData(
          NewItemDocLine."Item No.", NewItemDocLine.Description, NewItemDocLine."Location Code", NewItemDocLine."Variant Code",
          NewItemDocLine."Qty. per Unit of Measure");
        CalcReservEntry."Expected Receipt Date" := NewItemDocLine."Document Date";
        CalcReservEntry."Shipment Date" := NewItemDocLine."Document Date";

        UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * ForItemDocLine."Quantity (Base)") < 0);
    end;
