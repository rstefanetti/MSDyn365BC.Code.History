table 36 "Sales Header"
{
    ...
    fields
    {
        ...
        field(12441; "Corrected Doc. Type"; Option)
        {
            ...
            trigger OnValidate()
            begin
                if "Corrected Doc. Type" <> "Corrected Doc. Type"::" " then
        ...
        field(12442; "Corrected Doc. No."; Code[20])
        {
            ...
            TableRelation = IF ("Corrected Doc. Type" = CONST(Invoice)) "Sales Invoice Header" WHERE("Sell-to Customer No." = FIELD("Sell-to Customer No."))
            ELSE
            IF ("Corrected Doc. Type" = CONST("Credit Memo")) "Sales Cr.Memo Header" WHERE("Sell-to Customer No." = FIELD("Sell-to Customer No."));
            ...
            begin
                if ("Corrected Doc. No." <> xRec."Corrected Doc. No.") then begin
                    ...
                    if "Corrected Doc. No." = '' then begin
                        ...
                        CorrDocMgt.CheckSalesCorrDocHeader(Rec);
                        case "Corrected Doc. Type" of
                            "Corrected Doc. Type"::Invoice:
                                begin
                                    SalesInvHeader.Get("Corrected Doc. No.");
                                    if ("Posting Date" <> 0D) and ("Posting Date" < SalesInvHeader."Posting Date") then
                                        Error(Text12405, "Posting Date", SalesInvHeader."Posting Date");
                                    if SalesInvHeader."Corrective Document" then begin
                                        "Original Doc. Type" := SalesInvHeader."Original Doc. Type";
                                        "Original Doc. No." := SalesInvHeader."Original Doc. No.";
                                    end else begin
                            ...
                            "Corrected Doc. Type"::"Credit Memo":
                                begin
                                    ...
                                    if ("Posting Date" <> 0D) and ("Posting Date" < SalesCrMemoHeader."Posting Date") then
                                        Error(Text12405, "Posting Date", SalesCrMemoHeader."Posting Date");
                                    if SalesCrMemoHeader."Corrective Document" then begin
                                        "Original Doc. Type" := SalesCrMemoHeader."Original Doc. Type";
                                        "Original Doc. No." := SalesCrMemoHeader."Original Doc. No.";
                                    end else begin
            ...
            begin
                ...
                if (xRec."Agreement No." <> "Agreement No.") then begin
                    if ("Agreement No." <> '') then begin
                        ...
                        if HideValidationDialog or not GuiAllowed or (xRec."Agreement No." = '') then
                            Confirmed := true
                        else
                        ...
                        if Confirmed then begin
                            ...
                            CustomerAgreement.CheckBlockedCustOnDocs(Cust, "Document Type".AsInteger(), false, false);
                            CustomerAgreement.TestField("Customer Posting Group");
                            if "Posting Date" <> 0D then begin
                                if CustomerAgreement."Starting Date" > "Posting Date" then
                                    Error(Text12404, CustomerAgreement.FieldCaption("Starting Date"), FieldCaption("Posting Date"));
                                if (CustomerAgreement."Expire Date" <> 0D) and (CustomerAgreement."Expire Date" < "Posting Date") then
                                    Error(Text12403, CustomerAgreement.FieldCaption("Expire Date"), FieldCaption("Posting Date"));
                            end else begin
                                if CustomerAgreement."Starting Date" > "Document Date" then
                                    Error(Text12404, CustomerAgreement.FieldCaption("Starting Date"), FieldCaption("Document Date"));
                                if (CustomerAgreement."Expire Date" <> 0D) and (CustomerAgreement."Expire Date" < "Document Date") then
                                    Error(Text12403, CustomerAgreement.FieldCaption("Expire Date"), FieldCaption("Document Date"));
                            end;
                            ...
                            if "Payment Terms Code" <> CustomerAgreement."Payment Terms Code" then
                                Validate("Payment Terms Code", CustomerAgreement."Payment Terms Code");
                            if "Payment Method Code" <> CustomerAgreement."Payment Method Code" then
                                Validate("Payment Method Code", CustomerAgreement."Payment Method Code");

                            if "Gen. Bus. Posting Group" <> CustomerAgreement."Gen. Bus. Posting Group" then
                                Validate("Gen. Bus. Posting Group", CustomerAgreement."Gen. Bus. Posting Group");
                            if "VAT Bus. Posting Group" <> CustomerAgreement."VAT Bus. Posting Group" then
                                Validate("VAT Bus. Posting Group", CustomerAgreement."VAT Bus. Posting Group");
                            if "Customer Posting Group" <> CustomerAgreement."Customer Posting Group" then
                                Validate("Customer Posting Group", CustomerAgreement."Customer Posting Group");
                            if "Currency Code" <> CustomerAgreement."Currency Code" then
                                Validate("Currency Code", CustomerAgreement."Currency Code");
                            if "Customer Price Group" <> CustomerAgreement."Customer Price Group" then
                                Validate("Customer Price Group", CustomerAgreement."Customer Price Group");
                            if "Customer Disc. Group" <> CustomerAgreement."Customer Disc. Group" then
                                Validate("Customer Disc. Group", CustomerAgreement."Customer Disc. Group");
                            if "Language Code" <> CustomerAgreement."Language Code" then
                                Validate("Language Code", CustomerAgreement."Language Code");
                            if "Salesperson Code" <> CustomerAgreement."Salesperson Code" then
                                Validate("Salesperson Code", CustomerAgreement."Salesperson Code");

                            if "Responsibility Center" <> CustomerAgreement."Responsibility Center" then
                                Validate("Responsibility Center", CustomerAgreement."Responsibility Center");
                            if "Location Code" <> CustomerAgreement."Location Code" then
                                Validate("Location Code", CustomerAgreement."Location Code");
                            if "Ship-to Code" <> CustomerAgreement."Ship-to Code" then
                                Validate("Ship-to Code", CustomerAgreement."Ship-to Code");
                            if "Shipment Method Code" <> CustomerAgreement."Shipment Method Code" then
                                Validate("Shipment Method Code", CustomerAgreement."Shipment Method Code");
                            if "Shipping Advice" <> CustomerAgreement."Shipping Advice" then
                                Validate("Shipping Advice", CustomerAgreement."Shipping Advice");
                            if "Shipping Agent Code" <> CustomerAgreement."Shipping Agent Code" then
                                Validate("Shipping Agent Code", CustomerAgreement."Shipping Agent Code");
                            if "Shipping Agent Service Code" <> CustomerAgreement."Shipping Agent Service Code" then
                                Validate("Shipping Agent Service Code", CustomerAgreement."Shipping Agent Service Code");
                            if "Shipping Time" <> CustomerAgreement."Shipping Time" then
                                Validate("Shipping Time", CustomerAgreement."Shipping Time");

                            if "Prices Including VAT" <> CustomerAgreement."Prices Including VAT" then
                                Validate("Prices Including VAT", CustomerAgreement."Prices Including VAT");
                            Validate("External Agreement No.", CustomerAgreement."External Agreement No.");

                            if (xRec."Currency Code" <> "Currency Code") or
                               (xRec."Gen. Bus. Posting Group" <> "Gen. Bus. Posting Group") or
                               (xRec."VAT Bus. Posting Group" <> "VAT Bus. Posting Group")
                            then
                                RecreateSalesLines(FieldCaption("Agreement No."));

                            OldDimSetID := "Dimension Set ID";
                            "Dimension Set ID" :=
                              CustomerAgreement.GetDefaultDimSetID("Dimension Set ID", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code");
                            UpdateAllLineDim("Dimension Set ID", OldDimSetID);

                            UpdateSalesLinesByFieldNo(FieldNo("Agreement No."), false);
                        end;
                    end else begin
        ...
        field(12498; "Additional VAT Ledger Sheet"; Boolean)
        {
            ...
            trigger OnValidate()
            begin
                TestField("Include In Purch. VAT Ledger", false);
            end;
        }
    ...
    begin
        ...
        DocSignMgt.SetDefaults(DATABASE::"Sales Header", "Document Type".AsInteger(), "No.");
            ...
            case "Document Type" of
                ...
                "Document Type"::Invoice:
                    begin
                        if "Corrective Document" then
                            NoSeriesMgt.SetDefaultSeries("Posting No. Series", SalesSetup."Posted Corr. Invoice Nos.")
                        else
                            if ("No. Series" <> '') and
                ...
                "Document Type"::"Return Order":
                    begin
                        ...
                        "Include In Purch. VAT Ledger" := true;
                    end;
                "Document Type"::"Credit Memo":
                    begin
                        if "Corrective Document" then
                            NoSeriesMgt.SetDefaultSeries("Posting No. Series", SalesSetup."Posted Corr. Credit Memo Nos.")
                        else
                            if ("No. Series" <> '') and
            ...
            case "Document Type" of
                ...
                "Document Type"::Invoice:
                    begin
                        ...
                        if "Corrective Document" then
                            SalesSetup.TestField("Posted Corr. Invoice Nos.");
                    end;
                "Document Type"::"Return Order":
                ...
                "Document Type"::"Credit Memo":
                    begin
                        ...
                        if "Corrective Document" then
                            SalesSetup.TestField("Posted Corr. Credit Memo Nos.");
                    end;
                ...
                if not IsHandled then
                    case ChangedFieldNo of
                        ...
                        FieldNo("Agreement No."):
                            if SalesLine."No." <> '' then
                                SalesLine.Validate("Agreement No.", "Agreement No.");
                        else
    ...
    begin
        ...
        if ShippingAgentServices.Get("Shipping Agent Code", "Shipping Agent Service Code") then
            ...
            if "Agreement No." = '' then begin
                ...
                CustomerAgreement.Get("Bill-to Customer No.", "Agreement No.");
                "Shipping Time" := CustomerAgreement."Shipping Time";
            end;
        end;
    ...
    [Scope('OnPrem')]
    procedure RecalculateSalesLines(TargetAmount: Decimal)
    var
        SalesLine: Record "Sales Line";
        CalcAmount: Decimal;
        "Part": Decimal;
            ...
            repeat
                if (SalesLine.Type <> SalesLine.Type::" ") and (SalesLine."No." <> '') then begin
                    if "Amount Including VAT" <> 0 then
                        Part := SalesLine."Amount Including VAT" / "Amount Including VAT"
                    else
                        Part := 0;
                    SalesLine.Validate("Unit Price", Round(Part * TargetAmount));
                    SalesLine.Validate("Amount Including VAT", SalesLine."Unit Price");
                    SalesLine.Modify();

                    SalesLine1 := SalesLine;
                    CalcAmount := CalcAmount + SalesLine."Unit Price";
                end;
            until SalesLine.Next = 0;
            if (CalcAmount <> TargetAmount) and (CalcAmount <> 0) then begin
                SalesLine1.Validate("Unit Price", SalesLine1."Unit Price" + TargetAmount - CalcAmount);
                SalesLine1.Modify();
            end;
        end;
    end;
        ...
        if "Corrected Doc. No." <> '' then
            case "Corrected Doc. Type" of
                "Corrected Doc. Type"::Invoice:
                    begin
                        SalesInvHeader.Get("Corrected Doc. No.");
                        TestField("Agreement No.", SalesInvHeader."Agreement No.");
                    end;
                "Corrected Doc. Type"::"Credit Memo":
                    begin
                        SalesCrMemoHeader.Get("Corrected Doc. No.");
                        TestField("Agreement No.", SalesCrMemoHeader."Agreement No.");
                    end;
            end;
    end;
    ...
    begin
        ...
        SalesLine.UpdateCorrLine(TempSalesLine);
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckCreditMaxBeforeInsert(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean; HideCreditCheckDialogue: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCreateSalesLine(var TempSalesLine: Record "Sales Line" temporary; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(TRUE, false)]
    local procedure OnBeforeTestStatusOpen(var SalesHeader: Record "Sales Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnRecreateReservEntryReqLineOnAfterLoop(var SalesHeader: Record "Sales Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeSetBillToCustomerAddressFieldsFromCustomer(var SalesHeader: Record "Sales Header"; var BillToCustomer: Record Customer; var SkipBillToContact: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeValidateShippingAgentCode(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean)
    begin
    end;
