report 206 "Sales - Invoice"
{
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(Description_VatBusPostGroup; VatBusPostGroup.Description)
                    {
                    }
                    column(BasisVATText_VATBus; BasisVATText)
                    {
                    }
                    column(EU3PartyText; EU3PartyText)
                    {
                    }
                    column(RefNoText; RefNoText)
                    {
                    }
                    column(RefNo_SalesInvHdr; "Sales Invoice Header"."Reference No.")
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(Description_VatProdPostGroup; VatProdPostGroup.Description)
                        {
                        }
                        column(BasisVATText_VATProd; BasisVATText)
                        {
                        }
                        column(PrintOnInvoice_VatProdPostGroup; VatProdPostGroup."Print on Invoice")
                        {
                        }
                        ...
                        begin
                            ...
                            BasisVATText := '';
                            if VatProdPostGroup.Get("VAT Prod. Posting Group") then
                                BasisVATText := BasisVATTextContent;
                        end;
                    ...
                    dataitem(PaymentDiscount; "Integer")
                    {
                        DataItemTableView = SORTING(Number) WHERE(Number = CONST(1));
                        column(PaymDiscPossible_CustLedg; PaymDiscPossible)
                        {
                        }
                        column(PaymDiscPossibleText_CustLedg; PaymDiscPossibleText)
                        {
                        }
                    }
            ...
            begin
                ...
                BasisVATText := '';
                if VatBusPostGroup.Get("VAT Bus. Posting Group") then
                    if VatBusPostGroup."Print on Invoice" then
                        BasisVATText := BasisVATTextContent;

                EU3PartyText := '';
                if "EU 3-Party Trade" then
                    EU3PartyText := EU3PartyTextContent;

                PaymDiscPossibleText := '';
                PaymDiscPossible := 0;
                CustLedgEntry.SetRange("Document Type", CustLedgEntry."Document Type"::Invoice);
                CustLedgEntry.SetRange("Document No.", "No.");
                if CustLedgEntry.FindFirst then begin
                    PaymDiscPossible := CustLedgEntry."Original Pmt. Disc. Possible";
                    if PaymDiscPossible <> 0 then
                        PaymDiscPossibleText := PaymDiscPossibleContent;
                end;
    ...
    var
        ...
        BasisVATText: Text[30];
        RefNoText: Text[30];
        EU3PartyText: Text[30];
        BasisVATTextContent: Label 'Basis of VAT';
        EU3PartyTextContent: Label 'EU 3-Party Trade';
        PaymDiscPossibleText: Text[30];
        PaymDiscPossible: Decimal;
        PaymDiscPossibleContent: Label 'Payment Discount Possible';
    ...
    local procedure FormatDocumentFields(var SalesInvoiceHeader: Record "Sales Invoice Header")
    begin
