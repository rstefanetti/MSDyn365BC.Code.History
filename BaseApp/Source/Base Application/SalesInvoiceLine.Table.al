table 113 "Sales Invoice Line"
{
    ...
    fields
    {
        ...
        field(12423; "Item Shpt. Entry No."; Integer)
        {
        ...
        {
            Caption = 'Original No.';
            TableRelation = IF ("Original Type" = CONST("G/L Account")) "G/L Account"
            ELSE
    ...
    keys
    {
        ...
        key(Key8; "Original Doc. Type", "Original Doc. No.", "Original Doc. Line No.")
        {
        }
        key(Key9; "Order No.", "Order Line No.", "Posting Date")
        {
        }
        key(Key10; "Document No.", "Location Code")
        {
            MaintainSQLIndex = false;
    ...
    [Scope('OnPrem')]
    procedure UpdateAmountsOnLines(DocumentNo: Code[20]; var AdjmtVATEntry: Record "VAT Entry")
    var
            ...
            repeat
                ...
                if AdjmtVATEntry.FindFirst then begin
                    ...
                    if TempVATAmountLine."VAT Amount" <> 0 then
                        NewVATAmount :=
                          RemainingVATAmount + VATAmount + (VATAmount * -AdjmtVATEntry.Amount / TempVATAmountLine."VAT Amount");
                    RemainingVATAmount := NewVATAmount - Round(NewVATAmount);

                    "Amount (LCY)" := NewAmount;
                    "Amount Including VAT (LCY)" := NewAmount + Round(NewVATAmount);
                    Modify;
                end;
            until Next = 0;
    end;
