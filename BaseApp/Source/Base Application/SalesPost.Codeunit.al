codeunit 80 "Sales-Post"
{
    ...
    begin
        ...
        if not IndirectCall then begin
        ...
        OnRunOnBeforeMakeInventoryAdjustment(SalesHeader, SalesInvHeader);
        ...
        if not (InvtPickPutaway or SuppressCommit) and CommitAllowed and (not IndirectCall) then begin
            Commit();
    ...
    var
        ...
        InvCustLedgEntry2: Record "Cust. Ledger Entry";
        ...
        TempGetSalesLines: Boolean;
        Text12400: Label 'FA No. %1 must be disposed';
    ...
    begin
        with SalesHeader do begin
            ...
            CommitAllowed := ("Prepmt. Diff. Appln. Entry No." = 0) and (not "Prepmt. Diff.");
            ...
            if Invoice then
                CheckCorrDocPostingValues(SalesHeader);
            ...
            if GLSetup."Enable Russian Accounting" then
                DocSignMgt.CheckDocSignatures(DATABASE::"Sales Header", "Document Type".AsInteger(), "No.");
            ...
            if "Prepmt. Diff." then begin
                InvCustLedgEntry2.Get("Prepmt. Diff. Appln. Entry No.");
                "Prepmt. Diff. Appln. Entry No." := 0;
                InitialVATTransactionNo := "No. Printed";
                "No. Printed" := 0;
            end;
            ...
            if not PreviewMode and ModifyHeader then begin
                Modify;
                if not SuppressCommit and CommitAllowed then
                    Commit();
            end;
            ...
            if SalesHeader.Invoice then
                if SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice] then begin
                    ...
                    if not IsHandled then begin
                        ...
                        if SalesInvLine2.Get(InvCustLedgEntry2."Document No.", SalesInvLine."Line No.") then begin
                            SalesInvLine2."Amount (LCY)" += SalesInvLine."Amount (LCY)";
                            SalesInvLine2."Amount Including VAT (LCY)" += SalesInvLine."Amount Including VAT (LCY)";
                        ...
                        OnAfterSalesInvLineInsert(
                          ...
                          SalesHeader, TempItemChargeAssgntSales, TempWhseShptHeader, TempWhseRcptHeader);
                    ...
                    if not IsHandled then begin
                        ...
                        if SalesInvLine2.Get(InvCustLedgEntry2."Document No.", SalesCrMemoLine."Line No.") then begin
                            SalesInvLine2."Amount (LCY)" -= SalesCrMemoLine."Amount (LCY)";
                            SalesInvLine2."Amount Including VAT (LCY)" -= SalesCrMemoLine."Amount Including VAT (LCY)";
                            SalesInvLine2.Modify();
                        end;
                        IsHandled := false;
    ...
    begin
        ...
        with SalesHeader do begin
            ...
            if not (GLSetup."Enable Russian Accounting" and "Prepmt. Diff.") then
    ...
    begin
        ...
        if SalesLine."No Ledger Entry" then
            exit;
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            "Red Storno" := SalesHeader.Correction and
            ...
            if SalesLine.Type = SalesLine.Type::Item then begin
                ...
                if Item."CD Specific Tracking" then begin
                    ItemTracking := true;
                ...
                if Item."Unit Volume Mandatory" then
                    SalesLine.TestField("Unit Volume");
            end;
    ...
    begin
        ...
        with TempTrackingSpecificationInv do begin
            ...
            OnPostItemChargePerOrderOnAfterTempTrackingSpecificationInvSetFilters(SalesHeader, ItemJnlLine2);
            if IsEmpty() then
    ...
    begin
        ...
        with SalesLine do begin
            ...
            if GLSetup."Enable Russian Accounting" then begin
                ...
                if (FADeprBook."Disposal Date" = 0D) or (FADeprBook."Book Value" <> 0) then
                    Error(Text12400, SalesLine."No.");
            end;
        end;
    end;
            ...
            if Ship and ("Shipping No." = '') then
                ...
                then
                    if not PreviewMode then begin
                        ...
                        if ("Shipping No. Series" = "Posting No. Series") and ("Posting No." <> '') then
                            "Shipping No." := "Posting No."
                        else
    ...
    begin
        ...
        with SalesHeader do
            if Invoice and ("Posting No." = '') then begin
                ...
                then begin
                    if not PreviewMode then begin
                        if ("Shipping No. Series" = "Posting No. Series") and ("Shipping No." <> '') then
                            "Posting No." := "Shipping No."
                        else
                            if ("Return Receipt No. Series" = "Posting No. Series") and ("Return Receipt No." <> '') then
                                "Posting No." := "Return Receipt No."
                            else
            ...
            if Receive and ("Return Receipt No." = '') then
                ...
                then
                    if not PreviewMode then begin
                        ...
                        if ("Return Receipt No. Series" = "Posting No. Series") and ("Posting No." <> '') then
                            "Return Receipt No." := "Posting No."
                        else
    ...
    begin
        ...
        with SalesHeader do begin
            ...
            DocSign.SetRange("Table ID", DATABASE::"Sales Header");
            DocSign.SetRange("Document Type", "Document Type");
    ...
    begin
        ...
        with SalesHeader do begin
            ...
            then begin
                ...
                DocSignMgt.DeleteDocSign(DATABASE::"Sales Header", "Document Type".AsInteger(), "No.");
                if not PreviewMode then
    ...
    begin
        ...
        with SalesLine do begin
            ...
            xSalesLine."Amount (LCY)" := Amount;
            xSalesLine."Amount Including VAT (LCY)" := "Amount Including VAT";
            ...
            if FindSet() then begin
                ...
                repeat
                    ...
                    if Item."Item Tracking Code" <> '' then begin
                        ...
                        CDTrackingSetup."Location Code" := "Location Code";
                        ItemTrackingManagement.GetItemTrackingSetup(
                            ItemTrackingCode, CDTrackingSetup, ItemJnlLine."Entry Type"::Sale.AsInteger(), Inbound, ItemTrackingSetup);
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            "Prepmt. Diff. Appln. Entry No." := SalesHeader."Prepmt. Diff. Appln. Entry No.";
            "Agreement No." := SalesHeader."Agreement No.";
            "VAT Entry Type" := SalesHeader."VAT Entry Type";
            if SalesHeader."Prepmt. Diff." then begin
                "Prepmt. Diff." := true;
                CVLedgEntryBuf.CopyFromCustLedgEntry(InvCustLedgEntry2);
                GenJnlPostLine.TransferCVLedgerEntryBuf(CVLedgEntryBuf);
            end;
            if SalesHeader."Corrective Document" then
                "Corrective Doc. Type" := "Corrective Doc. Type"::Correction;
    ...
    begin
        ...
        OnPostBalancingEntryOnBeforeFindCustLedgEntry(
          SalesHeader, TotalSalesLine2, DocType.AsInteger(), DocNo, ExtDocNo, CustLedgEntry, EntryFound);
    ...
    begin
        IsHandled := false;
        OnBeforeCheckCustBlockage(SalesHeader, CustCode, ExecuteDocCheck, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        with SalesHeader do begin
            ...
            if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then
                CopyFACommentLines("No.", SalesInvHeader."No.");
        end;
    end;
    ...
    begin
        ...
        SalesShptLine.Amount := xSalesLine.Amount;
        SalesShptLine."Amount Including VAT" := xSalesLine."Amount Including VAT";
        SalesShptLine."Amount (LCY)" := xSalesLine."Amount (LCY)";
        SalesShptLine."Amount Including VAT (LCY)" := xSalesLine."Amount Including VAT (LCY)";
        ...
        OnAfterInsertShipmentLine(SalesHeader, SalesLine, SalesShptLine);
    end;
    ...
    [Scope('OnPrem')]
    procedure GetSalesLinesToPrint(var SalesHeader: Record "Sales Header"; var NewSalesLine: Record "Sales Line"; var OldSalesLine: Record "Sales Line"; QtyType: Option General,Invoicing,Shipping)
    var
        TempCost: Decimal;
    begin
        NewSalesLine.SetSalesHeader(SalesHeader);
        OldSalesLine.SetSalesHeader(SalesHeader);
        TempGetSalesLines := true;
        TempCost := 0;
        SumSalesLines2(SalesHeader, NewSalesLine, OldSalesLine, QtyType, true, false, TempCost);
        TempGetSalesLines := false;
    end;

    local procedure CopyFACommentLines(FromNumber: Code[20]; ToNumber: Code[20])
    var
    ...
    [Scope('OnPrem')]
    procedure CreateCorrDoc(DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"; PrepaymentAdjustment: Boolean)
    var
        SalesHeader: Record "Sales Header";
        SalesLine: Record "Sales Line";
        ...
        CorrType: Option Loss,Gain;
        AccountType: Option Vendor,Customer;
    begin
        InvEntry.Get(DtldCVLedgEntryBuf."CV Ledger Entry No.");
        PayEntry.Get(DtldCVLedgEntryBuf."Applied CV Ledger Entry No.");

        case InvEntry."Document Type" of
            InvEntry."Document Type"::Invoice:
                begin
                    FromSalesInvHeader.Get(InvEntry."Document No.");
                    DocType := DocType::"Posted Invoice";
                end;
            InvEntry."Document Type"::"Credit Memo":
                begin
                    FromSalesCrMemoHeader.Get(InvEntry."Document No.");
        ...
        if PrepaymentAdjustment then
            ...
            SalesHeader."Prepmt. Diff. Appln. Entry No." := DtldCVLedgEntryBuf."Transaction No.";
        // here Transaction No. stores DtldVLE.EntryNo
        ...
        then
            AmtDiffMgt.UpdateCorrDocDimSetID(
              AccountType::Customer, SalesHeader."Dimension Set ID", CorrType, SalesHeader."Prepmt. Diff.");

        if UpdateSalesLines(
             SalesHeader,
             DtldCVLedgEntryBuf."Amount (LCY)",
             InvEntry."Document No.",
             InvEntry."Currency Code",
             PayEntry."Currency Code",
             CorrType)
        then begin
    ...
    [Scope('OnPrem')]
    procedure UpdateSalesLines(var SalesHeader: Record "Sales Header"; Amount: Decimal; DocNo: Code[20]; CurrencyCode: Code[20]; CurrencyCode2: Code[20]; CorrType: Option Loss,Gain): Boolean
    var
        SalesLine: Record "Sales Line";
        ...
        Currency: Record Currency;
        AmtDiffMgt: Codeunit PrepmtDiffManagement;
        RcptShptIsFound: Boolean;
        TempSalesReturnRcptLine: Record "Return Receipt Line";
        AccountType: Option Vendor,Customer;
    begin
        if SalesHeader."Prepmt. Diff." then
            CustLedgEntry.Get(SalesHeader."Prepmt. Diff. Appln. Entry No.");
        SalesHeader.CalcFields("Amount Including VAT");
        SalesHeader.RecalculateSalesLines(Abs(Amount));
        ...
        repeat
            if (SalesLine.Type <> SalesLine.Type::" ") and (SalesLine."No." <> '') then begin
                if UseZeroVAT then begin // zero VAT
                    VATProdPostingGroup.Get(SalesLine."VAT Prod. Posting Group");
                    VATProdPostingGroup.TestField("Zero VAT Group Code");
                    SalesLine."VAT Prod. Posting Group" := VATProdPostingGroup."Zero VAT Group Code";
                end;
                TmpSalesLine := SalesLine;
                case SalesLine.Type of
                    SalesLine.Type::Item:
                        if SalesHeader."Prepmt. Diff." then begin
                            InvPostingGroup.Get(SalesLine."Posting Group");
                            case CustLedgEntry."Document Type" of
                                CustLedgEntry."Document Type"::Invoice:
                                    begin
                                        FindSalesShipments(SalesLine, TempSalesShptLine, DocNo, CustLedgEntry."Posting Date");
                                        RcptShptIsFound := not TempSalesShptLine.IsEmpty;
                                    end;
                                CustLedgEntry."Document Type"::"Credit Memo":
                                    begin
                                        FindSalesReturnReceipts(SalesLine, TempSalesReturnRcptLine, DocNo, CustLedgEntry."Posting Date");
                                        RcptShptIsFound := not TempSalesReturnRcptLine.IsEmpty;
                                    end;
                            end;
                            if RcptShptIsFound then begin
                                SalesLineReserve.DeleteLineWithItemTracking(SalesLine);
                                SalesLine.Validate(Type, SalesLine.Type::"Charge (Item)");
                                SalesLine."Gen. Prod. Posting Group" := TmpSalesLine."Gen. Prod. Posting Group";
                                SalesLine."VAT Prod. Posting Group" := TmpSalesLine."VAT Prod. Posting Group";
                                if CurrencyCode2 = '' then begin
                                    InvPostingGroup.TestField("Sales PD Charge Conv. (Item)");
                                    SalesLine.Validate("No.", InvPostingGroup."Sales PD Charge Conv. (Item)");
                                end else begin
                                    InvPostingGroup.TestField("Sales PD Charge FCY (Item)");
                                    SalesLine.Validate("No.", InvPostingGroup."Sales PD Charge FCY (Item)");
                                end;
                                ...
                                case CustLedgEntry."Document Type" of
                                    CustLedgEntry."Document Type"::Invoice:
                                        SalesItemChargeAssgnt(SalesLine, TempSalesShptLine, SalesHeader."Prices Including VAT");
                                    CustLedgEntry."Document Type"::"Credit Memo":
                                        SalesReturnItemChargeAssgnt(SalesLine, TempSalesReturnRcptLine, SalesHeader."Prices Including VAT");
                                end;
                            end;
                        end;
                    SalesLine.Type::"Fixed Asset":
                        begin
                            if SalesHeader."Prepmt. Diff." then
                                FAProcessing2(SalesLine, CustLedgEntry."Entry No.");
                            SalesLine."Line Discount %" := 0;
                            SalesLine.Validate(Quantity, 1);
                            SalesLine.Validate("Unit Price", TmpSalesLine."Unit Price");
                        end;
                ...
                SalesLine."Shortcut Dimension 1 Code" := TmpSalesLine."Shortcut Dimension 1 Code";
                SalesLine."Shortcut Dimension 2 Code" := TmpSalesLine."Shortcut Dimension 2 Code";
                SalesLine."System-Created Entry" := true;
                SalesLine."Dimension Set ID" := TmpSalesLine."Dimension Set ID";
                SalesLine.Modify();
                if SalesHeader."Prepmt. Diff." and (CurrencyCode2 <> '') then
                    AmtDiffMgt.UpdateCorrDocDimSetID(
                      AccountType::Customer,
                      SalesLine."Dimension Set ID",
                      CorrType,
                      SalesHeader."Prepmt. Diff.");
            end;
        until SalesLine.Next = 0;
        exit(true);
    end;
    ...
    [Scope('OnPrem')]
    procedure UpdateSalesLinesForUnapply(var SalesHeader: Record "Sales Header"): Boolean
    var
        SalesLine: Record "Sales Line";
        CustLedgEntry: Record "Cust. Ledger Entry";
        DtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry";
    begin
        DtldCustLedgEntry.Get(SalesHeader."Prepmt. Diff. Appln. Entry No.");
    ...
    [Scope('OnPrem')]
    procedure CreatePDDocForUnapply(DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer")
    var
        SalesHeader: Record "Sales Header";
        SalesLine: Record "Sales Line";
        FromSalesInvHeader: Record "Sales Invoice Header";
        ...
        AccountType: Option Vendor,Customer;
    begin
        if not DtldCustLedgEntry.Get(DtldCVLedgEntryBuf."Entry No.") then
            exit;

        InvEntry.Get(DtldCustLedgEntry."Cust. Ledger Entry No.");
        PayEntry.Get(DtldCustLedgEntry."Applied Cust. Ledger Entry No.");
        ...
        SalesHeader.Init();
        SalesHeader.SetHideValidationDialog(true);
        SalesHeader."No." := DtldCustLedgEntry."Document No.";

        case InvEntry."Document Type" of
            InvEntry."Document Type"::Invoice:
                begin
                    if DtldCustLedgEntry."Amount (LCY)" < 0 then begin
                        SalesHeader."Document Type" := SalesHeader."Document Type"::Invoice;
                        CorrType := CorrType::Loss;
                    end else begin
                        SalesHeader."Document Type" := SalesHeader."Document Type"::"Credit Memo";
    ...
    [Scope('OnPrem')]
    procedure FindSalesShipments(var SalesLine: Record "Sales Line"; var TmpSalesShptLine: Record "Sales Shipment Line"; DocNo: Code[20]; PostingDate: Date)
    var
        ValueEntry: Record "Value Entry";
        ItemLedgEntry: Record "Item Ledger Entry";
        SalesShptLine: Record "Sales Shipment Line";
    begin
        TmpSalesShptLine.Reset();
        TmpSalesShptLine.DeleteAll();
                ...
                then begin
                    ...
                    if ValueEntry.ItemValueEntryExists then
                        if SalesShptLine.Get(ItemLedgEntry."Document No.", ItemLedgEntry."Document Line No.") then begin
                            TmpSalesShptLine := SalesShptLine;
                            if TmpSalesShptLine.Insert() then;
                        end;
                end;
    ...
    [Scope('OnPrem')]
    procedure FindSalesReturnReceipts(var SalesLine: Record "Sales Line"; var TmpReturnReceiptLine: Record "Return Receipt Line"; DocNo: Code[20]; PostingDate: Date)
    var
        ValueEntry: Record "Value Entry";
        ItemLedgEntry: Record "Item Ledger Entry";
        ReturnRcptLine: Record "Return Receipt Line";
    begin
        TmpReturnReceiptLine.Reset();
        TmpReturnReceiptLine.DeleteAll();
                ...
                then begin
                    ...
                    if ValueEntry.ItemValueEntryExists then
                        if ReturnRcptLine.Get(ItemLedgEntry."Document No.", ItemLedgEntry."Document Line No.") then begin
                            TmpReturnReceiptLine := ReturnRcptLine;
                            if TmpReturnReceiptLine.Insert() then;
                        end;
                end;
            until ValueEntry.Next = 0;
    end;
    ...
    [Scope('OnPrem')]
    procedure SalesItemChargeAssgnt(var SalesLine: Record "Sales Line"; var TmpSalesShptLine: Record "Sales Shipment Line"; PricesIncludingVAT: Boolean)
    var
        ...
        with SalesLine do begin
            ...
            if not PricesIncludingVAT then
                ...
                ItemChargeAssgntLineAmt :=
                  Round(CalcLineAmount / (1 + "VAT %" / 100), Currency."Unit-Amount Rounding Precision");
            ItemChargeAssgntSales."Unit Cost" :=
              Round(ItemChargeAssgntLineAmt / Quantity, Currency."Unit-Amount Rounding Precision");

            TmpSalesShptLine.Find('-');
            AssignItemChargeSales.CreateShptChargeAssgnt(TmpSalesShptLine, ItemChargeAssgntSales);

            AssignItemChargeSales.AssignItemCharges(SalesLine, 1, ItemChargeAssgntLineAmt, AssignItemChargeSales.AssignByAmountMenuText);
        end;
    end;

    [Scope('OnPrem')]
    procedure SalesReturnItemChargeAssgnt(var SalesLine: Record "Sales Line"; var TmpSalesReturnRcptLine: Record "Return Receipt Line"; PricesIncludingVAT: Boolean)
    var
        AssignItemChargeSales: Codeunit "Item Charge Assgnt. (Sales)";
        ItemChargeAssgntSales: Record "Item Charge Assignment (Sales)";
    begin
        with SalesLine do begin
            ...
            if not PricesIncludingVAT then
                ...
                ItemChargeAssgntSales."Unit Cost" :=
                  Round(CalcLineAmount / Quantity / (1 + "VAT %" / 100), Currency."Unit-Amount Rounding Precision");

            TmpSalesReturnRcptLine.Find('-');
            AssignItemChargeSales.CreateRcptChargeAssgnt(TmpSalesReturnRcptLine, ItemChargeAssgntSales);

            // AssignItemChargeSales.SuggestAssignment2(SalesLine,1,2,0);
        end;
    end;

    [Scope('OnPrem')]
    procedure FAProcessing2(var SalesLine: Record "Sales Line"; InvEntryNo: Integer)
    var
        CustLedgEntry: Record "Cust. Ledger Entry";
    ...
    begin
        TmpSalesLine := SalesLine;
        SalesLine.Type := SalesLine.Type::"G/L Account";
        FADeprBook.Get(SalesLine."No.", SalesLine."Depreciation Book Code");
        CustLedgEntry.Get(InvEntryNo);
        ...
        FALedgEntry.SetRange("FA No.", SalesLine."No.");
        FALedgEntry.SetRange("Depreciation Book Code", SalesLine."Depreciation Book Code");
        ...
        FALedgEntry.SetRange("Document No.", CustLedgEntry."Document No.");
        if FALedgEntry.FindFirst then begin
            case FALedgEntry."Result on Disposal" of
                FALedgEntry."Result on Disposal"::Gain:
                    SalesLine.Validate("No.", FAPostingGroup.GetDisposalGainLossAccount(true));
                FALedgEntry."Result on Disposal"::Loss:
                    SalesLine.Validate("No.", FAPostingGroup.GetDisposalGainLossAccount(false));
            end;
            SalesLine.Validate("Gen. Prod. Posting Group", TmpSalesLine."Gen. Prod. Posting Group");
            SalesLine.Validate("VAT Prod. Posting Group", TmpSalesLine."VAT Prod. Posting Group");
        end;
    end;

    [Scope('OnPrem')]
    procedure UpdateCorrLineAmounts(SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line")
    var
        CorrDocSign: Integer;
    begin
        if not SalesHeader."Corrective Document" then
            exit;
    ...
    [Scope('OnPrem')]
    procedure UpdateCorrLineLCYAmounts(SalesHeader: Record "Sales Header"; var TempSalesLine: Record "Sales Line"; SalesLine: Record "Sales Line")
    var
        CorrDocSign: Integer;
    begin
        if not SalesHeader."Corrective Document" then
        ...
        CorrDocSign := GetCorrDocSign(SalesLine);
        with TempSalesLine do begin
            "Amount (LCY) (After)" := "Amount (LCY) (Before)" + CorrDocSign * SalesLine."Amount (LCY)";
            "Amt. Incl. VAT (LCY) (After)" :=
              "Amt. Incl. VAT (LCY) (Before)" + CorrDocSign * SalesLine."Amount Including VAT (LCY)";
        end;
    end;
    ...
    [Scope('OnPrem')]
    procedure CheckCorrLinePostingValues(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line")
    begin
        if (not SalesHeader."Corrective Document") or (SalesLine.Type = SalesLine.Type::" ") then
            exit;

        if SalesHeader."Corrective Doc. Type" <> SalesHeader."Corrective Doc. Type"::Revision then
            if (SalesLine."Quantity (Before)" = SalesLine."Quantity (After)") and
               (SalesLine."Unit Price (Before)" = SalesLine."Unit Price (After)")
            then
                Error(NothingToPostErr);
    end;
    ...
    begin
        ItemJnlPostLine.SetPreviewMode(PreviewMode);
    ...
    begin
        with GenJnlLine do begin
            ...
            "Include In Other VAT Ledger" := SalesHeader."Include In Purch. VAT Ledger";
            "Additional VAT Ledger Sheet" := SalesHeader."Additional VAT Ledger Sheet";
            "Corrected Document Date" := SalesHeader."Corrected Document Date";
            if SalesHeader."Additional VAT Ledger Sheet" then
                SalesHeader.TestField("Corrected Document Date");
            "Document Line No." := InvoicePostBuffer."Document Line No.";
            if SalesHeader."Prepmt. Diff." then begin
                "Prepmt. Diff." := true;
                CVLedgEntryBuf.CopyFromCustLedgEntry(InvCustLedgEntry2);
                CVLedgEntryBuf."Entry No." := InvCustLedgEntry2."Transaction No.";
                CVLedgEntryBuf."Transaction No." := InitialVATTransactionNo;
                GenJnlPostLine.TransferCVLedgerEntryBuf(CVLedgEntryBuf);
            end;
            if SalesHeader."Corrective Document" then
                "Corrective Doc. Type" := SalesHeader."Corrective Doc. Type";
    ...
    local procedure CheckCustAgreement(SalesHeader: Record "Sales Header"; CustomerNo: Code[20])
    var
        Customer: Record Customer;
    begin
        Customer.Get(CustomerNo);
        if Customer."Agreement Posting" <> Customer."Agreement Posting"::"No Agreement" then
            SalesHeader.TestField("Agreement No.");
    end;

    [Scope('OnPrem')]
    procedure SetIndirectCall(NewIndirectCall: Boolean)
    begin
        IndirectCall := NewIndirectCall;
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterInsertShipmentLine(var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; var SalesShptLine: record "Sales Shipment Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertReturnReceiptLineWhsePost(SalesLine: Record "Sales Line"; ReturnRcptHeader: Record "Return Receipt Header"; WhseShip: Boolean; WhseReceive: Boolean; TempWhseRcptHeader: Record "Warehouse Receipt Header" temporary; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnRunOnBeforeMakeInventoryAdjustment(var SalesHeader: Record "Sales Header"; SalesInvHeader: Record "Sales Invoice Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckCustBlockage(SalesHeader: Record "Sales Header"; CustCode: Code[20]; var ExecuteDocCheck: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    local procedure CheckCorrDocPostingValues(SalesHeader: Record "Sales Header")
    var
        SalesLine: Record "Sales Line";
    begin
        if not SalesHeader."Corrective Document" or
           (SalesHeader."Corrective Doc. Type" = SalesHeader."Corrective Doc. Type"::Revision)
        then
            exit;
        ...
        if SalesLine.FindSet then
            repeat
                if (SalesLine."Quantity (Before)" = SalesLine."Quantity (After)") and
                   (SalesLine."Unit Price (Before)" = SalesLine."Unit Price (After)")
                then
                    Error(CorrectiveSalesLineErr, SalesLine."Document Type", SalesLine."Document No.", SalesLine."Line No.");
            until SalesLine.Next = 0;
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemChargePerOrderOnAfterTempTrackingSpecificationInvSetFilters(SalesHeader: record "Sales Header"; var ItemJnlLine2: record "Item Journal Line")
    begin
    end;
