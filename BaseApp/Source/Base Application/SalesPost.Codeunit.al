codeunit 80 "Sales-Post"
{
    ...
    local procedure PostGLAndCustomer(var SalesHeader: Record "Sales Header"; var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary; var CustLedgEntry: Record "Cust. Ledger Entry"; LineCount: Integer)
    var
        IsHandled: Boolean;
    begin
        ...
        with SalesHeader do begin
            ...
            if SalesTaxCalculationOverridden then begin
                if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then
                    OnFinalizeInvoicePostingSalesTax(SalesHeader, SalesInvHeader, SuppressCommit)
                else
                    OnFinalizeCreditmemoPostingSalesTax(SalesHeader, SalesCrMemoHeader, SuppressCommit);
            end;
    ...
    begin
        ...
        with SalesLine do
            if ((SalesLineQty = 0) or ("Unit Price" = 0)) and not
        ...
        with PrepmtSalesLine do begin
            ...
            if FindSet() then
                repeat
                    ...
                    UpdatePrepmtAmountInvBuf("Line No.", ActualCurrencyFactor, SalesHeader."Prepmt. Include Tax");
                until Next() = 0;
            Reset;
        end;
    end;
    ...
    begin
        ...
        with TempICGenJnlLine do begin
            ...
            "Bal. Tax Area Code" := SalesLine."Tax Area Code";
            "Bal. Tax Liable" := SalesLine."Tax Liable";
            "Bal. Tax Group Code" := SalesLine."Tax Group Code";
            Validate("Bal. VAT %");
            if "Bal. VAT %" <> 0 then
    ...
    begin
        with GenJournalLine do begin
            ...
            "Account No." := CustomerPostingGroup."Invoice Rounding Account";
            "Currency Code" := SalesHeader."Currency Code";
            Amount := LineAmount;
            "Source Currency Code" := SalesHeader."Currency Code";
            "Source Currency Amount" := Amount;
                    ...
                    if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then begin
                        ...
                        if SalesTaxCountry <> SalesTaxCountry::NoTax then
                            TaxAmountDifference.CopyTaxDifferenceRecords(
                              TaxAmountDifference."Document Product Area"::Sales, "Document Type".AsInteger(), "No.",
    ...
    [IntegrationEvent(false, false)]
    local procedure OnFinalizeInvoicePostingSalesTax(var SalesHeader: Record "Sales Header"; var SalesInvoiceHeader: Record "Sales Invoice Header"; CommitIsSuppressed: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFinalizeCreditmemoPostingSalesTax(var SalesHeader: Record "Sales Header"; var SalesCrMemoHeader: Record "Sales Cr.Memo Header"; CommitIsSuppressed: Boolean)
    begin
    end;
    ...
    begin
        if (ReturnReceiptLine.Type <> ReturnReceiptLine.Type::"Charge (Item)") or (ReturnReceiptLine."No." = '') or
           ((ReturnReceiptLine.Type = ReturnReceiptLine.Type::"Charge (Item)") and (ReturnReceiptLine."Gen. Prod. Posting Group" <> ''))
        then
            exit;
    ...
    var
        ItemCharge: Record "Item Charge";
    begin
        if (SalesShipmentLine.Type <> SalesShipmentLine.Type::"Charge (Item)") or (SalesShipmentLine."No." = '') or
           ((SalesShipmentLine.Type = SalesShipmentLine.Type::"Charge (Item)") and (SalesShipmentLine."Gen. Prod. Posting Group" <> ''))
        then
            exit;
    ...
    var
        ItemCharge: Record "Item Charge";
    begin
        if (SalesLine.Type <> SalesLine.Type::"Charge (Item)") or (SalesLine."No." = '') or
           ((SalesLine.Type = SalesLine.Type::"Charge (Item)") and (SalesLine."Gen. Prod. Posting Group" <> ''))
        then
            exit;
