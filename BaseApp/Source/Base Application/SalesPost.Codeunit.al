codeunit 80 "Sales-Post"
{
    ...
    begin
        with SalesHeader do begin
            ...
            UpdatePostingDescriptionOnInvoice("Posting No.", SalesHeader, ModifyHeader);
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            if Cust.Get(SalesHeader."Sell-to Customer No.") then
                "Customer Salesperson Code" := Cust."Salesperson Code";
    ...
    begin
        ...
        with SalesHeader do
            if ("Posting Description" = Format("Document Type") + ' ' + "No.") or (PostingNo <> '') then begin
                if not ("Document Type" in ["Document Type"::Order, "Document Type"::"Return Order"]) then
                    "Posting Description" :=
                      CopyStr(CopyStr(Format("Document Type"), 1, 4) + '. ' + "Posting No." + '/' +
                        "Bill-to Name", 1, MaxStrLen("Posting Description"))
                else
                    "Posting Description" :=
                      CopyStr(CopyStr(Format("Document Type"), 1, 4) + '. ' + "No." + '/' +
                        "Bill-to Name", 1, MaxStrLen("Posting Description"));
                ModifyHeader := true;
            end;
    end;
