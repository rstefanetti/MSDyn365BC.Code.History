report 393 "Suggest Vendor Payments"
{
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(CreateVATAgentPayment; CreateVATAgentPayment)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Create VAT Agent Payment';
                    }
                    group(Control22)
                    {
                        ShowCaption = false;
    ...
    [Scope('OnPrem')]
    procedure GetVATAgentPmtAmount(): Decimal
    var
    ...
    [Scope('OnPrem')]
    procedure MakeBudgetPmtJnlLine(VATAgentPmtAmount: Decimal)
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
        VendAgrmt: Record "Vendor Agreement";
        Vend: Record Vendor;
    ...
    [Scope('OnPrem')]
    procedure InitVATAgentPayment(NewCreateVATAgentPayment: Boolean)
    begin
