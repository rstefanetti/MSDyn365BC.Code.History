report 11108 "VAT - VIES Declaration XML"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING(Type, "Country/Region Code", "VAT Registration No.", "VAT Bus. Posting Group", "VAT Prod. Posting Group", "Posting Date") WHERE(Type = CONST(Sale), "Bill-to/Pay-to No." = FILTER(<> ''), "Document Type" = FILTER(" " | Invoice | "Credit Memo" | Payment | Refund));
            ...
            begin
                SetFilter("Posting Date", '%1..%2', RepPeriodFrom, RepPeriodTo);
                NoOfRecs := 0;
                Clear(Customer);
            end;
        }
