    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the interest on arrears that has been paid on the vendor transaction.';
                }
                ...
                field("Document Occurrence"; "Document Occurrence")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the sequence number of a vendor bill that is based on a single purchase invoice with multiple due dates.';
                    Visible = false;
                }
                field("Vendor Bill No."; "Vendor Bill No.")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the bill number that is used to link the bill header with the bill lines.';
                }
        ...
        area(navigation)
        {
            group("Ent&ry")
            {
                ...
                action(BillList)
                {
                    ...
                    Caption = '&Bill List';
                    Image = Document;
                    ToolTip = 'Specifies the Bill applied.';
                    ...
                    begin
                        if "Document Type" = "Document Type"::Payment then
                            PostedVendorBillHeader.SetRange("No.", "Document No.")
                        else
                            PostedVendorBillHeader.SetRange("No.", "Vendor Bill List");
                        PAGE.Run(PAGE::"List of Posted Vend. Bill List", PostedVendorBillHeader);
                    end;
                }
            }
        }
            ...
            group("F&unctions")
            {
                ...
                action(CreateWithHoldTaxEntry)
                {
                    ...
                    begin
                        if "Document Type" <> "Document Type"::Payment then
                            Error(
                              Text12100, "Entry No.", "Document Type");
