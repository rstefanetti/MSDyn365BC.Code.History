codeunit 134023 "ERM Pmt Tol Multi Doc Customer"
{
    ...
    begin
        ...
        CreateGenJournalLinesDifferentAmount(GenJournalLine, GenJournalLine."Document Type"::Invoice, CustomerNo, InvAmounts);
        ...
        UpdateMaxPaymentToleranceOnCustLedgerEntry(CustomerNo, GenJournalLine."Document Type"::Invoice, MaxTolAmounts);
        ...
        CreateDocumentLine(
            GenJournalLine, GenJournalLine."Document Type"::Payment, CustomerNo, '', -TotalInvAmount + TotalTolAmount / 2,
        ...
        ApplyAndPostJournalLines(GenJournalLine, GenJournalLine."Document Type"::Invoice);
        ...
        ExpectedPmtTolAmounts.Add(0);
        VerifyPmtToleranceOnClosedCustLedgerEntry(CustomerNo, GenJournalLine."Document Type"::Invoice, ExpectedPmtTolAmounts);
    end;
    ...
    begin
        ...
        FillListWithDecimalValues(
            ...
            -LibraryRandom.RandDecInRange(100, 200, 2), -LibraryRandom.RandDecInRange(100, 200, 2));
        CreateGenJournalLinesDifferentAmount(GenJournalLine, GenJournalLine."Document Type"::"Credit Memo", CustomerNo, CrMemoAmounts);
        LibraryERM.PostGeneralJnlLine(GenJournalLine);
        ...
        FillListWithDecimalValues(
            MaxTolAmounts, TotalTolAmount, -LibraryRandom.RandDecInRange(5, 10, 2), -LibraryRandom.RandDecInRange(5, 10, 2), 0, 0);
        UpdateMaxPaymentToleranceOnCustLedgerEntry(CustomerNo, GenJournalLine."Document Type"::"Credit Memo", MaxTolAmounts);
        ...
        CreateDocumentLine(
            GenJournalLine, GenJournalLine."Document Type"::Refund, CustomerNo, '', -TotalCrMemoAmount + TotalTolAmount / 2,
            LibraryRandom.RandDateFromInRange(GenJournalLine."Due Date", 10, 20), 1);

        // [WHEN] Apply Refund to four posted Sales Credit Memos and then post Payment.
        ApplyAndPostJournalLines(GenJournalLine, GenJournalLine."Document Type"::"Credit Memo");
        ...
        ExpectedPmtTolAmounts.Add(0);
        ExpectedPmtTolAmounts.Add(0);
        VerifyPmtToleranceOnClosedCustLedgerEntry(CustomerNo, GenJournalLine."Document Type"::"Credit Memo", ExpectedPmtTolAmounts);
    end;
    ...
    local procedure CreateGenJournalLinesDifferentAmount(var GenJournalLine: Record "Gen. Journal Line"; DocumentType: Option; CustomerNo: Code[20]; Amounts: List of [Decimal])
    var
        GenJournalBatch: Record "Gen. Journal Batch";
    ...
    begin
        ...
        for i := 1 to Amounts.Count() do
            LibraryJournals.CreateGenJournalLine(
                ...
                GenJournalLine."Account Type"::Customer, CustomerNo, GenJournalLine."Account Type"::"G/L Account", BalGLAccountNo, Amounts.Get(i));
    end;
    ...
    local procedure UpdateMaxPaymentToleranceOnCustLedgerEntry(CustomerNo: Code[20]; DocumentType: Option; MaxPmtTolAmounts: List of [Decimal])
    var
        CustLedgerEntry: Record "Cust. Ledger Entry";
    ...
    local procedure VerifyPmtToleranceOnClosedCustLedgerEntry(CustomerNo: Code[20]; DocumentType: Option; ExpectedPmtTolerances: List of [Decimal])
    var
        CustLedgerEntry: Record "Cust. Ledger Entry";
