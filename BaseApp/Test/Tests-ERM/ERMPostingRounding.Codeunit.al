codeunit 134157 "ERM Posting Rounding"
{
    ...
    local procedure VerifyInvoicePostBufferZeroAmounts(InvoicePostBuffer: Record "Invoice Post. Buffer")
    begin
        with InvoicePostBuffer do begin
            TestField(Amount, 0);
            TestField("Amount (ACY)", 0);
            TestField("VAT Amount", 0);
            TestField("VAT Amount (ACY)", 0);
            TestField("VAT Base Amount", 0);
            TestField("VAT Base Amount (ACY)", 0);
        end;
    end;
}
