codeunit 138940 "BC O365 E2E Page Tests"
{
    ...
    var
        ...
        TaxMustBeSetupPrefixTxt: Label 'You haven''t set up tax information for your business';
    ...
    [Test]
    [HandlerFunctions('EmailDialogModalPageHandler,BCEmailSetupPageHandler,HandleBCO365TaxAreaList')]
    [Scope('OnPrem')]
    ...
    begin
        ...
        // [THEN] There are correct taxes applied
        SalesInvoiceHeader.Get(PostedInvoiceNo);
        Assert.AreEqual('LYNGBY, AB', SalesInvoiceHeader."Tax Area Code", 'Correct tax rate is not applied');
        RecallPostedInvoiceNotification(PostedInvoiceNo);
    end;
    ...
    begin
        Assert.AreNotEqual(
          ...
          StrPos(Notification.Message, TaxMustBeSetupPrefixTxt),
    ...
    var
        BCO365TaxSettings: TestPage "BC O365 Tax Settings";
    begin
        CreateAndSetupTaxRateWithStateAndCity('AB', 3, 'Lyngby', 5);
        LibraryVariableStorage.Enqueue('LYNGBY, AB');
        BCO365TaxSettings.OpenEdit;
        BCO365TaxSettings.TaxRate.AssistEdit;
        BCO365TaxSettings.Close;
    end;

    [ModalPageHandler]
    [Scope('OnPrem')]
    procedure HandleBCO365TaxAreaList(var BCO365TaxSettingsList: TestPage "BC O365 Tax Settings List")
    var
        TaxAreaCode: Code[10];
    begin
        TaxAreaCode := CopyStr(LibraryVariableStorage.DequeueText, 1, MaxStrLen(TaxAreaCode));
        BCO365TaxSettingsList.GotoKey(TaxAreaCode);
        BCO365TaxSettingsList.OK.Invoke;
    end;
