codeunit 144078 "ERM Extra VAT"
{
    ...
    begin
        ...
        CreateSalesDocumentWithMultipleLines(
          SalesLine, VATPostingSetup, SalesHeader."Document Type"::Invoice, '', '', false, CustNo, LineAmount);
        DocumentNo := PostSalesDocument(SalesLine);
        CreateSalesDocumentWithMultipleLines(
          SalesLine, VATPostingSetup, SalesHeader."Document Type"::"Credit Memo", '', '', false, CustNo, LineAmount / 2);
    ...
    begin
        with SalesHeader do begin
            Get(SalesLine."Document Type", SalesLine."Document No.");
            "Applies-to Doc. Type" := "Document Type"::Invoice;
    ...
    local procedure CreateSevPaymentsWithAppliesToId(var GenJournalLine: Record "Gen. Journal Line"; var PaymentNo: array[3] of Code[20]; AccountType: Option; AccountNo: Code[20]; Amount: array[3] of Decimal; Sign: Integer)
    var
        GenJournalBatch: Record "Gen. Journal Batch";
        i: Integer;
    begin
    ...
    local procedure CreatePaymentWithAppliesToId(var GenJournalLine: Record "Gen. Journal Line"; AccountType: Option; AccountNo: Code[20]; Amount: Decimal)
    begin
    ...
    begin
        with PurchaseHeader do begin
            ...
            Validate("Vendor Invoice No.", IncStr("Vendor Invoice No."));
            Validate("Check Total", UpdatePrepmtOnPurchLine(PurchaseHeader, PrepmtPct));
            Modify(true);
        end;
    end;
    ...
    begin
        with VATEntry do begin
            ...
            SetRange("Document No.", ExpectedDocumentNo);
    ...
    begin
        with VATEntry do begin
            SetRange("VAT Calculation Type", "VAT Calculation Type"::"Reverse Charge VAT");
            SetRange("Bill-to/Pay-to No.", VendorNo);
            SetRange("Document No.", ExcludingDocumentNo);
    ...
    begin
        ...
        with VATEntryInvoice do begin
            ...
            repeat
                ...
                Assert.AreNearlyEqual(
                  Abs(Base), BaseAmt, GeneralLedgerSetup."Amount Rounding Precision", StrSubstNo(VATFieldErr, FieldCaption(Base)));
            until Next = 0;
        end;
    end;
    ...
    begin
        with VATEntry do begin
            ...
            Next;
            Assert.AreEqual(ReverseSalesDocNo, "Document No.", StrSubstNo(VATFieldErr, FieldCaption("Document No.")));
            Assert.AreEqual(Type::Sale, Type, StrSubstNo(VATFieldErr, FieldCaption(Type)));
        end;
    end;
        ...
        with VATEntry do begin
            ...
            for i := ArrayLen(SalesLine) downto 1 do begin
                Assert.AreEqual(
                  ...
                  FieldCaption("Unrealized VAT Entry No."));
