codeunit 144561 "ERM Split VAT"
{
    ...
    [Test]
    [HandlerFunctions('SalesInvoiceReportRPH')]
    [Scope('OnPrem')]
    procedure PrintPostedAutoGeneratedLineWithFullVAT()
    var
        SalesHeader: Record "Sales Header";
        SalesLine: Record "Sales Line";
        InvoiceNo: Code[20];
    begin
        ...
        // [SCENARIO 372077] "Sales - Invoice" report should print 0 in "Line Amount" and "Invoice Discount Base Amount"
        Initialize;

        // [GIVEN] Posted Sales Invoice with "Line Amount" = "X" in split VAT line with Full VAT
        CreateSalesDocWithSplitVATLineFullVAT(SalesHeader, SalesLine, SalesHeader."Document Type"::Invoice);
        InvoiceNo := LibrarySales.PostSalesDocument(SalesHeader, true, true);

        // [WHEN] Print "Sales - Invoice" Report
        RunSalesInvoiceReport(InvoiceNo);
        ...
        VerifySalesInvoiceReport(SalesLine);
    end;

    [Test]
    [HandlerFunctions('SalesCreditMemoReportRPH')]
    [Scope('OnPrem')]
    procedure PrintPostedCreditMemoAutoGeneratedLineWithFullVAT()
    var
        SalesHeader: Record "Sales Header";
        SalesLine: Record "Sales Line";
        InvoiceNo: Code[20];
    begin
        // [FEATURE] [Full VAT] [Credit Memo]
        // [SCENARIO 376365] "Sales - Credit Memo" report should not print VAT Reversal line in the Invoice body section
        Initialize;

        // [GIVEN] Posted Sales Credit Memo with split VAT line with Full VAT
        CreateSalesDocWithSplitVATLineFullVAT(SalesHeader, SalesLine, SalesHeader."Document Type"::"Credit Memo");
        InvoiceNo := LibrarySales.PostSalesDocument(SalesHeader, true, true);

        // [WHEN] Print "Sales - Invoice" Report
        RunSalesCreditMemoReport(InvoiceNo);

        // [THEN] Positive Invoice Line is shown in the Invoice body section (TFS 376365)
        // [THEN] Negative (VAT Reversal) Line is not shown in the Invoice body section (TFS 376365)
        // [THEN] Both VAT lines are shown in the VAT Amount Specification section (TFS 376615)
        // [THEN] VAT Amount line with VAT% = 0 has "Line Amount" = 0, "Inv. Disc. Base Amount" = 0 (TFS 376615)
        VerifySalesCreditMemoReport(SalesLine);
    end;
    ...
    [RequestPageHandler]
    [Scope('OnPrem')]
    procedure SalesInvoiceReportRPH(var SalesInvoice: TestRequestPage "Sales - Invoice")
    begin
    ...
    [RequestPageHandler]
    [Scope('OnPrem')]
    procedure SalesCreditMemoReportRPH(var SalesCreditMemo: TestRequestPage "Sales - Credit Memo")
    begin
