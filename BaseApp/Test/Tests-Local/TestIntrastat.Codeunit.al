codeunit 134153 "Test Intrastat"
{
    ...
    begin
        ...
        // Exercise
        RunIntrastatMakeDiskTaxAuth(IntrastatJnlBatch, Filename, IntrastatJnlLine.Type::Shipment);
        Assert.IsTrue(FileManagement.ServerFileExists(Filename), FileNotCreatedErr);

        // Verify
        Commit();
        asserterror RunIntrastatMakeDiskTaxAuth(IntrastatJnlBatch, Filename, IntrastatJnlLine.Type::Shipment);
    ...
    begin
        ...
        IntrastatJnlLine.Validate("Shpt. Method Code", ShipmentMethod.Code);
        IntrastatJnlLine.Modify(true);

        // [WHEN] Run 'Intrastat - Make Disk Tax Auth' report
        Filename := FileManagement.ServerTempFileName('txt');
        RunIntrastatMakeDiskTaxAuth(IntrastatJnlBatch, Filename, IntrastatJnlLine.Type::Receipt);
    ...
    begin
        ...
        IntrastatJnlBatch.Validate("Statistics Period", Format(PostingDate, 0, '<Month,2><Year,2>'));
    ...
    local procedure RunIntrastatMakeDiskTaxAuth(IntrastatJnlBatch: Record "Intrastat Jnl. Batch"; Filename: Text; IntraJnlLineType: Option)
    var
        IntrastatJnlLine: Record "Intrastat Jnl. Line";
    ...
    begin
        ...
        IntrastatJnlLine.SetRange(Type, IntraJnlLineType);
