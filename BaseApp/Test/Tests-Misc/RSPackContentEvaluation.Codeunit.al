codeunit 138400 "RS Pack Content - Evaluation"
{
    ...
    begin
        // [SCENARIO] There is 3 Gen Bus. Posting group and 6 Prod. Posting groups
        Assert.RecordCount(GenBusPostingGroup, 3);
    ...
    begin
        // [SCENARIO] There are 2 Gen. Posting groups, where Product Group code is 'NO VAT'
        // [WHEN] Find "Gen. Posting Setup" records
        // [THEN] There are 3 groups, where "Gen. Prod. Posting Group" = 'NOVAT'
    ...
    begin
        // [FEATURE] [Item sales forecast]
        // [SCENARIO] There are 650 in total item ledger entries 405 sales 241 purchases.
    ...
    begin
        // [SCENARIO] Tax Group NONTAXABLE should be one of 3 groups
        TaxGroup.Get(NONTAXABLETok);
        Assert.RecordCount(TaxGroup, 3);
    end;
    ...
    begin
        with ShipToAddress do begin
            ...
            repeat
                TestField("Country/Region Code", 'CA');
                VerifyPostCodeRegionCountry("Post Code", 'CA');
            until Next = 0;
        end;
    end;
