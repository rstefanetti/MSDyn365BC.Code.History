codeunit 134775 "Word & RDLC Report Selections"
{
    ...
    [HandlerFunctions('StandardStatementPDFHandler')]
    [Scope('OnPrem')]
    procedure OneCustomer_OneSelection_WORD_Blanked_DisabledCheck()
    var
        Customer: Record Customer;
    ...
    begin
        ...
        // [SCENARIO 228763] one WORD report selections, disabled WordMerge check for empty dataset
        Initialize;

        // [GIVEN] Report Selections setup:
        // [GIVEN] Usage = "C.Statement", Sequence = 1, Report ID = "Standard Statement" (WORD)
        // [GIVEN] Customer without entries
        // [GIVEN] Subscribe COD 9651 "Document Report Mgt.".OnBeforeMergeWordDocument() and modify "VerifyXmlHasData" := FALSE
        // [WHEN] Run "Statement" (SaveAs PDF) report
        BindSubscription(WordRDLCReportSelections);
        OneCustomer_OneSelection(CreateCustomer(Customer), REPORT::"Standard Statement", WorkDate, WorkDate);
