page 20296 "Use Case Hierarchies"
{
    Caption = 'Events';
    Editable = false;
    PageType = ListPart;
        ...
        area(Content)
        {
            repeater(Group)
            {
                ...
                field(Enable; EnabledTxt)
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Enabled';
                    ToolTip = 'Specifies use case is enabled for usage.';
                    Editable = false;
                    StyleExpr = true;
                    Style = Subordinate;
                    ...
                    begin
                        if Enabled then
                            ...
                            UseCaseEvent.Get("Event Name");
                            UseCase.Get("Case ID");
                            if (UseCase."Tax Table ID" <> UseCaseEvent."Table ID") and (UseCaseObjectHelper.IsTableRelationEmpty("Case ID", "Table Relation ID")) then
                ...
                field(TableRelation; TableRelationTxt)
                {
                    ...
                    StyleExpr = true;
                    Style = Subordinate;
                    ToolTip = 'Specifies the table relation between use case and event. if the table linked with event and use case are same then relation should be left blank.';
                    trigger OnAssistEdit()
                    var
                        UseCaseEvent: Record "Use Case Event";
                    ...
                    begin
                        UseCaseEvent.Get("Event Name");
                        if UseCaseEvent."Table ID" <> 0 then begin
                            UseCase.Get("Case ID");
                            if IsNullGuid("Table Relation ID") then
                                "Table Relation ID" := UseCaseEntityMgmt.CreateTableLinking("Case ID", UseCase."Tax Table ID", UseCaseEvent."Table ID");
                            Modify();
                            Commit();
                ...
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the sequence of execution on use cases.';
                }
            }
        }
    }
